...
KONOHA.VictoryManager = function(Config)
{
	this.Config = Config;
 ...
 */
KONOHA.VictoryManager.prototype.init = function(gameState)
{
 ...
 */
KONOHA.VictoryManager.prototype.checkEvents = function(gameState, events)
{
	if (gameState.getVictoryConditions().has("wonder"))
	{
...
KONOHA.VictoryManager.prototype.removeCriticalEnt = function(gameState, criticalEntId)
{
 ...
 */
KONOHA.VictoryManager.prototype.manageCriticalEntHealers = function(gameState, queues)
{
	...
	for (let data of this.criticalEnts.values())
	{
		...
		queues.healer.addPlan(new KONOHA.TrainingPlan(gameState, template, { "role": "criticalEntHealer", "base": 0 }, 1, 1));
		return;
	}
};
 ...
 */
KONOHA.VictoryManager.prototype.manageCriticalEntGuards = function(gameState)
{
...
KONOHA.VictoryManager.prototype.tryAssignMilitaryGuard = function(gameState, guardEnt, criticalEnt, checkForSameAccess)
{
	if (guardEnt.getMetadata(PlayerID, "plan") !== undefined ||
	    ...
	    KONOHA.getLandAccess(gameState, criticalEnt) != KONOHA.getLandAccess(gameState, guardEnt)))
		return false;
...
KONOHA.VictoryManager.prototype.pickCriticalEntRetreatLocation = function(gameState, criticalEnt, emergency)
{
	...
	let accessIndex = KONOHA.getLandAccess(gameState, criticalEnt);
	let bestBase = KONOHA.getBestBase(gameState, criticalEnt, true);
 ...
 */
KONOHA.VictoryManager.prototype.assignGuardToCriticalEnt = function(gameState, guardEnt, criticalEntId)
{
	...
	let guardEntAccess = KONOHA.getLandAccess(gameState, guardEnt);
	let criticalEntAccess = KONOHA.getLandAccess(gameState, criticalEnt);
	if (guardEntAccess == criticalEntAccess)
	{
		let queued = KONOHA.returnResources(gameState, guardEnt);
...
KONOHA.VictoryManager.prototype.resetCaptureGaiaRelic = function(gameState)
{
...
KONOHA.VictoryManager.prototype.update = function(gameState, events, queues)
{
 ...
 */
KONOHA.VictoryManager.prototype.captureGaiaRelic = function(gameState, relic)
{
	...
	for (let plan of plans)
	{
		...
		for (let ent of attack.unitCollection.values())
			capture += ent.captureStrength() * KONOHA.getAttackBonus(ent, relic, "Capture");
	}
	// No need to make a new attack if already enough units
	if (capture > sumCapturePoints / 50)
		return;
	let relicPosition = relic.position();
	let access = KONOHA.getLandAccess(gameState, relic);
...
KONOHA.VictoryManager.prototype.abortCaptureGaiaRelic = function(gameState, relicId)
{
...
KONOHA.VictoryManager.prototype.Serialize = function()
{
	return {
...
KONOHA.VictoryManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
