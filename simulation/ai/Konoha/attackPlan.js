...
KONOHA.AttackPlan = function(gameState, Config, uniqueID, type, data)
{
	this.Config = Config;
	this.name = uniqueID;
	...
	for (let base of gameState.ai.HQ.baseManagers)
	{
		if (!base.anchor || !base.anchor.position())
			continue;
		let access = KONOHA.getLandAccess(gameState, base.anchor);
		if (!rallyPoint)
		{
	...
	if (gameState.ai.HQ.navalMap)
	{
		for (let structure of gameState.getEnemyStructures().values())
		{
			...
			if (!structure.position())
				continue;
			let access = KONOHA.getLandAccess(gameState, structure);
			...
			else if (!this.overseas)
			{
				...
				if (!sea)
				{
					if (this.target)
					{
						API3.warn("Konoha: " + this.type + " " + this.name + " has an inaccessible target " +
	...
	{
		priority = 10000;
		this.unitStat.Infantry = { "priority": 1, "minSize": 32, "targetSize": 35, "batchSize": 3, "classes": ["Infantry"],
			"interests": [["strength", 1], ["costsResource", 0.5, "stone"], ["costsResource", 0.6, "metal"]] };
		this.unitStat.FastMoving = { "priority": 1, "minSize": 0, "targetSize": 1, "batchSize": 2, "classes": ["FastMoving+CitizenSoldier"],
...
KONOHA.AttackPlan.prototype.init = function(gameState)
{
...
KONOHA.AttackPlan.prototype.getName = function()
{
	return this.name;
};

KONOHA.AttackPlan.prototype.getType = function()
{
	return this.type;
};

KONOHA.AttackPlan.prototype.isStarted = function()
{
	return this.state !== "unexecuted" && this.state !== "completing";
};

KONOHA.AttackPlan.prototype.isPaused = function()
{
	return this.paused;
};

KONOHA.AttackPlan.prototype.setPaused = function(boolValue)
{
 ...
 */
KONOHA.AttackPlan.prototype.canStart = function()
{
	if (!this.canBuildUnits)
		return true;
...
KONOHA.AttackPlan.prototype.mustStart = function()
{
	if (this.isPaused())
		return false;
...
KONOHA.AttackPlan.prototype.forceStart = function()
{
	for (let unitCat in this.unitStat)
	{
		let Unit = this.unitStat[unitCat];
...
KONOHA.AttackPlan.prototype.emptyQueues = function()
{
	this.queue.empty();
...
KONOHA.AttackPlan.prototype.removeQueues = function(gameState)
{
...
KONOHA.AttackPlan.prototype.addBuildOrder = function(gameState, name, unitStats, resetQueue)
{
	if (!this.isStarted())
	{
...
KONOHA.AttackPlan.prototype.addSiegeUnits = function(gameState)
{
...
KONOHA.AttackPlan.prototype.updatePreparation = function(gameState)
{
	...
	for (let ent of this.unitCollection.values())
	{
		...
		if (ent.resourceCarrying() && ent.resourceCarrying().length)
			queued = KONOHA.returnResources(gameState, ent);
		let index = KONOHA.getLandAccess(gameState, ent);
		if (index == rallyIndex)
...
KONOHA.AttackPlan.prototype.trainMoreUnits = function(gameState)
{
	...
	if (firstOrder[0] < firstOrder[3].targetSize)
	{
		...
		if (queue.length() <= 5)
		{
			...
			else
			{
				...
				let trainingPlan = new KONOHA.TrainingPlan(gameState, template, data, max, max);
...
KONOHA.AttackPlan.prototype.assignUnits = function(gameState)
{
	let plan = this.name;
	let added = false;
	...
	let keep = this.type != "Rush" ?
		/*6 + 4 * gameState.getNumPlayerEnemies() + 8 * this.Config.personality.defensive*/0 : 8; //0: Nagasushi
	...
	for (const ent of gameState.getOwnEntitiesByRole("worker", true).values())
	{
		...
		{
			API3.warn("Konoha problem ent without base ");
...
KONOHA.AttackPlan.prototype.isAvailableUnit = function(gameState, ent)
{
	if (!ent.position())
		return false;
...
KONOHA.AttackPlan.prototype.reassignFastUnit = function(gameState)
{
	for (let ent of this.unitCollection.values())
	{
...
KONOHA.AttackPlan.prototype.chooseTarget = function(gameState)
{
	...
	let targetIndex = KONOHA.getLandAccess(gameState, this.target);
 ...
 */
KONOHA.AttackPlan.prototype.getNearestTarget = function(gameState, position, sameLand)
{
 ...
 */
KONOHA.AttackPlan.prototype.defaultTargetFinder = function(gameState, playerEnemy)
{
...
KONOHA.AttackPlan.prototype.isValidTarget = function(ent)
{
	if (!ent.position())
		return false;
	if (this.sameLand && KONOHA.getLandAccess(this.gameState, ent) != this.sameLand)
		return false;
...
KONOHA.AttackPlan.prototype.rushTargetFinder = function(gameState, playerEnemy)
{
	let targets = new API3.EntityCollection(gameState.sharedScript);
	let buildings;
...
KONOHA.AttackPlan.prototype.raidTargetFinder = function(gameState)
{
	let targets = new API3.EntityCollection(gameState.sharedScript);
 ...
 */
KONOHA.AttackPlan.prototype.checkTargetObstruction = function(gameState, target, position)
{
	if (KONOHA.getLandAccess(gameState, target) != gameState.ai.accessibility.getAccessValue(position))
		return target;
...
KONOHA.AttackPlan.prototype.getPathToTarget = function(gameState, fixedRallyPoint = false)
{
	let startAccess = gameState.ai.accessibility.getAccessValue(this.rallyPoint);
	let endAccess = KONOHA.getLandAccess(gameState, this.target);
...
KONOHA.AttackPlan.prototype.setRallyPoint = function(gameState)
{
 ...
 */
KONOHA.AttackPlan.prototype.StartAttack = function(gameState)
{
	if (this.Config.debug > 1)
	...
	let targetAccess = KONOHA.getLandAccess(gameState, this.target);
...
KONOHA.AttackPlan.prototype.update = function(gameState, events)
{
  ...
  if (this.unitCollection.length < this.nUnits * 0.4 && this.type != "Reinforcement")
  {
	...
	if (this.state == "")
	{
      ...
      {	// if our unit is attacked by a siege unit, we'll send some melee units to help it.
        ...
        {
          let allowCapture = KONOHA.allowCapture(gameState, ent, attacker);
        ...
        if (KONOHA.isSiegeUnit(attacker) && attacker.hasClass("Melee") && ourUnit.hasClass("Melee") && ourUnit.canAttackTarget(attacker, KONOHA.allowCapture(gameState, ourUnit, attacker)))
				{
					ourUnit.attack(attacker.id(), KONOHA.allowCapture(gameState, ourUnit, attacker));
					ourUnit.setMetadata(PlayerID, "lastAttackPlanUpdateTime", time);
				}
      }

			else if (KONOHA.isSiegeUnit(ourUnit) || ourUnit.hasClass("BoltShooter"))
			{	// if our siege units are attacked, we'll send some units to deal with enemies.
				...
				for (let ent of collec.values())
				{
					if (KONOHA.isSiegeUnit(ent))	// needed as mauryan elephants are not filtered out
						continue;
			...
			else
			{
				...
				/*else if (KONOHA.isSiegeUnit(attacker)|| attacker.hasClass("BoltShooter"))
				{	// if our unit is attacked by a siege unit, we'll send some melee units to help it.
				...
				else
				{
					...
					{
						let allowCapture = KONOHA.allowCapture(gameState, ent, attacker);
						if (KONOHA.isSiegeUnit(ent) || !ent.canAttackTarget(attacker, allowCapture))
							continue;
					...
					let allowCapture = KONOHA.allowCapture(gameState, ourUnit, attacker);
		...
		for (let ent of this.unitCollection.values())
		{
			...
			{
				if (KONOHA.isSiegeUnit(target) || target.hasClass("Hero"))
    ...
    let targetClassesPierce;
		if (this.type == "Rush")
			targetClassesUnit = { "attack": ["Unit", "Structure"], "avoid": ["Palisade", "Wall", "Tower", "Fortress", "CivCentre", "Field", "Palisade"], "vetoEntities": veto };
		else
		{
       ...
       targetClassesPierce = { "attack": ["Unit", "Structure"], "avoid": ["Structure", "Field", "Siege"], "vetoEntities": veto };
		...
		for (let check = 0; check < lgth; check++)
		{
			...
			let siegeUnit = KONOHA.isSiegeUnit(ent);
      let pierceUnit = KONOHA.isPierceUnit(ent);
			if (ent.isIdle())
				needsUpdate = true;
			...
			if (this.isBlocked)
			{
  			...
  			range += 60; //Nagaushi
				...
				range += 30;
			range *= range;
			let entAccess = KONOHA.getLandAccess(gameState, ent);
			// Checking for gates if we're a siege unit.
			if (siegeUnit)
			{
				let mStruct = enemyStructures.filter(enemy => {
					if (!enemy.position() || !ent.canAttackTarget(enemy, KONOHA.allowCapture(gameState, ent, enemy)))
						return false;
					if (API3.SquareVectorDistance(enemy.position(), ent.position()) > range)
						return false;
					if (enemy.foundationProgress() == 0)
						return false;
					if (KONOHA.getLandAccess(gameState, enemy) != entAccess)
						return false;
					return true;
				}).toEntityArray();
				if (mStruct.length)
				{
					...
					if (mStruct[0].hasClass("Gate"))
						ent.attack(mStruct[0].id(), KONOHA.allowCapture(gameState, ent, mStruct[0]));
					else
					{
						let rand = randIntExclusive(0, mStruct.length * 0.2);
						ent.attack(mStruct[0].id(), KONOHA.allowCapture(gameState, ent, mStruct[rand]));
					}
				}
				else
				{
			...
			else
			{
				...
				if (mUnit.length)
				{
					...
					ent.attack(mUnit[rand].id(), KONOHA.allowCapture(gameState, ent, mUnit[rand]));
				}
				...
				else
				{
					...
					if (mStruct.length)
					{
						...
						{
							let rand = randIntExclusive(0, mStruct.length * 0.2);
							ent.attack(mStruct[rand].id(), KONOHA.allowCapture(gameState, ent, mStruct[rand]));
						}
					}
					else if (needsUpdate)  // really nothing   let's try to help our nearest unit
					{
						...
						this.unitCollection.forEach(unit => {
							...
							if (dist > distmin)
								return;
							distmin = dist;
							if (!ent.canAttackTarget(target, KONOHA.allowCapture(gameState, ent, target)))
								return;
							attacker = target;
						});
						if (attacker)
							ent.attack(attacker.id(), KONOHA.allowCapture(gameState, ent, attacker));
					}
				}
			}
		}
...
KONOHA.AttackPlan.prototype.UpdateTransporting = function(gameState, events)
{
	let done = true;
	for (let ent of this.unitCollection.values())
	{
...
KONOHA.AttackPlan.prototype.UpdateWalking = function(gameState, events)
{
...
KONOHA.AttackPlan.prototype.UpdateTarget = function(gameState)
{
	...
	if (this.target && this.target.hasClass("Unit"))
	{
		...
		if (!this.targetPos)
		{
			let holder = KONOHA.getHolder(gameState, this.target);
			...
			for (let attackType in attackManager.startedAttacks)
			{
				for (let attack of attackManager.startedAttacks[attackType])
				{
					...
					if (!attack.target || !gameState.getEntityById(attack.target.id()) ||
						...
						continue;
					if (accessIndex != KONOHA.getLandAccess(gameState, attack.target))
						continue;
...
/** reset any units */
KONOHA.AttackPlan.prototype.Abort = function(gameState)
{
...
KONOHA.AttackPlan.prototype.removeUnit = function(ent, update)
{
	if (ent.getMetadata(PlayerID, "role") == "attack")
	{
...
KONOHA.AttackPlan.prototype.checkEvents = function(gameState, events)
{
...
KONOHA.AttackPlan.prototype.waitingForTransport = function()
{
	for (let ent of this.unitCollection.values())
		if (ent.getMetadata(PlayerID, "transport") !== undefined)
			return true;
...
KONOHA.AttackPlan.prototype.hasSiegeUnits = function()
{
	for (let ent of this.unitCollection.values())
		if (KONOHA.isSiegeUnit(ent))
			return true;
	return false;
};

KONOHA.AttackPlan.prototype.hasForceOrder = function(data, value)
{
	for (let ent of this.unitCollection.values())
	{
 ...
 */
KONOHA.AttackPlan.prototype.getAttackAccess = function(gameState)
{
	for (let ent of this.unitCollection.filterNearest(this.position, 1).values())
		return KONOHA.getLandAccess(gameState, ent);

	return 0;
};

KONOHA.AttackPlan.prototype.debugAttack = function()
{
...
KONOHA.AttackPlan.prototype.Serialize = function()
{
	let properties = {
		"name": this.name,
		"type": this.type,
...
KONOHA.AttackPlan.prototype.Deserialize = function(gameState, data)
{
