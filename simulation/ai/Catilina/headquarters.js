...
CATILINA.HQ = function(Config)
{
	...
	this.baseManagers = [];
	this.attackManager = new CATILINA.AttackManager(this.Config);
	this.buildManager = new CATILINA.BuildManager();
	this.defenseManager = new CATILINA.DefenseManager(this.Config);
	this.tradeManager = new CATILINA.TradeManager(this.Config);
	this.navalManager = new CATILINA.NavalManager(this.Config);
	this.researchManager = new CATILINA.ResearchManager(this.Config);
	this.diplomacyManager = new CATILINA.DiplomacyManager(this.Config);
	this.garrisonManager = new CATILINA.GarrisonManager(this.Config);
	this.victoryManager = new CATILINA.VictoryManager(this.Config);
...
CATILINA.HQ.prototype.init = function(gameState, queues)
{
	this.territoryMap = CATILINA.createTerritoryMap(gameState);
	...
	this.borderMap = CATILINA.createBorderMap(gameState);
 ...
 */
CATILINA.HQ.prototype.postinit = function(gameState)
{
 ...
 */
CATILINA.HQ.prototype.createBase = function(gameState, ent, type)
{
	let access = CATILINA.getLandAccess(gameState, ent);
	let newbase;
	for (let base of this.baseManagers)
	{
	...
	if (!newbase)
	{
		newbase = new CATILINA.BaseManager(gameState, this.Config);
 ...
 */
CATILINA.HQ.prototype.getSeaBetweenIndices = function(gameState, index1, index2)
{
...
CATILINA.HQ.prototype.checkEvents = function(gameState, events)
{
	let addBase = false;
	...
	for (let evt of events.OwnershipChanged)   // capture events
	{
		...
		{
			CATILINA.getBestBase(gameState, ent).assignEntity(gameState, ent);
			...
			if (ent.hasClass("Ship"))
				CATILINA.setSeaAccess(gameState, ent);
		...
		else
		{
			...
			if (!ent.decaying() && ent.resourceDropsiteTypes())
				...
				base = CATILINA.getBestBase(gameState, ent) || this.baseManagers[0];
	...
	for (let evt of events.TrainingFinished)
	{
      ...
      let stance = "defensive";
			...
			if (!(ent.hasClass("Melee") && ent.hasClass("Elephant")) && !(ent.hasClass("Melee") && ent.hasClasses("Siege")) && st != stance && st != "passive")
				ent.setStance(stance);//Nagasushi
			...
			if (ent.getMetadata(PlayerID, "role") == "worker")
			{
				...
				if (ent.getMetadata(PlayerID, "base") === undefined)
				{
					base = CATILINA.getBestBase(gameState, ent);
					base.assignEntity(gameState, ent);
				}
				else
			...
			else if (ent.resourceSupplyType() && ent.position())
			{
				...
				{
					if (!dropsite.position() || CATILINA.getLandAccess(gameState, dropsite) != access)
						continue;
...
CATILINA.HQ.prototype.checkPhaseRequirements = function(gameState, queues)
{
	...
	for (let entityReq of requirements)
	{
		...
		case "Town":
			    ...
			    !queues.militaryBuilding.hasQueuedUnits())
			{
				...
				{
					plan = new CATILINA.ConstructionPlan(gameState, "structures/{civ}/market", { "phaseUp": true });
				...
				{
					plan = new CATILINA.ConstructionPlan(gameState, "structures/{civ}/temple", { "phaseUp": true });
					queue = "economicBuilding";
					break;
				}
				...
				{
					plan = new CATILINA.ConstructionPlan(gameState, "structures/{civ}/forge", { "phaseUp": true });
					queue = "militaryBuilding";
					break;
				}
        if (!gameState.getOwnEntitiesByClass("GovernmentCenter", true).hasEntities() &&
				    this.canBuild(gameState, "structures/{civ}/government_center"))
				{
					plan = new CATILINA.ConstructionPlan(gameState, "structures/{civ}/government_center", { "phaseUp": true });
					queue = "militaryBuilding";
					break;
				}
			}
		...
		default:
			...
			if (!queues[queue].hasQueuedUnits())
			{
				...
				if (structure && this.canBuild(gameState, structure))
					plan = new CATILINA.ConstructionPlan(gameState, structure, { "phaseUp": true });
			}
		}
...
CATILINA.HQ.prototype.OnPhaseUp = function(gameState, phase)
{
};

/** This code trains citizen workers, trying to keep close to a ratio of worker/soldiers */
CATILINA.HQ.prototype.trainMoreWorkers = function(gameState, queues)
{
	// default template
	...
	let size = numberOfWorkers < 13 ? 6 : Math.min(5, Math.ceil((numberOfWorkers / 10) + 1)); //Nagasushi
	...
	if (/* numberTotal > this.targetNumWorkers ||*/ (numberTotal >= this.Config.Economy.popPhase2 &&
		...
		return;
	if (numberQueued > 50 || (numberOfQueuedSupports > 20 && numberOfQueuedSoldiers > 20) || numberInTraining > 50) //Nagasushi
		return;
  ...
  supportNum = supportMax * (1 - Math.exp(-alpha*numberTotal/supportMax));
	...
	if (!templateDef || numberOfSupports + numberOfQueuedSupports > supportNum)
	{
		...
		if (numberTotal < 45)
			...
			requirements = [ ["strength", 1], ["canGather", 200]];
  ...
  if (!template && templateDef)
		queues.villager.addPlan(new CATILINA.TrainingPlan(gameState, templateDef, { "role": "worker", "base": 0, "support": true }, size, size));
	else if (template)
		queues.citizenSoldier.addPlan(new CATILINA.TrainingPlan(gameState, template, { "role": "worker", "base": 0 }, size, size));
};

/** picks the best template based on parameters and classes */
CATILINA.HQ.prototype.findBestTrainableUnit = function(gameState, classes, requirements)
{
	let units;
	...
	units.sort((a, b) => {
		...
		for (let param of parameters)
		{
			if (param[0] == "strength")
			{
				aValue += CATILINA.getMaxStrength(a[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance) * param[1];
				bValue += CATILINA.getMaxStrength(b[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance) * param[1];
			}
			else if (param[0] == "siegeStrength")
			{
				aValue += CATILINA.getMaxStrength(a[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance, "Structure") * param[1];
				bValue += CATILINA.getMaxStrength(b[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance, "Structure") * param[1];
			}
 ...
 */
CATILINA.HQ.prototype.bulkPickWorkers = function(gameState, baseRef, number)
{
...
CATILINA.HQ.prototype.getTotalResourceLevel = function(gameState)
{
	let total = {};
 ...
 */
CATILINA.HQ.prototype.GetCurrentGatherRates = function(gameState)
{
 ...
 */
CATILINA.HQ.prototype.GetWantedGatherRates = function(gameState)
{
 ...
 */
CATILINA.HQ.prototype.pickMostNeededResources = function(gameState, allowedResources = [])
{
 ...
 */
CATILINA.HQ.prototype.findEconomicCCLocation = function(gameState, template, resource, proximity, fromStrategic)
{
	...
	// obstruction map
	let obstructions = CATILINA.createObstructionMap(gameState, 0, template);
	let halfSize = 0;
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		else
		{
			...
			for (let cc of ccList)
			{
				...
				if (dist < minDist)
					minDist = dist;
				accessible = accessible || index == CATILINA.getLandAccess(gameState, cc.ent);
			}
			if (norm == 0)
				continue;
			...
			oversea = !accessible && dpList.some(dp => CATILINA.getLandAccess(gameState, dp.ent) == index);
			if (!oversea)
			{
		...
		if (this.borderMap.map[j] & CATILINA.fullBorder_Mask)	// disfavor the borders of the map
			norm *= 0.5;
 ...
 */
CATILINA.HQ.prototype.findStrategicCCLocation = function(gameState, template)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		if (this.borderMap.map[j] & CATILINA.fullBorder_Mask)
			currentVal += 10000;
 ...
 */
CATILINA.HQ.prototype.findMarketLocation = function(gameState, template)
{
  ...
  if (!markets.length)
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		if (this.borderMap.map[j] & CATILINA.narrowFrontier_Mask)
			continue;
		...
		for (let market of markets)
		{
			...
			{
				if (CATILINA.getSeaAccess(gameState, market) != gameState.ai.accessibility.getAccessValue(pos, true))
					continue;
				gainMultiplier = traderTemplatesGains.navalGainMultiplier;
			}
			else if (CATILINA.getLandAccess(gameState, market) == index &&
				!CATILINA.isLineInsideEnemyTerritory(gameState, market.position(), pos))
 ...
 */
CATILINA.HQ.prototype.findDefensiveLocation = function(gameState, template)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		if (!wonderMode)
		{
			...
			if (!(this.borderMap.map[j] & CATILINA.fullFrontier_Mask))
				continue;
			if (this.borderMap.map[j] & CATILINA.largeFrontier_Mask && isTower)
				continue;
		}
...
CATILINA.HQ.prototype.buildTemple = function(gameState, queues)
{
	...
	if (this.canBuild(gameState, "structures/{civ}/temple_vesta"))
		...
		return;
	queues.economicBuilding.addPlan(new CATILINA.ConstructionPlan(gameState, templateName));
};

CATILINA.HQ.prototype.buildMarket = function(gameState, queues)
{
	if (gameState.getOwnEntitiesByClass("Market", true).hasEntities() ||
		!this.canBuild(gameState, "structures/{civ}/market"))
		return;
	...
	let plan = new CATILINA.ConstructionPlan(gameState, "structures/{civ}/market");
	plan.queueToReset = "economicBuilding";
	queues.economicBuilding.addPlan(plan);
};

/** Build a farmstead */
CATILINA.HQ.prototype.buildFarmstead = function(gameState, queues)
{
	...
	queues.economicBuilding.addPlan(new CATILINA.ConstructionPlan(gameState, "structures/{civ}/farmstead"));
};
 ...
 */
CATILINA.HQ.prototype.buildWonder = function(gameState, queues, force = false)
{
	...
	/*		if (resources[res] && resources[res] > 0.7 * cost[res])
				...
				++lowLevel;*/                                                 //Ban Wonder Nagasushi
		}
		if (highLevel == 0 || lowLevel > 1)
			return;
	}

	queues.wonder.addPlan(new CATILINA.ConstructionPlan(gameState, "structures/{civ}/wonder"));
};

/** Build a corral, and train animals there */
CATILINA.HQ.prototype.manageCorral = function(gameState, queues)
{
	                ...
	                nCorral < this.currentPhase && gameState.getPopulation() > 30 * nCorral)
	{
		...
		{
			queues.corral.addPlan(new CATILINA.ConstructionPlan(gameState, "structures/{civ}/corral"));
			return;
		}
		if (!nCorral)
			return;
	...
	for (let corral of gameState.getOwnEntitiesByClass("Corral", true).values())
	{
		...
		for (let trainable of trainables)
		{
			...
			if (count > nCorral)
				continue;
			queues.corral.addPlan(new CATILINA.TrainingPlan(gameState, trainable, { "trainer": corral.id() }));
			return;
		}
	}
};
 ...
 */
CATILINA.HQ.prototype.buildMoreHouses = function(gameState, queues)
{
	...
	{
		let plan = new CATILINA.ConstructionPlan(gameState, houseTemplateString);
	...
	if (freeSlots < 5)
	{
		if (this.buildManager.isUnbuildable(gameState, house))
		{
			...
			priority = 3 * this.Config.priorities.house;//Nagasushi
	}
	else
...
CATILINA.HQ.prototype.checkBaseExpansion = function(gameState, queues)
{
	...
	if (/*numUnits > activeBases * (65 + numvar + (10 + numvar)*(activeBases-1)) || */
       this.saveResources && pop >= 0.96 * gameState.getPopulationMax() && gameState.getOwnEntitiesByClass("Arsenal", true).hasEntities()
     && gameState.getOwnEntitiesByClass("Fortress", true).hasEntities())//Nagasushi
	{
		if (this.Config.debug > 2)
...
CATILINA.HQ.prototype.buildNewBase = function(gameState, queues, resource)
{
	...
	queues.civilCentre.addPlan(new CATILINA.ConstructionPlan(gameState, template, { "base": -1, "resource": resource }));
	return true;
};

/** Deals with building fortresses and towers along our border with enemies. */
CATILINA.HQ.prototype.buildDefenses = function(gameState, queues)
{
	...
	if (!this.saveResources && (this.currentPhase > 2 || gameState.isResearching(gameState.getPhaseName(3))))
	{
		...
		if (this.canBuild(gameState, "structures/{civ}/fortress"))
		{
				...
				gameState.getOwnFoundationsByClass("Fortress").length < 2)
			{
				...
				let plan = new CATILINA.ConstructionPlan(gameState, "structures/{civ}/fortress");
	...
	if (this.Config.Military.numSentryTowers && this.currentPhase < 2 && this.canBuild(gameState, "structures/{civ}/sentry_tower"))
	{
		...
		if (numTowers < this.Config.Military.numSentryTowers && gameState.ai.elapsedTime > towerLapseTime + this.fortStartTime)
		{
			...
			queues.defenseBuilding.addPlan(new CATILINA.ConstructionPlan(gameState, "structures/{civ}/sentry_tower"));
		}
		return;
	}
		...
		gameState.getOwnFoundationsByClass("Tower").length < 3)
	{
		...
		let plan = new CATILINA.ConstructionPlan(gameState, "structures/{civ}/defense_tower");
		plan.queueToReset = "defenseBuilding";
		queues.defenseBuilding.addPlan(plan);
	}
};

CATILINA.HQ.prototype.buildForge = function(gameState, queues)
{
	if (this.getAccountedPopulation(gameState) < this.Config.Military.popForForge ||
		...
		return;
	// Build a Market before the Forge. //Build a government centre if we are Han. //Nagasushi
	if (!gameState.getOwnEntitiesByClass("Market", true).hasEntities())
		return;

  if(this.canBuild(gameState, "structures/{civ}/government_center") && !gameState.getOwnEntitiesByClass("GovernmentCenter", true).hasEntities())
    return;

	if (this.canBuild(gameState, "structures/{civ}/forge"))
		queues.militaryBuilding.addPlan(new CATILINA.ConstructionPlan(gameState, "structures/{civ}/forge"));
};

CATILINA.HQ.prototype.buildGovernment = function(gameState, queues)
{
	if (this.getAccountedPopulation(gameState) < this.Config.Military.popForForge ||
		queues.militaryBuilding.hasQueuedUnits() || gameState.getOwnEntitiesByClass("GovernmentCenter", true).length)
		return;
	// Build a Market before the government centre. //Build a government centre if we are Han. //Nagasushi
	if (!gameState.getOwnEntitiesByClass("Market", true).hasEntities())
		return;
	if (this.canBuild(gameState, "structures/{civ}/government_center"))
  queues.militaryBuilding.addPlan(new CATILINA.ConstructionPlan(gameState, "structures/{civ}/government_center"));
};
 ...
 */
CATILINA.HQ.prototype.constructTrainingBuildings = function(gameState, queues)
{
	    ...
	    this.phasing == 2 && gameState.getOwnStructures().filter(API3.Filters.byClass("Village")).length < 5)
	{
		...
		if (numBarracks + numRanges == 0)
		{
			...
			if (template)
			{
				...
				let plan = new CATILINA.ConstructionPlan(gameState, template, { "militaryBase": true });
		...
		if (numBarracks + numRanges == 1 && this.getAccountedPopulation(gameState) > this.Config.Military.popForBarracks2)
		{
			...
			if (template)
			{
				queues.militaryBuilding.addPlan(new CATILINA.ConstructionPlan(gameState, template, { "militaryBase": true }));
				return;
			}
		}
    ...
    if (numStables == 0 && stableTemplate && this.getAccountedPopulation(gameState) > this.Config.Military.popForBarracks2 + 60)
		{
			queues.militaryBuilding.addPlan(new CATILINA.ConstructionPlan(gameState, stableTemplate, { "militaryBase": true }));
			return;
		}

                // Fourth barracks/range and stable, if needed.
		if (numBarracks + numRanges + numStables == 4 && this.getAccountedPopulation(gameState) > this.Config.Military.popForBarracks2 + 100)
		{
			let template = barracksTemplate || stableTemplate || rangeTemplate;
			if (template)
			{
    ...
    if (numStables == 1 && stableTemplate && this.getAccountedPopulation(gameState) > this.Config.Military.popForBarracks2 + 100)
    {
      queues.militaryBuilding.addPlan(new CATILINA.ConstructionPlan(gameState, stableTemplate, { "militaryBase": true }));
      return;
    }
                // Fifth barracks/range and stable, if needed.
		if (numBarracks + numRanges + numStables == 6 && this.getAccountedPopulation(gameState) > this.Config.Military.popForBarracks2 + 110)
		{
			let template = barracksTemplate || stableTemplate || rangeTemplate;
			if (template)
			{
	...
	if (!nAdvanced || nAdvanced < this.bAdvanced.length && this.getAccountedPopulation(gameState) > 110)
	{
		for (let advanced of this.bAdvanced)
		{
			...
			if (template.hasDefensiveFire() || template.trainableEntities(civ))
				queues.militaryBuilding.addPlan(new CATILINA.ConstructionPlan(gameState, advanced, { "militaryBase": true }));
			else	// not a military building, but still use this queue
				queues.militaryBuilding.addPlan(new CATILINA.ConstructionPlan(gameState, advanced));
			return;
		}
	}
};
 ...
 */
CATILINA.HQ.prototype.findBestBaseForMilitary = function(gameState)
{
	...
	for (let cce of ccEnts)
	{
		...
		if (enemyFound && !gameState.isPlayerEnemy(cce.owner()))
			continue;
		let access = CATILINA.getLandAccess(gameState, cce);
		let isEnemy = gameState.isPlayerEnemy(cce.owner());
		for (let cc of ccEnts)
		{
			if (cc.owner() != PlayerID)
				continue;
			if (CATILINA.getLandAccess(gameState, cc) != access)
				continue;
 ...
 */
CATILINA.HQ.prototype.trainEmergencyUnits = function(gameState, positions)
{
	...
	for (let pos of positions)
	{
		...
		for (let base of this.baseManagers)
		{
			if (!base.anchor || !base.anchor.position())
				continue;
			if (CATILINA.getLandAccess(gameState, base.anchor) != access)
				continue;
	...
	gameState.ai.queues.emergency.addPlan(new CATILINA.TrainingPlan(gameState, templateFound[0], metadata, 1, 1));
	return true;
};

CATILINA.HQ.prototype.canBuild = function(gameState, structure)
{
...
CATILINA.HQ.prototype.updateTerritories = function(gameState)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		if (this.borderMap.map[j] & CATILINA.outside_Mask)
			continue;
		if (this.borderMap.map[j] & CATILINA.fullFrontier_Mask)
			this.borderMap.map[j] &= ~CATILINA.fullFrontier_Mask;	// reset the frontier
		...
		else
		{
			...
			for (let a of around)
			{
				...
				if (jz < 0 || jz >= width)
					continue;
				if (this.borderMap.map[jx+width*jz] & CATILINA.outside_Mask)
					continue;
				...
				{
					this.borderMap.map[j] |= CATILINA.narrowFrontier_Mask;
					break;
				}
			...
			if (onFrontier && !(this.borderMap.map[j] & CATILINA.narrowFrontier_Mask))
				this.borderMap.map[j] |= CATILINA.largeFrontier_Mask;
...
CATILINA.HQ.prototype.reassignTerritories = function(deletedBase)
{
	let cellSize = this.territoryMap.cellSize;
	let width = this.territoryMap.width;
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		if (this.territoryMap.getOwnerIndex(j) != PlayerID)
		{
			API3.warn("Catilina reassignTerritories: should never happen");
			this.basesMap.map[j] = 0;
			continue;
		}
 ...
 */
CATILINA.HQ.prototype.getBaseByID = function(baseID)
{
	for (let base of this.baseManagers)
		if (base.ID == baseID)
			return base;
 ...
 */
CATILINA.HQ.prototype.numActiveBases = function()
{
...
CATILINA.HQ.prototype.numPotentialBases = function()
{
	if (!this.turnCache.base)
		this.updateBaseCache();
...
CATILINA.HQ.prototype.updateBaseCache = function()
{
...
CATILINA.HQ.prototype.resetBaseCache = function()
{
 ...
 */
CATILINA.HQ.prototype.assignGatherers = function()
{
	for (let base of this.baseManagers)
	{
...
CATILINA.HQ.prototype.isDangerousLocation = function(gameState, pos, radius)
{
...
CATILINA.HQ.prototype.isNearInvadingArmy = function(pos)
{
...
CATILINA.HQ.prototype.isUnderEnemyFire = function(gameState, pos, radius = 0)
{
...
CATILINA.HQ.prototype.updateCaptureStrength = function(gameState)
{
	...
	for (let ent of gameState.getOwnUnits().values())
	{
		...
		if (!this.capturableTargets.has(targetId))
			this.capturableTargets.set(targetId, {
				"strength": ent.captureStrength() * CATILINA.getAttackBonus(ent, target, "Capture"),
				"ents": new Set([ent.id()])
			});
		else
		{
			let capturableTarget = this.capturableTargets.get(target.id());
			capturableTarget.strength += ent.captureStrength() * CATILINA.getAttackBonus(ent, target, "Capture");
	...
	for (let [targetId, capturableTarget] of this.capturableTargets)
	{
		...
		for (let entId of capturableTarget.ents)
		{
			...
			if (allowCapture === undefined)
				allowCapture = CATILINA.allowCapture(gameState, ent, target);
			let orderData = ent.unitAIOrderData();
...
CATILINA.HQ.prototype.AddTCGatherer = function(supplyID)
{
...
CATILINA.HQ.prototype.RemoveTCGatherer = function(supplyID)
{
...
CATILINA.HQ.prototype.GetTCGatherer = function(supplyID)
{
	if (this.turnCache.resourceGatherer && this.turnCache.resourceGatherer[supplyID])
...
CATILINA.HQ.prototype.AddTCResGatherer = function(resource)
{
...
CATILINA.HQ.prototype.GetTCResGatherer = function(resource)
{
	if (this.turnCache["resourceGatherer-" + resource])
 ...
 */
CATILINA.HQ.prototype.isResourceExhausted = function(resource)
{
 ...
 */
CATILINA.HQ.prototype.isDefendable = function(ent)
{
 ...
 */
CATILINA.HQ.prototype.getAccountedPopulation = function(gameState)
{
 ...
 */
CATILINA.HQ.prototype.getAccountedWorkers = function(gameState)
{
 ...
 */
CATILINA.HQ.prototype.update = function(gameState, queues, events)
{
	...
	*/
	...
	if (this.currentPhase > 1 && gameState.ai.playedTurn % 3 == 0)
	{
      ...
      this.buildGovernment(gameState, queues);//Nagasushi Han support
		}
...
CATILINA.HQ.prototype.Serialize = function()
{
	let properties = {
		"phasing": this.phasing,
...
CATILINA.HQ.prototype.Deserialize = function(gameState, data)
{
	...
	for (let base of data.baseManagers)
	{
		...
		let newbase = new CATILINA.BaseManager(gameState, this.Config);
