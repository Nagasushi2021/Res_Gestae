...
KONOHA.HQ = function(Config)
{
	...
	this.baseManagers = [];
	this.attackManager = new KONOHA.AttackManager(this.Config);
	this.buildManager = new KONOHA.BuildManager();
	this.defenseManager = new KONOHA.DefenseManager(this.Config);
	this.tradeManager = new KONOHA.TradeManager(this.Config);
	this.navalManager = new KONOHA.NavalManager(this.Config);
	this.researchManager = new KONOHA.ResearchManager(this.Config);
	this.diplomacyManager = new KONOHA.DiplomacyManager(this.Config);
	this.garrisonManager = new KONOHA.GarrisonManager(this.Config);
	this.victoryManager = new KONOHA.VictoryManager(this.Config);
...
KONOHA.HQ.prototype.init = function(gameState, queues)
{
	this.territoryMap = KONOHA.createTerritoryMap(gameState);
	...
	this.borderMap = KONOHA.createBorderMap(gameState);
 ...
 */
KONOHA.HQ.prototype.postinit = function(gameState)
{
 ...
 */
KONOHA.HQ.prototype.createBase = function(gameState, ent, type)
{
	let access = KONOHA.getLandAccess(gameState, ent);
	let newbase;
	for (let base of this.baseManagers)
	{
	...
	if (!newbase)
	{
		newbase = new KONOHA.BaseManager(gameState, this.Config);
 ...
 */
KONOHA.HQ.prototype.getSeaBetweenIndices = function(gameState, index1, index2)
{
...
KONOHA.HQ.prototype.checkEvents = function(gameState, events)
{
	let addBase = false;
	...
	for (let evt of events.OwnershipChanged)   // capture events
	{
		...
		{
			KONOHA.getBestBase(gameState, ent).assignEntity(gameState, ent);
			...
			if (ent.hasClass("Ship"))
				KONOHA.setSeaAccess(gameState, ent);
		...
		else
		{
			...
			if (!ent.decaying() && ent.resourceDropsiteTypes())
				...
				base = KONOHA.getBestBase(gameState, ent) || this.baseManagers[0];
	...
	for (let evt of events.TrainingFinished)
	{
      ...
      let stance = "defensive";
			...
			if (ent.getMetadata(PlayerID, "role") == "worker")
			{
				...
				if (ent.getMetadata(PlayerID, "base") === undefined)
				{
					base = KONOHA.getBestBase(gameState, ent);
					base.assignEntity(gameState, ent);
				}
				else
			...
			else if (ent.resourceSupplyType() && ent.position())
			{
				...
				{
					if (!dropsite.position() || KONOHA.getLandAccess(gameState, dropsite) != access)
						continue;
...
KONOHA.HQ.prototype.checkPhaseRequirements = function(gameState, queues)
{
	...
	for (let entityReq of requirements)
	{
		...
		case "Town":
			    ...
			    !queues.militaryBuilding.hasQueuedUnits())
			{
				...
				{
					plan = new KONOHA.ConstructionPlan(gameState, "structures/{civ}/market", { "phaseUp": true });
				...
				{
					plan = new KONOHA.ConstructionPlan(gameState, "structures/{civ}/temple", { "phaseUp": true });
					queue = "economicBuilding";
					break;
				}
				...
				{
					plan = new KONOHA.ConstructionPlan(gameState, "structures/{civ}/forge", { "phaseUp": true });
        ...
        if (!gameState.getOwnEntitiesByClass("GovernmentCenter", true).hasEntities() &&
				...
				{
					plan = new KONOHA.ConstructionPlan(gameState, "structures/{civ}/government_center", { "phaseUp": true });
					queue = "militaryBuilding";
					break;
				}
			}
		...
		default:
			...
			if (!queues[queue].hasQueuedUnits())
			{
				...
				if (structure && this.canBuild(gameState, structure))
					plan = new KONOHA.ConstructionPlan(gameState, structure, { "phaseUp": true });
			}
		}
...
KONOHA.HQ.prototype.OnPhaseUp = function(gameState, phase)
{
};

/** This code trains citizen workers, trying to keep close to a ratio of worker/soldiers */
KONOHA.HQ.prototype.trainMoreWorkers = function(gameState, queues)
{
	// default template
	...
	if (!template && templateDef)
		queues.villager.addPlan(new KONOHA.TrainingPlan(gameState, templateDef, { "role": "worker", "base": 0, "support": true }, size, size));
	else if (template)
		queues.citizenSoldier.addPlan(new KONOHA.TrainingPlan(gameState, template, { "role": "worker", "base": 0 }, size, size));
};

/** picks the best template based on parameters and classes */
KONOHA.HQ.prototype.findBestTrainableUnit = function(gameState, classes, requirements)
{
	let units;
	...
	units.sort((a, b) => {
		...
		for (let param of parameters)
		{
			if (param[0] == "strength")
			{
				aValue += KONOHA.getMaxStrength(a[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance) * param[1];
				bValue += KONOHA.getMaxStrength(b[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance) * param[1];
			}
			else if (param[0] == "siegeStrength")
			{
				aValue += KONOHA.getMaxStrength(a[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance, "Structure") * param[1];
				bValue += KONOHA.getMaxStrength(b[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance, "Structure") * param[1];
			}
 ...
 */
KONOHA.HQ.prototype.bulkPickWorkers = function(gameState, baseRef, number)
{
...
KONOHA.HQ.prototype.getTotalResourceLevel = function(gameState)
{
	let total = {};
 ...
 */
KONOHA.HQ.prototype.GetCurrentGatherRates = function(gameState)
{
 ...
 */
KONOHA.HQ.prototype.GetWantedGatherRates = function(gameState)
{
 ...
 */
KONOHA.HQ.prototype.pickMostNeededResources = function(gameState, allowedResources = [])
{
 ...
 */
KONOHA.HQ.prototype.findEconomicCCLocation = function(gameState, template, resource, proximity, fromStrategic)
{
	...
	// obstruction map
	let obstructions = KONOHA.createObstructionMap(gameState, 0, template);
	let halfSize = 0;
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		else
		{
			...
			for (let cc of ccList)
			{
				...
				if (dist < minDist)
					minDist = dist;
				accessible = accessible || index == KONOHA.getLandAccess(gameState, cc.ent);
			}
			if (norm == 0)
				continue;
			...
			oversea = !accessible && dpList.some(dp => KONOHA.getLandAccess(gameState, dp.ent) == index);
			if (!oversea)
			{
		...
		if (this.borderMap.map[j] & KONOHA.fullBorder_Mask)	// disfavor the borders of the map
			norm *= 0.5;
 ...
 */
KONOHA.HQ.prototype.findStrategicCCLocation = function(gameState, template)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		if (this.borderMap.map[j] & KONOHA.fullBorder_Mask)
			currentVal += 10000;
 ...
 */
KONOHA.HQ.prototype.findMarketLocation = function(gameState, template)
{
  ...
  if (!markets.length)
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		if (this.borderMap.map[j] & KONOHA.narrowFrontier_Mask)
			continue;
		...
		for (let market of markets)
		{
			...
			{
				if (KONOHA.getSeaAccess(gameState, market) != gameState.ai.accessibility.getAccessValue(pos, true))
					continue;
				gainMultiplier = traderTemplatesGains.navalGainMultiplier;
			}
			else if (KONOHA.getLandAccess(gameState, market) == index &&
				!KONOHA.isLineInsideEnemyTerritory(gameState, market.position(), pos))
 ...
 */
KONOHA.HQ.prototype.findDefensiveLocation = function(gameState, template)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		if (!wonderMode)
		{
			...
			if (!(this.borderMap.map[j] & KONOHA.fullFrontier_Mask))
				continue;
			if (this.borderMap.map[j] & KONOHA.largeFrontier_Mask && isTower)
				continue;
		}
...
KONOHA.HQ.prototype.buildTemple = function(gameState, queues)
{
	...
	if (this.canBuild(gameState, "structures/{civ}/temple_vesta"))
		...
		return;
	queues.economicBuilding.addPlan(new KONOHA.ConstructionPlan(gameState, templateName));
};

KONOHA.HQ.prototype.buildMarket = function(gameState, queues)
{
	if (gameState.getOwnEntitiesByClass("Market", true).hasEntities() ||
		!this.canBuild(gameState, "structures/{civ}/market"))
		return;
	...
	let plan = new KONOHA.ConstructionPlan(gameState, "structures/{civ}/market");
	plan.queueToReset = "economicBuilding";
	queues.economicBuilding.addPlan(plan);
};

/** Build a farmstead */
KONOHA.HQ.prototype.buildFarmstead = function(gameState, queues)
{
	...
	queues.economicBuilding.addPlan(new KONOHA.ConstructionPlan(gameState, "structures/{civ}/farmstead"));
};
 ...
 */
KONOHA.HQ.prototype.buildWonder = function(gameState, queues, force = false)
{
	...
	queues.wonder.addPlan(new KONOHA.ConstructionPlan(gameState, "structures/{civ}/wonder"));
};

/** Build a corral, and train animals there */
KONOHA.HQ.prototype.manageCorral = function(gameState, queues)
{
	                ...
	                nCorral < this.currentPhase && gameState.getPopulation() > 30 * nCorral)
	{
		...
		{
			queues.corral.addPlan(new KONOHA.ConstructionPlan(gameState, "structures/{civ}/corral"));
			return;
		}
		if (!nCorral)
			return;
	...
	for (let corral of gameState.getOwnEntitiesByClass("Corral", true).values())
	{
		...
		for (let trainable of trainables)
		{
			...
			if (count > nCorral)
				continue;
			queues.corral.addPlan(new KONOHA.TrainingPlan(gameState, trainable, { "trainer": corral.id() }));
			return;
		}
	}
};
 ...
 */
KONOHA.HQ.prototype.buildMoreHouses = function(gameState, queues)
{
	...
	{
		let plan = new KONOHA.ConstructionPlan(gameState, houseTemplateString);
...
KONOHA.HQ.prototype.checkBaseExpansion = function(gameState, queues)
{
...
KONOHA.HQ.prototype.buildNewBase = function(gameState, queues, resource)
{
	...
	queues.civilCentre.addPlan(new KONOHA.ConstructionPlan(gameState, template, { "base": -1, "resource": resource }));
	return true;
};

/** Deals with building fortresses and towers along our border with enemies. */
KONOHA.HQ.prototype.buildDefenses = function(gameState, queues)
{
	...
	if (!this.saveResources && (this.currentPhase > 2 || gameState.isResearching(gameState.getPhaseName(3))))
	{
		...
		if (this.canBuild(gameState, "structures/{civ}/fortress"))
		{
				...
				gameState.getOwnFoundationsByClass("Fortress").length < 2)
			{
				...
				let plan = new KONOHA.ConstructionPlan(gameState, "structures/{civ}/fortress");
	...
	if (this.Config.Military.numSentryTowers && this.currentPhase < 2 && this.canBuild(gameState, "structures/{civ}/sentry_tower"))
	{
		...
		if (numTowers < this.Config.Military.numSentryTowers && gameState.ai.elapsedTime > towerLapseTime + this.fortStartTime)
		{
			...
			queues.defenseBuilding.addPlan(new KONOHA.ConstructionPlan(gameState, "structures/{civ}/sentry_tower"));
		}
		return;
	}
		...
		gameState.getOwnFoundationsByClass("Tower").length < 3)
	{
		...
		let plan = new KONOHA.ConstructionPlan(gameState, "structures/{civ}/defense_tower");
		plan.queueToReset = "defenseBuilding";
		queues.defenseBuilding.addPlan(plan);
	}
};

KONOHA.HQ.prototype.buildForge = function(gameState, queues)
{
  ...
  if(this.canBuild(gameState, "structures/{civ}/government_center") && !gameState.getOwnEntitiesByClass("GovernmentCenter", true).hasEntities())
	...
	if (this.canBuild(gameState, "structures/{civ}/forge"))
		queues.militaryBuilding.addPlan(new KONOHA.ConstructionPlan(gameState, "structures/{civ}/forge"));
};

KONOHA.HQ.prototype.buildGovernment = function(gameState, queues)
{
	if (this.getAccountedPopulation(gameState) < this.Config.Military.popForForge ||
  ...
  queues.militaryBuilding.addPlan(new KONOHA.ConstructionPlan(gameState, "structures/{civ}/government_center"));
};
 ...
 */
KONOHA.HQ.prototype.constructTrainingBuildings = function(gameState, queues)
{
	    ...
	    this.phasing == 2 && gameState.getOwnStructures().filter(API3.Filters.byClass("Village")).length < 5)
	{
		...
		if (numBarracks + numRanges == 0)
		{
			...
			if (template)
			{
				...
				let plan = new KONOHA.ConstructionPlan(gameState, template, { "militaryBase": true });
		...
		{
			queues.militaryBuilding.addPlan(new KONOHA.ConstructionPlan(gameState, stableTemplate, { "militaryBase": true }));
			return;
		}*/
		...
		if (numBarracks + numRanges == 1 && this.getAccountedPopulation(gameState) > this.Config.Military.popForBarracks2)
		{
			...
			if (template)
			{
				queues.militaryBuilding.addPlan(new KONOHA.ConstructionPlan(gameState, template, { "militaryBase": true }));
				return;
			}
		}
	...
	{
		queues.militaryBuilding.addPlan(new KONOHA.ConstructionPlan(gameState, "structures/{civ}/elephant_stable", { "militaryBase": true }));
		return;
	}

	if (this.canBuild(gameState, "structures/{civ}/arsenal") && !gameState.getOwnEntitiesByClass("Arsenal", true).hasEntities())
	{
		queues.militaryBuilding.addPlan(new KONOHA.ConstructionPlan(gameState, "structures/{civ}/arsenal", { "militaryBase": true }));
		return;
	}
	...
	if (!nAdvanced || nAdvanced < this.bAdvanced.length && this.getAccountedPopulation(gameState) > 110)
	{
		for (let advanced of this.bAdvanced)
		{
			...
			if (template.hasDefensiveFire() || template.trainableEntities(civ))
				queues.militaryBuilding.addPlan(new KONOHA.ConstructionPlan(gameState, advanced, { "militaryBase": true }));
			else	// not a military building, but still use this queue
				queues.militaryBuilding.addPlan(new KONOHA.ConstructionPlan(gameState, advanced));
			return;
		}
	}
};
 ...
 */
KONOHA.HQ.prototype.findBestBaseForMilitary = function(gameState)
{
	...
	for (let cce of ccEnts)
	{
		...
		if (enemyFound && !gameState.isPlayerEnemy(cce.owner()))
			continue;
		let access = KONOHA.getLandAccess(gameState, cce);
		let isEnemy = gameState.isPlayerEnemy(cce.owner());
		for (let cc of ccEnts)
		{
			if (cc.owner() != PlayerID)
				continue;
			if (KONOHA.getLandAccess(gameState, cc) != access)
				continue;
 ...
 */
KONOHA.HQ.prototype.trainEmergencyUnits = function(gameState, positions)
{
	...
	for (let pos of positions)
	{
		...
		for (let base of this.baseManagers)
		{
			if (!base.anchor || !base.anchor.position())
				continue;
			if (KONOHA.getLandAccess(gameState, base.anchor) != access)
				continue;
	...
	gameState.ai.queues.emergency.addPlan(new KONOHA.TrainingPlan(gameState, templateFound[0], metadata, 1, 1));
	return true;
};

KONOHA.HQ.prototype.canBuild = function(gameState, structure)
{
...
KONOHA.HQ.prototype.updateTerritories = function(gameState)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		if (this.borderMap.map[j] & KONOHA.outside_Mask)
			continue;
		if (this.borderMap.map[j] & KONOHA.fullFrontier_Mask)
			this.borderMap.map[j] &= ~KONOHA.fullFrontier_Mask;	// reset the frontier
		...
		else
		{
			...
			for (let a of around)
			{
				...
				if (jz < 0 || jz >= width)
					continue;
				if (this.borderMap.map[jx+width*jz] & KONOHA.outside_Mask)
					continue;
				...
				{
					this.borderMap.map[j] |= KONOHA.narrowFrontier_Mask;
					break;
				}
			...
			if (onFrontier && !(this.borderMap.map[j] & KONOHA.narrowFrontier_Mask))
				this.borderMap.map[j] |= KONOHA.largeFrontier_Mask;
...
KONOHA.HQ.prototype.reassignTerritories = function(deletedBase)
{
	let cellSize = this.territoryMap.cellSize;
	let width = this.territoryMap.width;
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		if (this.territoryMap.getOwnerIndex(j) != PlayerID)
		{
			API3.warn("Konoha reassignTerritories: should never happen");
			this.basesMap.map[j] = 0;
			continue;
		}
 ...
 */
KONOHA.HQ.prototype.getBaseByID = function(baseID)
{
	for (let base of this.baseManagers)
		if (base.ID == baseID)
			return base;
 ...
 */
KONOHA.HQ.prototype.numActiveBases = function()
{
...
KONOHA.HQ.prototype.numPotentialBases = function()
{
	if (!this.turnCache.base)
		this.updateBaseCache();
...
KONOHA.HQ.prototype.updateBaseCache = function()
{
...
KONOHA.HQ.prototype.resetBaseCache = function()
{
 ...
 */
KONOHA.HQ.prototype.assignGatherers = function()
{
	for (let base of this.baseManagers)
	{
...
KONOHA.HQ.prototype.isDangerousLocation = function(gameState, pos, radius)
{
...
KONOHA.HQ.prototype.isNearInvadingArmy = function(pos)
{
...
KONOHA.HQ.prototype.isUnderEnemyFire = function(gameState, pos, radius = 0)
{
...
KONOHA.HQ.prototype.updateCaptureStrength = function(gameState)
{
	...
	for (let ent of gameState.getOwnUnits().values())
	{
		...
		if (!this.capturableTargets.has(targetId))
			this.capturableTargets.set(targetId, {
				"strength": ent.captureStrength() * KONOHA.getAttackBonus(ent, target, "Capture"),
				"ents": new Set([ent.id()])
			});
		else
		{
			let capturableTarget = this.capturableTargets.get(target.id());
			capturableTarget.strength += ent.captureStrength() * KONOHA.getAttackBonus(ent, target, "Capture");
	...
	for (let [targetId, capturableTarget] of this.capturableTargets)
	{
		...
		for (let entId of capturableTarget.ents)
		{
			...
			if (allowCapture === undefined)
				allowCapture = KONOHA.allowCapture(gameState, ent, target);
			let orderData = ent.unitAIOrderData();
...
KONOHA.HQ.prototype.AddTCGatherer = function(supplyID)
{
...
KONOHA.HQ.prototype.RemoveTCGatherer = function(supplyID)
{
...
KONOHA.HQ.prototype.GetTCGatherer = function(supplyID)
{
	if (this.turnCache.resourceGatherer && this.turnCache.resourceGatherer[supplyID])
...
KONOHA.HQ.prototype.AddTCResGatherer = function(resource)
{
...
KONOHA.HQ.prototype.GetTCResGatherer = function(resource)
{
	if (this.turnCache["resourceGatherer-" + resource])
 ...
 */
KONOHA.HQ.prototype.isResourceExhausted = function(resource)
{
 ...
 */
KONOHA.HQ.prototype.isDefendable = function(ent)
{
 ...
 */
KONOHA.HQ.prototype.getAccountedPopulation = function(gameState)
{
 ...
 */
KONOHA.HQ.prototype.getAccountedWorkers = function(gameState)
{
 ...
 */
KONOHA.HQ.prototype.update = function(gameState, queues, events)
{
...
KONOHA.HQ.prototype.Serialize = function()
{
	let properties = {
		"phasing": this.phasing,
...
KONOHA.HQ.prototype.Deserialize = function(gameState, data)
{
	...
	for (let base of data.baseManagers)
	{
		...
		let newbase = new KONOHA.BaseManager(gameState, this.Config);
