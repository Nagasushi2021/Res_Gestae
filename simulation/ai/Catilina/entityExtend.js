...
CATILINA.isSiegeUnit = function(ent)
{
	return (ent.hasClass("Siege") && ent.hasClass("Melee")) || (ent.hasClass("Elephant") && ent.hasClass("Melee")) || ent.hasClass("StoneThrower");
};//Nagasushi

CATILINA.isPierceUnit = function(ent)
{
	return (ent.hasClass("Ranged") && !ent.hasClass("StoneThrower") && !ent.hasClass("StoneThrower"));
};//Nagasushi

/** returns true if this unit should be considered as "fast". */
CATILINA.isFastMoving = function(ent)
{
...
CATILINA.getMaxStrength = function(ent, debugLevel, DamageTypeImportance, againstClass)
{
	let strength = 0;
	...
	for (let type of attackTypes)
	{
		...
		for (let str in attackStrength)
		{
			...
			if (DamageTypeImportance[str])
				...
				API3.warn("Catilina: " + str + " unknown attackStrength in getMaxStrength (please add " + str + "  to config.js).");
		}
		...
		for (let str in attackTimes)
		{
			...
			case "prepare":
				...
				break;
			default:
				API3.warn("Catilina: " + str + " unknown attackTimes in getMaxStrength");
			}
		}
	}
	...
	if (resistanceStrength.Damage)
		for (let str in resistanceStrength.Damage)
		{
			...
			if (DamageTypeImportance[str])
				...
				API3.warn("Catilina: " + str + " unknown resistanceStrength in getMaxStrength (please add " + str + "  to config.js).");
		}
...
CATILINA.getLandAccess = function(gameState, ent)
{
	if (ent.hasClass("Unit"))
	{
		...
		if (!pos)
		{
			let holder = CATILINA.getHolder(gameState, ent);
			if (holder)
				return CATILINA.getLandAccess(gameState, holder);

			API3.warn("Catilina error: entity without position, but not garrisoned");
...
CATILINA.getSeaAccess = function(gameState, ent)
{
...
CATILINA.setSeaAccess = function(gameState, ent)
{
	CATILINA.getSeaAccess(gameState, ent);
};

/** Decide if we should try to capture (returns true) or destroy (return false) */
CATILINA.allowCapture = function(gameState, ent, target)
{
	...
	{
		capture = ent.captureStrength() * CATILINA.getAttackBonus(ent, target, "Capture");
	...
	else
	{
		...
		{
			capturable.strength += ent.captureStrength() * CATILINA.getAttackBonus(ent, target, "Capture");
...
CATILINA.getAttackBonus = function(ent, target, type)
{
	let attackBonus = 1;
...
CATILINA.returnResources = function(gameState, ent)
{
	...
	for (let dropsite of dropsiteCollection.values())
	{
		...
		if (owner !== PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
			continue;
		if (CATILINA.getLandAccess(gameState, dropsite) != access)
			continue;
...
CATILINA.IsSupplyFull = function(gameState, ent)
{
 ...
 */
CATILINA.getBestBase = function(gameState, ent, onlyConstructedBase = false, exclude = false)
{
	let pos = ent.position();
	let accessIndex;
	if (!pos)
	{
		...
		pos = holder.position();
		accessIndex = CATILINA.getLandAccess(gameState, holder);
	}
	else
		accessIndex = CATILINA.getLandAccess(gameState, ent);
...
CATILINA.getHolder = function(gameState, ent)
{
...
CATILINA.getBuiltEntity = function(gameState, ent)
{
 ...
 */
CATILINA.isNotWorthBuilding = function(gameState, ent)
{
 ...
 */
CATILINA.isLineInsideEnemyTerritory = function(gameState, pos1, pos2, step=70)
{
...
CATILINA.gatherTreasure = function(gameState, ent, water = false)
{
	...
	let treasureFound;
	let distmin = Math.min();
	let access = water ? CATILINA.getSeaAccess(gameState, ent) : CATILINA.getLandAccess(gameState, ent);
	for (let treasure of gameState.ai.HQ.treasures.values())
	{
		...
		if (lastGathered && gameState.ai.elapsedTime - lastGathered < 20)
			continue;
		if (!water && access != CATILINA.getLandAccess(gameState, treasure))
			continue;
		if (water && access != CATILINA.getSeaAccess(gameState, treasure))
			continue;
...
CATILINA.dumpEntity = function(ent)
{
	if (!ent)
		return;
