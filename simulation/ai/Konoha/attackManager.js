 ...
 * Attack Manager
 */
KONOHA.AttackManager = function(Config)
{
	this.Config = Config;
...
KONOHA.AttackManager.prototype.checkEvents = function(gameState, events)
{
	...
	if (targetPlayer !== undefined)
		KONOHA.chatAnswerRequestAttack(gameState, targetPlayer, answer, other);
 ...
 */
KONOHA.AttackManager.prototype.assignBombers = function(gameState)
{
	...
	for (let ent of bombers.values())
	{
		...
		{
			if (!ent.canAttackTarget(struct, KONOHA.allowCapture(gameState, ent, struct)))
				continue;
 ...
 */
KONOHA.AttackManager.prototype.update = function(gameState, queues, events)
{
	...
	for (let attackType in this.upcomingAttacks)
	{
		for (let i = 0; i < this.upcomingAttacks[attackType].length; ++i)
		{
			...
			if (attack.isStarted())
				API3.warn("Konoha problem in attackManager: attack in preparation has already started ???");
			...
			else if (updateStep == 2)
			{
				if (attack.StartAttack(gameState))
				{
					...
					if (this.Config.chat)
						KONOHA.chatLaunchAttack(gameState, attack.targetPlayer, attack.getType());
	...
	if (this.rushNumber < this.maxRushes && barracksNb >= 1)
	{
		if (unexecutedAttacks.Rush === 0)
		{
			...
			let attackPlan = new KONOHA.AttackPlan(gameState, this.Config, this.totalNumber, "Rush", data);
		...
		(this.startedAttacks.Attack.length + this.startedAttacks.HugeAttack.length == 0 || gameState.getPopulationMax() - gameState.getPopulation() > 12))
	{
			...
			!gameState.ai.HQ.baseManagers[1])	// if we have no base ... nothing else to do than attack
		{
			...
			let attackPlan = new KONOHA.AttackPlan(gameState, this.Config, this.totalNumber, type);
			if (attackPlan.failed)
...
KONOHA.AttackManager.prototype.getPlan = function(planName)
{
	for (let attackType in this.upcomingAttacks)
	{
		for (let attack of this.upcomingAttacks[attackType])
...
KONOHA.AttackManager.prototype.pausePlan = function(planName)
{
...
KONOHA.AttackManager.prototype.unpausePlan = function(planName)
{
	let attack = this.getPlan(planName);
	if (attack)
...
KONOHA.AttackManager.prototype.pauseAllPlans = function()
{
	for (let attackType in this.upcomingAttacks)
		for (let attack of this.upcomingAttacks[attackType])
			attack.setPaused(true);
...
KONOHA.AttackManager.prototype.unpauseAllPlans = function()
{
	for (let attackType in this.upcomingAttacks)
		for (let attack of this.upcomingAttacks[attackType])
			attack.setPaused(false);
...
KONOHA.AttackManager.prototype.getAttackInPreparation = function(type)
{
 ...
 * for future attacks.
 */
KONOHA.AttackManager.prototype.getEnemyPlayer = function(gameState, attack)
{
	let enemyPlayer;
	...
	if (attack.type != "HugeAttack")
	{
		...
		for (let ourcc of ccEnts.values())
		{
			...
			let access = KONOHA.getLandAccess(gameState, ourcc);
			for (let enemycc of ccEnts.values())
			{
				...
				if (!gameState.isPlayerEnemy(enemycc.owner()))
					continue;
				if (access != KONOHA.getLandAccess(gameState, enemycc))
					continue;
 ...
 */
KONOHA.AttackManager.prototype.getWonderEnemyPlayer = function(gameState, attack)
{
	let enemyPlayer;
	let enemyWonder;
	let moreAdvanced;
 ...
 */
KONOHA.AttackManager.prototype.getRelicEnemyPlayer = function(gameState, attack)
{
	let enemyPlayer;
...
KONOHA.AttackManager.prototype.cancelAttacksAgainstPlayer = function(gameState, player)
{
	for (let attackType in this.upcomingAttacks)
		for (let attack of this.upcomingAttacks[attackType])
...
KONOHA.AttackManager.prototype.raidTargetEntity = function(gameState, ent)
{
	let data = { "target": ent };
	let attackPlan = new KONOHA.AttackPlan(gameState, this.Config, this.totalNumber, "Raid", data);
	if (attackPlan.failed)
		return null;
	if (this.Config.debug > 1)
 ...
 */
KONOHA.AttackManager.prototype.numAttackingUnitsAround = function(pos, dist)
{
	let num = 0;
	for (let attackType in this.startedAttacks)
		for (let attack of this.startedAttacks[attackType])
 ...
 */
KONOHA.AttackManager.prototype.switchDefenseToAttack = function(gameState, target, data)
{
	...
	let attackPlan = new KONOHA.AttackPlan(gameState, this.Config, this.totalNumber, attackType, attackData);
	if (attackPlan.failed)
		return false;
	this.totalNumber++;
	attackPlan.init(gameState);
	...
	let targetAccess = KONOHA.getLandAccess(gameState, target);
	for (let army of gameState.ai.HQ.defenseManager.armies)
	{
		...
		while (army.ownEntities.length > 0)
		{
			...
			let accessOk = unit.getMetadata(PlayerID, "transport") !== undefined ||
			               unit.position() && KONOHA.getLandAccess(gameState, unit) == targetAccess;
...
KONOHA.AttackManager.prototype.Serialize = function()
{
	let properties = {
...
KONOHA.AttackManager.prototype.Deserialize = function(gameState, data)
{
	...
	for (let key in data.upcomingAttacks)
	{
		...
		{
			let attack = new KONOHA.AttackPlan(gameState, this.Config, dataAttack.properties.name);
