 ...
 */
KONOHA.DefenseArmy = function(gameState, foeEntities, type)
{
 ...
 */
KONOHA.DefenseArmy.prototype.addFoe = function(gameState, enemyId, force)
{
 ...
 */
KONOHA.DefenseArmy.prototype.removeFoe = function(gameState, enemyId, enemyEntity)
{
 ...
 */
KONOHA.DefenseArmy.prototype.addOwn = function(gameState, id, force)
{
...
KONOHA.DefenseArmy.prototype.removeOwn = function(gameState, id, Entity)
{
 ...
 */
KONOHA.DefenseArmy.prototype.clear = function(gameState)
{
	...
	else
	{
		...
		for (let struct of gameState.getAllyStructures().values())
		{
			...
			if (!pos || !gameState.isPlayerMutualAlly(gameState.ai.HQ.territoryMap.getOwner(pos)))
				continue;
			if (KONOHA.getLandAccess(gameState, struct) !== armyAccess)
				continue;
...
KONOHA.DefenseArmy.prototype.assignUnit = function(gameState, entID)
{
	...
	for (let id of this.foeEntities)
	{
		...
		if (!ent.canAttackTarget(eEnt, KONOHA.allowCapture(gameState, ent, eEnt)))
			continue;
		...
		if (this.assignedAgainst[id].length > 8 ||
			this.assignedAgainst[id].length > 5 && !eEnt.hasClass("Hero") && !KONOHA.isSiegeUnit(eEnt))
			continue;
	...
	let ownIndex = KONOHA.getLandAccess(gameState, ent);
	...
	if (ownIndex == foeIndex || ent.hasClass("Ship"))
	{
		...
		ent.attack(idFoe, KONOHA.allowCapture(gameState, ent, foeEnt), queued);
	}
	else
...
KONOHA.DefenseArmy.prototype.getType = function()
{
	return this.type;
};

KONOHA.DefenseArmy.prototype.getState = function()
{
 ...
 */
KONOHA.DefenseArmy.prototype.merge = function(gameState, otherArmy)
{
...
KONOHA.DefenseArmy.prototype.needsDefenders = function(gameState)
{
	let defenseRatio;
...
KONOHA.DefenseArmy.prototype.recalculatePosition = function(gameState, force)
{
...
KONOHA.DefenseArmy.prototype.recalculateStrengths = function(gameState)
{
	this.ownStrength = 0;
	this.foeStrength = 0;
...
KONOHA.DefenseArmy.prototype.evaluateStrength = function(ent, isOwn, remove)
{
	if (!ent)
		return;
	...
	if (ent.hasClass("Structure"))
	{
		...
		entStrength = KONOHA.getMaxStrength(ent, this.Config.debug, this.Config.DamageTypeImportance);
...
KONOHA.DefenseArmy.prototype.checkEvents = function(gameState, events)
{
...
KONOHA.DefenseArmy.prototype.update = function(gameState)
{
	for (let entId of this.ownEntities)
	{
		let ent = gameState.getEntityById(entId);
		...
		else if (orderData.length && orderData[0].target && orderData[0].attackType && orderData[0].attackType === "Capture")
		{
			...
			if (target && !KONOHA.allowCapture(gameState, ent, target))
...
KONOHA.DefenseArmy.prototype.Serialize = function()
{
	return {
		"ID": this.ID,
		"type": this.type,
...
KONOHA.DefenseArmy.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
