 ...
 */
KONOHA.NavalManager = function(Config)
{
	this.Config = Config;
...
KONOHA.NavalManager.prototype.init = function(gameState, deserializing)
{
	// docks
	...
	for (let ship of this.ships.values())
		KONOHA.setSeaAccess(gameState, ship);
	for (let dock of this.docks.values())
		KONOHA.setSeaAccess(gameState, dock);
};

KONOHA.NavalManager.prototype.updateFishingBoats = function(sea, num)
{
	if (this.wantedFishShips[sea])
		this.wantedFishShips[sea] = num;
};

KONOHA.NavalManager.prototype.resetFishingBoats = function(gameState, sea)
{
	if (sea !== undefined)
...
KONOHA.NavalManager.prototype.getFishSea = function(gameState, fish)
{
...
KONOHA.NavalManager.prototype.canFishSafely = function(gameState, fish)
{
...
KONOHA.NavalManager.prototype.getUnconnectedSeas = function(gameState, region)
{
	let seas = gameState.ai.accessibility.regionLinks[region].slice();
	this.docks.forEach(dock => {
		if (!dock.hasClass("Dock") || KONOHA.getLandAccess(gameState, dock) != region)
			return;
		let i = seas.indexOf(KONOHA.getSeaAccess(gameState, dock));
		if (i != -1)
			seas.splice(i--, 1);
	});
	return seas;
...
KONOHA.NavalManager.prototype.checkEvents = function(gameState, queues, events)
{
...
KONOHA.NavalManager.prototype.getPlan = function(ID)
{
...
KONOHA.NavalManager.prototype.addPlan = function(plan)
{
 ...
 */
KONOHA.NavalManager.prototype.requireTransport = function(gameState, ent, startIndex, endIndex, endPos)
{
	if (!ent.canGarrison())
		return false;
	...
	if (ent.getMetadata(PlayerID, "transport") !== undefined)
	{
		if (this.Config.debug > 0)
			API3.warn("Konoha naval manager error: unit " + ent.id() + " has already required a transport");
		return false;
	}
	...
	for (let plan of this.transportPlans)
	{
		...
		if (KONOHA.isSiegeUnit(ent) && plan.units.filter(unit => KONOHA.isSiegeUnit(unit)).length > 3)
			continue;
		plans.push(plan);
	}
	...
	let plan = new KONOHA.TransportPlan(gameState, [ent], startIndex, endIndex, endPos);
	if (plan.failed)
	{
		if (this.Config.debug > 1)
...
KONOHA.NavalManager.prototype.splitTransport = function(gameState, plan)
{
	if (this.Config.debug > 1)
		API3.warn(">>>> split of transport plan started <<<<");
	let newplan = new KONOHA.TransportPlan(gameState, [], plan.startIndex, plan.endIndex, plan.endPos);
	if (newplan.failed)
	{
		if (this.Config.debug > 1)
 ...
 */
KONOHA.NavalManager.prototype.createTransportIfNeeded = function(gameState, fromPos, toPos, toAccess)
{
	...
	for (let ship of this.ships.values())
	{
		...
		let plan = new KONOHA.TransportPlan(gameState, units, fromAccess, toAccess, toPos, ship);
		if (plan.failed)
			continue;
		plan.init(gameState);
		this.transportPlans.push(plan);
...
KONOHA.NavalManager.prototype.setMinimalTransportShips = function(gameState, sea, number)
{
	if (!sea)
		return;
...
KONOHA.NavalManager.prototype.checkLevels = function(gameState, queues)
{
...
KONOHA.NavalManager.prototype.maintainFleet = function(gameState, queues)
{
	if (queues.ships.hasQueuedUnits())
		return;
	...
	for (let sea = 0; sea < this.seaShips.length; ++sea)
	{
		...
		if (this.seaTransportShips[sea].length < this.wantedTransportShips[sea])
		{
			...
			if (template)
			{
				queues.ships.addPlan(new KONOHA.TrainingPlan(gameState, template, { "sea": sea }, 1, 1));
				continue;
			}
		}
		...
		if (this.seaFishShips[sea].length < this.wantedFishShips[sea])
		{
			...
			if (template)
			{
				queues.ships.addPlan(new KONOHA.TrainingPlan(gameState, template, { "base": 0, "role": "worker", "sea": sea }, 1, 1));
				continue;
			}
		}
	}
...
KONOHA.NavalManager.prototype.assignShipsToPlans = function(gameState)
{
	for (let plan of this.transportPlans)
...
KONOHA.NavalManager.prototype.isShipBoarding = function(ship)
{
	if (!ship.position())
		return false;
 ...
 */
KONOHA.NavalManager.prototype.moveApart = function(gameState)
{
	let blockedShips = [];
	let blockedIds = [];
	...
	for (let ship of this.ships.values())
	{
		...
		else if (ship.isIdle())
		{
			...
			if (KONOHA.gatherTreasure(gameState, ship, true))
				continue;
			...
			{
				if (KONOHA.getSeaAccess(gameState, dock) != sea)
					continue;
...
KONOHA.NavalManager.prototype.buildNavalStructures = function(gameState, queues)
{
			...
			gameState.ai.HQ.canBuild(gameState, "structures/{civ}/dock"))
		{
			...
			for (let base of gameState.ai.HQ.baseManagers)
			{
				...
				for (let sea of remaining)
				{
					...
					queues.dock.addPlan(new KONOHA.ConstructionPlan(gameState, "structures/{civ}/dock", { "land": wantedLand, "sea": sea }));
					dockStarted = true;
					break;
				}
			}
	...
	queues.militaryBuilding.addPlan(new KONOHA.ConstructionPlan(gameState, template, { "land": wantedLand, "sea": sea }));
};

/** goal can be either attack (choose ship with best arrowCount) or transport (choose ship with best capacity) */
KONOHA.NavalManager.prototype.getBestShip = function(gameState, sea, goal)
{
...
KONOHA.NavalManager.prototype.update = function(gameState, queues, events)
{
...
KONOHA.NavalManager.prototype.Serialize = function()
{
	let properties = {
...
KONOHA.NavalManager.prototype.Deserialize = function(gameState, data)
{
	...
	for (let i in data.transports)
	{
		...
		let plan = new KONOHA.TransportPlan(gameState, [], dataPlan.startIndex, dataPlan.endIndex, dataPlan.endPos);
