...
KONOHA.GarrisonManager = function(Config)
{
	this.Config = Config;
...
KONOHA.GarrisonManager.prototype.update = function(gameState, events)
{
	...
	for (let [id, data] of this.holders.entries())
	{
		...
		for (let j = 0; j < list.length; ++j)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						API3.warn("Konoha garrison error: unit " + ent.id() + " (" + ent.genericName() +
		...
		if (gameState.ai.elapsedTime - holder.getMetadata(PlayerID, "holderTimeUpdate") > 3)
		{
			...
			for (let ent of gameState.getEnemyEntities().values())
			{
				...
				else if (KONOHA.isSiegeUnit(ent))
				{
...
KONOHA.GarrisonManager.prototype.numberOfGarrisonedUnits = function(holder)
{
...
/** TODO should add the units garrisoned inside garrisoned units */
KONOHA.GarrisonManager.prototype.numberOfGarrisonedSlots = function(holder)
{
	if (!this.holders.has(holder.id()))
...
KONOHA.GarrisonManager.prototype.allowMelee = function(holder)
{
	if (!this.holders.has(holder.id()))
		return undefined;
...
KONOHA.GarrisonManager.prototype.garrison = function(gameState, ent, holder, type)
{
 ...
 */
KONOHA.GarrisonManager.prototype.leaveGarrison = function(ent)
{
...
KONOHA.GarrisonManager.prototype.cancelGarrison = function(ent)
{
	ent.stopMoving();
	this.leaveGarrison(ent);
...
KONOHA.GarrisonManager.prototype.keepGarrisoned = function(ent, holder, around)
{
	...
	case 'protection':	// hurt unit for healing or infantry for defense
		...
		if (ent.attackTypes() && ent.attackTypes().indexOf("Melee") !== -1)
			return false;
		if (around.unit)
			return ent.hasClass("Support") || KONOHA.isSiegeUnit(ent);	// only ranged siege here and below as melee siege already released above
...
KONOHA.GarrisonManager.prototype.registerHolder = function(gameState, holder)
{
 ...
 */
KONOHA.GarrisonManager.prototype.addDecayingStructure = function(gameState, entId, justCaptured)
{
...
KONOHA.GarrisonManager.prototype.removeDecayingStructure = function(entId)
{
	if (!this.decayingStructures.has(entId))
		return;
...
KONOHA.GarrisonManager.prototype.Serialize = function()
{
	return { "holders": this.holders, "decayingStructures": this.decayingStructures };
};

KONOHA.GarrisonManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
