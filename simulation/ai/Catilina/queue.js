...
CATILINA.Queue.prototype.empty = function()
{
	this.plans = [];
};

CATILINA.Queue.prototype.addPlan = function(newPlan)
{
	if (!newPlan)
		return;
...
CATILINA.Queue.prototype.check= function(gameState)
{
...
CATILINA.Queue.prototype.getNext = function()
{
...
CATILINA.Queue.prototype.startNext = function(gameState)
{
	if (this.plans.length > 0)
	{
 ...
 */
CATILINA.Queue.prototype.maxAccountWanted = function(gameState, fraction)
{
...
CATILINA.Queue.prototype.queueCost = function()
{
	let cost = new API3.Resources();
	for (let plan of this.plans)
...
CATILINA.Queue.prototype.length = function()
{
	return this.plans.length;
};

CATILINA.Queue.prototype.hasQueuedUnits = function()
{
	return this.plans.length > 0;
};

CATILINA.Queue.prototype.countQueuedUnits = function()
{
	let count = 0;
	for (let plan of this.plans)
		count += plan.number;
...
CATILINA.Queue.prototype.hasQueuedUnitsWithClass = function(classe)
{
	return this.plans.some(plan => plan.template && plan.template.hasClass(classe));
};

CATILINA.Queue.prototype.countQueuedUnitsWithClass = function(classe)
{
	let count = 0;
	for (let plan of this.plans)
...
CATILINA.Queue.prototype.countQueuedUnitsWithMetadata = function(data, value)
{
	let count = 0;
	for (let plan of this.plans)
...
CATILINA.Queue.prototype.Serialize = function()
{
	let plans = [];
	for (let plan of this.plans)
...
CATILINA.Queue.prototype.Deserialize = function(gameState, data)
{
	...
	for (let dataPlan of data.plans)
	{
		...
		if (dataPlan.category == "unit")
			plan = new CATILINA.TrainingPlan(gameState, dataPlan.type);
		else if (dataPlan.category == "building")
			plan = new CATILINA.ConstructionPlan(gameState, dataPlan.type);
		else if (dataPlan.category == "technology")
			plan = new CATILINA.ResearchPlan(gameState, dataPlan.type);
		else
		{
			API3.warn("Catilina deserialization error: plan unknown " + uneval(dataPlan));
			continue;
		}
