...
KONOHA.isFastMoving = function(ent)
{
...
KONOHA.getMaxStrength = function(ent, debugLevel, DamageTypeImportance, againstClass)
{
	let strength = 0;
	...
	for (let type of attackTypes)
	{
		...
		for (let str in attackStrength)
		{
			...
			if (DamageTypeImportance[str])
				...
				API3.warn("Konoha: " + str + " unknown attackStrength in getMaxStrength (please add " + str + "  to config.js).");
		}
		...
		for (let str in attackTimes)
		{
			...
			case "prepare":
				...
				break;
			default:
				API3.warn("Konoha: " + str + " unknown attackTimes in getMaxStrength");
			}
		}
	}
	...
	if (resistanceStrength.Damage)
		for (let str in resistanceStrength.Damage)
		{
			...
			if (DamageTypeImportance[str])
				...
				API3.warn("Konoha: " + str + " unknown resistanceStrength in getMaxStrength (please add " + str + "  to config.js).");
		}
...
KONOHA.getLandAccess = function(gameState, ent)
{
	if (ent.hasClass("Unit"))
	{
		...
		if (!pos)
		{
			let holder = KONOHA.getHolder(gameState, ent);
			if (holder)
				return KONOHA.getLandAccess(gameState, holder);

			API3.warn("Konoha error: entity without position, but not garrisoned");
...
KONOHA.getSeaAccess = function(gameState, ent)
{
...
KONOHA.setSeaAccess = function(gameState, ent)
{
	KONOHA.getSeaAccess(gameState, ent);
};

/** Decide if we should try to capture (returns true) or destroy (return false) */
KONOHA.allowCapture = function(gameState, ent, target)
{
	...
	{
		capture = ent.captureStrength() * KONOHA.getAttackBonus(ent, target, "Capture");
	...
	else
	{
		...
		{
			capturable.strength += ent.captureStrength() * KONOHA.getAttackBonus(ent, target, "Capture");
...
KONOHA.getAttackBonus = function(ent, target, type)
{
	let attackBonus = 1;
...
KONOHA.returnResources = function(gameState, ent)
{
	...
	for (let dropsite of dropsiteCollection.values())
	{
		...
		if (owner !== PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
			continue;
		if (KONOHA.getLandAccess(gameState, dropsite) != access)
			continue;
...
KONOHA.IsSupplyFull = function(gameState, ent)
{
 ...
 */
KONOHA.getBestBase = function(gameState, ent, onlyConstructedBase = false, exclude = false)
{
	let pos = ent.position();
	let accessIndex;
	if (!pos)
	{
		...
		pos = holder.position();
		accessIndex = KONOHA.getLandAccess(gameState, holder);
	}
	else
		accessIndex = KONOHA.getLandAccess(gameState, ent);
...
KONOHA.getHolder = function(gameState, ent)
{
...
KONOHA.getBuiltEntity = function(gameState, ent)
{
 ...
 */
KONOHA.isNotWorthBuilding = function(gameState, ent)
{
 ...
 */
KONOHA.isLineInsideEnemyTerritory = function(gameState, pos1, pos2, step=70)
{
...
KONOHA.gatherTreasure = function(gameState, ent, water = false)
{
	...
	let treasureFound;
	let distmin = Math.min();
	let access = water ? KONOHA.getSeaAccess(gameState, ent) : KONOHA.getLandAccess(gameState, ent);
	for (let treasure of gameState.ai.HQ.treasures.values())
	{
		...
		if (lastGathered && gameState.ai.elapsedTime - lastGathered < 20)
			continue;
		if (!water && access != KONOHA.getLandAccess(gameState, treasure))
			continue;
		if (water && access != KONOHA.getSeaAccess(gameState, treasure))
			continue;
...
KONOHA.dumpEntity = function(ent)
{
	if (!ent)
		return;
