...
KONOHA.Worker.prototype.update = function(gameState, ent)
{
	...
	if (ent.getMetadata(PlayerID, "transport") !== undefined)
	{
		...
		if (subrole == "gatherer" || subrole == "hunter")
		{
			...
			if (plan.state == "boarding" && ent.position())
			{
				...
				{
					if (KONOHA.getLandAccess(gameState, structure) != plan.endIndex)
						continue;
					let resourceDropsiteTypes = KONOHA.getBuiltEntity(gameState, structure).resourceDropsiteTypes();
				...
				if (!hasDropsite)
				{
					...
					{
						if (!unit.position() || KONOHA.getLandAccess(gameState, unit) != plan.endIndex)
							continue;
	...
	this.entAccess = KONOHA.getLandAccess(gameState, ent);
		...
		if (unitAIState == "INDIVIDUAL.COMBAT.APPROACHING" && ent.unitAIOrderData().length)
		{
			...
			if (orderData && orderData.target)
			{
				...
				if (target && target.resourceSupplyType() && target.resourceSupplyType().generic == "food")
				{
					...
					{
						let distanceSquare = KONOHA.isFastMoving(ent) ? 90000 : 3000; //4550 calculated by Nagasushi
						...
						for (let dropsite of foodDropsites.values())
						{
							...
							if (owner != PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
								continue;
							if (targetAccess != KONOHA.getLandAccess(gameState, dropsite))
								continue;
	...
	if (unitAIStateOrder == "COMBAT")
	{
			...
			!ent.getMetadata(PlayerID, "PartOfArmy"))
		{
			...
			{
				let target = gameState.getEntityById(orderData.target);
				if (target && (!target.position() || KONOHA.getLandAccess(gameState, target) != this.entAccess))
				{
					if (this.retryWorking(gameState, subrole))
						return;
					ent.stopMoving();
			...
			!ent.getMetadata(PlayerID, "PartOfArmy"))
		{
			...
			if (orderData && orderData.target && orderData.attackType && orderData.attackType == "Capture")
			{
				...
				if (target && target.owner() > 0 && !gameState.isPlayerAlly(target.owner()))
					ent.attack(orderData.target, KONOHA.allowCapture(gameState, ent, target));
			}
		}
		return;
	}
	...
	if (subrole == "gatherer")
	{
		if (ent.isIdle())
		{
			...
			else if (!KONOHA.returnResources(gameState, ent))     // try to deposit resources
			{
		...
		else if (unitAIStateOrder == "GATHER")
		{
			...
			if (unitAIState == "INDIVIDUAL.GATHER.RETURNINGRESOURCE.APPROACHING")
			{
				if (gameState.ai.playedTurn % 10 == 0)
				{
					...
					if (dropsite && dropsite.position() && this.entAccess != KONOHA.getLandAccess(gameState, dropsite))
						KONOHA.returnResources(gameState, this.ent);
				}
	...
	else if (subrole == "builder")
	{
		...
		{
			let goalAccess = KONOHA.getLandAccess(gameState, target);
			let queued = KONOHA.returnResources(gameState, ent);
		...
		else	// Perform some sanity checks
		{
			if (unitAIStateOrder == "GATHER")
			{
				...
				else if (unitAIState == "INDIVIDUAL.GATHER.RETURNINGRESOURCE.APPROACHING")
				{
					...
					let dropsite = gameState.getEntityById(ent.unitAIOrderData()[0].target);
					if (dropsite && dropsite.position() && this.entAccess != KONOHA.getLandAccess(gameState, dropsite))
						KONOHA.returnResources(gameState, ent);
				}
			}
		}
	}
...
KONOHA.Worker.prototype.retryWorking = function(gameState, subrole)
{
	switch (subrole)
	{
	case "gatherer":
...
KONOHA.Worker.prototype.startBuilding = function(gameState)
{
	let target = gameState.getEntityById(this.ent.getMetadata(PlayerID, "target-foundation"));
	if (!target || target.foundationProgress() === undefined && target.needsRepair() == false)
		return false;
	if (KONOHA.getLandAccess(gameState, target) != this.entAccess)
		return false;
	this.ent.repair(target, target.hasClass("House"));  // autocontinue=true for houses
	return true;
};

KONOHA.Worker.prototype.startGathering = function(gameState)
{
	// First look for possible treasure if any
	if (KONOHA.gatherTreasure(gameState, this.ent))
		return true;
	...
	let findSupply = function(ent, supplies) {
		...
		for (let i = 0; i < supplies.length; ++i)
		{
			...
			if (KONOHA.IsSupplyFull(gameState, supplies[i].ent))
				continue;
	...
	let shouldBuild = this.ent.isBuilder() && foundations.some(function(foundation) {
		if (!foundation || KONOHA.getLandAccess(gameState, foundation) != this.entAccess)
			return false;
	...
	shouldBuild = this.ent.isBuilder() && foundations.some(function(foundation) {
		...
		if (structure.resourceDropsiteTypes() && structure.resourceDropsiteTypes().indexOf(resource) != -1)
		{
			let foundationAccess = KONOHA.getLandAccess(gameState, foundation);
 ...
 */
KONOHA.Worker.prototype.startHunting = function(gameState, position)
{
	// First look for possible treasure if any
	if (!position && KONOHA.gatherTreasure(gameState, this.ent))
		return true;
	...
	let isFastMoving = KONOHA.isFastMoving(this.ent);
	...
	let hasFoodDropsiteWithinDistance = function(supplyPosition, supplyAccess, distSquare)
	{
		for (let dropsite of foodDropsites.values())
		{
			...
			if (owner != PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
				continue;
			if (supplyAccess != KONOHA.getLandAccess(gameState, dropsite))
				continue;
	...
	for (let supply of resources.values())
	{
		...
		if (KONOHA.IsSupplyFull(gameState, supply))
			continue;
		// Check if available resource is worth one additionnal gatherer (except for farms).
		...
		let supplyAccess = KONOHA.getLandAccess(gameState, supply);
		...
		if (!isFastMoving && canFlee && territoryOwner == 0)
			continue;
		let distanceSquare = isFastMoving ? 35000 : (canFlee ? 4500 : 4500); //Nagasushi
...
KONOHA.Worker.prototype.startFishing = function(gameState)
{
	...
	let fisherSea = KONOHA.getSeaAccess(gameState, this.ent);
	...
	let nearestDropsiteDist = function(supply) {
		...
		for (let dropsite of fishDropsites)
		{
			...
			if (owner != PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
				continue;
			if (fisherSea != KONOHA.getSeaAccess(gameState, dropsite))
				continue;
...
KONOHA.Worker.prototype.gatherNearestField = function(gameState, baseID)
{
	...
	{
		if (KONOHA.IsSupplyFull(gameState, field))
			continue;
 ...
 */
KONOHA.Worker.prototype.buildAnyField = function(gameState, baseID)
{
 ...
 */
KONOHA.Worker.prototype.moveToGatherer = function(gameState, ent, forced)
{
	...
	for (let gatherer of gatherers.values())
	{
		...
		if (!gatherer.position() || gatherer.getMetadata(PlayerID, "transport") !== undefined ||
		    KONOHA.getLandAccess(gameState, gatherer) != access || gatherer.isIdle())
			continue;
 ...
 */
KONOHA.Worker.prototype.isInaccessibleSupply = function(gameState)
{
