...
CATILINA.GarrisonManager = function(Config)
{
	this.Config = Config;
...
CATILINA.GarrisonManager.prototype.update = function(gameState, events)
{
	...
	for (let [id, data] of this.holders.entries())
	{
		...
		for (let j = 0; j < list.length; ++j)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						API3.warn("Catilina garrison error: unit " + ent.id() + " (" + ent.genericName() +
		...
		if (gameState.ai.elapsedTime - holder.getMetadata(PlayerID, "holderTimeUpdate") > 3)
		{
			...
			for (let ent of gameState.getEnemyEntities().values())
			{
				...
				else if (CATILINA.isSiegeUnit(ent))
				{
...
CATILINA.GarrisonManager.prototype.numberOfGarrisonedUnits = function(holder)
{
...
/** TODO should add the units garrisoned inside garrisoned units */
CATILINA.GarrisonManager.prototype.numberOfGarrisonedSlots = function(holder)
{
	if (!this.holders.has(holder.id()))
...
CATILINA.GarrisonManager.prototype.allowMelee = function(holder)
{
	if (!this.holders.has(holder.id()))
		return undefined;
...
CATILINA.GarrisonManager.prototype.garrison = function(gameState, ent, holder, type)
{
 ...
 */
CATILINA.GarrisonManager.prototype.leaveGarrison = function(ent)
{
...
CATILINA.GarrisonManager.prototype.cancelGarrison = function(ent)
{
	ent.stopMoving();
	this.leaveGarrison(ent);
...
CATILINA.GarrisonManager.prototype.keepGarrisoned = function(ent, holder, around)
{
	...
	case 'protection':	// hurt unit for healing or infantry for defense
		...
		if (ent.attackTypes() && ent.attackTypes().indexOf("Melee") !== -1)
			return false;
		if (around.unit)
			return ent.hasClass("Support") || CATILINA.isSiegeUnit(ent);	// only ranged siege here and below as melee siege already released above
...
CATILINA.GarrisonManager.prototype.registerHolder = function(gameState, holder)
{
 ...
 */
CATILINA.GarrisonManager.prototype.addDecayingStructure = function(gameState, entId, justCaptured)
{
	if (this.decayingStructures.has(entId))
		return true;
	let ent = gameState.getEntityById(entId);
	if (!ent || !(ent.hasClass("Temple") && !ent.hasDefensiveFire())) //Nagasushi
		return false;
...
CATILINA.GarrisonManager.prototype.removeDecayingStructure = function(entId)
{
	if (!this.decayingStructures.has(entId))
		return;
...
CATILINA.GarrisonManager.prototype.Serialize = function()
{
	return { "holders": this.holders, "decayingStructures": this.decayingStructures };
};

CATILINA.GarrisonManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
