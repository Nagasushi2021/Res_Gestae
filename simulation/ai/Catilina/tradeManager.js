 ...
 * Manage the trade
 */
CATILINA.TradeManager = function(Config)
{
	this.Config = Config;
...
CATILINA.TradeManager.prototype.init = function(gameState)
{
...
CATILINA.TradeManager.prototype.hasTradeRoute = function()
{
	return this.tradeRoute !== undefined;
};

CATILINA.TradeManager.prototype.assignTrader = function(ent)
{
	ent.setMetadata(PlayerID, "role", "trader");
	this.traders.updateEnt(ent);
};

CATILINA.TradeManager.prototype.trainMoreTraders = function(gameState, queues)
{
	...
	if (!gameState.getTemplate(template))
	{
		if (this.Config.debug > 0)
			API3.warn("Catilina error: trying to train " + template + " for civ " +
			          gameState.getPlayerCiv() + " but no template found.");
		return;
	}
	queues.trader.addPlan(new CATILINA.TrainingPlan(gameState, template, metadata, 1, 1));
};

CATILINA.TradeManager.prototype.updateTrader = function(gameState, ent)
{
	if (ent.hasClass("Ship") && gameState.ai.playedTurn % 5 == 0 &&
	    !ent.unitAIState().startsWith("INDIVIDUAL.COLLECTTREASURE") &&
	    CATILINA.gatherTreasure(gameState, ent, true))
		return;
	...
	let access = ent.hasClass("Ship") ? CATILINA.getSeaAccess(gameState, ent) : CATILINA.getLandAccess(gameState, ent);
...
CATILINA.TradeManager.prototype.setTradingGoods = function(gameState)
{
 ...
 */
CATILINA.TradeManager.prototype.performBarter = function(gameState)
{
...
CATILINA.TradeManager.prototype.checkEvents = function(gameState, events)
{
...
CATILINA.TradeManager.prototype.activateProspection = function(gameState)
{
 ...
 */
CATILINA.TradeManager.prototype.checkRoutes = function(gameState, accessIndex)
{
	...
	for (let m1 of market1.values())
	{
		if (!m1.position())
			continue;
		let access1 = CATILINA.getLandAccess(gameState, m1);
		let sea1 = m1.hasClass("Naval") ? CATILINA.getSeaAccess(gameState, m1) : undefined;
		for (let m2 of market2.values())
		{
			...
			if (!m2.position())
				continue;
			let access2 = CATILINA.getLandAccess(gameState, m2);
			let sea2 = m2.hasClass("Naval") ? CATILINA.getSeaAccess(gameState, m2) : undefined;
			let land = access1 == access2 ? access1 : undefined;
			let sea = sea1 && sea1 == sea2 ? sea1 : undefined;
			if (!land && !sea)
				continue;
			if (land && CATILINA.isLineInsideEnemyTerritory(gameState, m1.position(), m2.position()))
				continue;
			let gainMultiplier;
	...
	if (this.Config.chat)
	{
		...
		{	// Warn an ally that we have a trade route with him
			CATILINA.chatNewTradeRoute(gameState, owner);
...
CATILINA.TradeManager.prototype.checkTrader = function(gameState, ent)
{
	    ...
	    possibleRoute.target.id() != presentRoute.source && possibleRoute.target.id() != presentRoute.target)
	{
		...
		{
			let closestBase = CATILINA.getBestBase(gameState, ent, true);
...
CATILINA.TradeManager.prototype.prospectForNewMarket = function(gameState, queues)
{
...
CATILINA.TradeManager.prototype.isNewMarketWorth = function(expectedGain)
{
	if (!Resources.GetTradableCodes().length)
		return false;
...
CATILINA.TradeManager.prototype.update = function(gameState, events, queues)
{
...
CATILINA.TradeManager.prototype.routeEntToId = function(route)
{
	if (!route)
		return undefined;
...
CATILINA.TradeManager.prototype.routeIdToEnt = function(gameState, route)
{
	if (!route)
		return undefined;
...
CATILINA.TradeManager.prototype.Serialize = function()
{
	return {
...
CATILINA.TradeManager.prototype.Deserialize = function(gameState, data)
{
	for (let key in data)
	{
