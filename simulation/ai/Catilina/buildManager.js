...
CATILINA.BuildManager = function()
{
...
CATILINA.BuildManager.prototype.init = function(gameState)
{
...
CATILINA.BuildManager.prototype.incrementBuilderCounters = function(civ, ent, increment)
{
	for (let buildable of ent.buildableEntities(civ))
	{
		if (this.builderCounters.has(buildable))
		{
			...
			if (count < 0)
			{
				API3.warn(" Catilina error in incrementBuilderCounters for " + buildable + " with count < 0");
				continue;
			}
			...
			API3.warn(" Catilina error in incrementBuilderCounters for " + buildable + " not yet set");
	}
};

/** Update the builders counters */
CATILINA.BuildManager.prototype.checkEvents = function(gameState, events)
{
 ...
 */
CATILINA.BuildManager.prototype.findStructuresByFilter = function(gameState, filter)
{
	const result = [];
 ...
 */
CATILINA.BuildManager.prototype.findStructureWithClass = function(gameState, classes)
{
	return this.findStructuresByFilter(gameState, API3.Filters.byClasses(classes))[0];
};

CATILINA.BuildManager.prototype.hasBuilder = function(template)
{
	let numBuilders = this.builderCounters.get(template);
	return numBuilders && numBuilders > 0;
};

CATILINA.BuildManager.prototype.isUnbuildable = function(gameState, template)
{
	return this.unbuildables.has(template) && this.unbuildables.get(template).time > gameState.ai.elapsedTime;
};

CATILINA.BuildManager.prototype.setBuildable = function(template)
{
...
CATILINA.BuildManager.prototype.setUnbuildable = function(gameState, template, time = 90, reason = "room")
{
	if (!this.unbuildables.has(template))
...
CATILINA.BuildManager.prototype.numberMissingRoom = function(gameState)
{
	let num = 0;
...
CATILINA.BuildManager.prototype.resetMissingRoom = function(gameState)
{
...
CATILINA.BuildManager.prototype.Serialize = function()
{
	return {
...
CATILINA.BuildManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
