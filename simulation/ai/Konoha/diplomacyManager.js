 ...
 */
KONOHA.DiplomacyManager = function(Config)
{
	this.Config = Config;
 ...
 */
KONOHA.DiplomacyManager.prototype.init = function(gameState)
{
 ...
 */
KONOHA.DiplomacyManager.prototype.tributes = function(gameState)
{
	...
	for (let i = 1; i < gameState.sharedScript.playersData.length; ++i)
	{
			...
			else if (this.Config.chat && availableResources[res] === 0 && allyResources[res] > totalResources[res] + 600)
			{
				...
				for (let k = 0; k < mostNeeded.length; ++k)
				{
					if (mostNeeded[k].type == res && mostNeeded[k].wanted > 0)
					{
						...
						KONOHA.chatRequestTribute(gameState, res);
		...
		if (this.Config.chat)
			KONOHA.chatSentTribute(gameState, i);
		Engine.PostCommand(PlayerID, { "type": "tribute", "player": i, "amounts": tribute });
	}
};

KONOHA.DiplomacyManager.prototype.checkEvents = function(gameState, events)
{
	...
	for (let evt of events.DiplomacyChanged)
	{
		...
		else if (gameState.sharedScript.playersData[evt.player].isAlly[PlayerID] && gameState.isPlayerEnemy(evt.player))
		{
			...
			KONOHA.chatAnswerRequestDiplomacy(gameState, evt.player, "ally", response);
		}
 ...
 */
KONOHA.DiplomacyManager.prototype.lastManStandingCheck = function(gameState)
{
	...
	if (playerToTurnAgainst)
	{
		...
		{
			if (request.status === "waitingForTribute")
				KONOHA.chatAnswerRequestDiplomacy(gameState, player, request.requestType, "decline");
 ...
 */
KONOHA.DiplomacyManager.prototype.handleDiplomacyRequest = function(gameState, player, requestType)
{
	...
	KONOHA.chatAnswerRequestDiplomacy(gameState, player, requestType, response, requiredTribute);
};

KONOHA.DiplomacyManager.prototype.changePlayerDiplomacy = function(gameState, player, newDiplomaticStance)
{
	...
	if (this.Config.chat)
		KONOHA.chatNewDiplomacy(gameState, player, newDiplomaticStance);
};

KONOHA.DiplomacyManager.prototype.checkRequestedTributes = function(gameState)
{
	for (let [player, data] of this.receivedDiplomacyRequests)
		if (data.status === "waitingForTribute" && gameState.ai.elapsedTime > data.warnTime)
		{
			if (data.sentWarning)
			{
				...
				KONOHA.chatAnswerRequestDiplomacy(gameState, player, data.requestType, "decline");
			}
			else
			{
				data.sentWarning = true;
				data.warnTime = gameState.ai.elapsedTime + 60;
				KONOHA.chatAnswerRequestDiplomacy(gameState, player, data.requestType, "waitingForTribute", {
					"wanted": data.wanted,
					"type": data.type
				});
			}
 ...
 */
KONOHA.DiplomacyManager.prototype.sendDiplomacyRequest = function(gameState)
{
	let player;
	let max = 0;
	for (let i = 1; i < gameState.sharedScript.playersData.length; ++i)
	...
	KONOHA.chatNewRequestDiplomacy(gameState, player, requestType, "sendRequest");
};

KONOHA.DiplomacyManager.prototype.checkSentDiplomacyRequests = function(gameState)
{
	for (let [player, data] of this.sentDiplomacyRequests)
		...
		{
			KONOHA.chatNewRequestDiplomacy(gameState, player, data.requestType, "requestExpired");
			this.sentDiplomacyRequests.delete(player);
		}
};

KONOHA.DiplomacyManager.prototype.update = function(gameState, events)
{
...
KONOHA.DiplomacyManager.prototype.Serialize = function()
{
	return {
...
KONOHA.DiplomacyManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
