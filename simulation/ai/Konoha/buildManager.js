...
KONOHA.BuildManager.prototype.init = function(gameState)
{
...
KONOHA.BuildManager.prototype.incrementBuilderCounters = function(civ, ent, increment)
{
	for (let buildable of ent.buildableEntities(civ))
	{
		if (this.builderCounters.has(buildable))
		{
			...
			if (count < 0)
			{
				API3.warn(" Konoha error in incrementBuilderCounters for " + buildable + " with count < 0");
				continue;
			}
			...
			API3.warn(" Konoha error in incrementBuilderCounters for " + buildable + " not yet set");
	}
};

/** Update the builders counters */
KONOHA.BuildManager.prototype.checkEvents = function(gameState, events)
{
 ...
 */
KONOHA.BuildManager.prototype.findStructuresByFilter = function(gameState, filter)
{
	const result = [];
 ...
 */
KONOHA.BuildManager.prototype.findStructureWithClass = function(gameState, classes)
{
	return this.findStructuresByFilter(gameState, API3.Filters.byClasses(classes))[0];
};

KONOHA.BuildManager.prototype.hasBuilder = function(template)
{
	let numBuilders = this.builderCounters.get(template);
	return numBuilders && numBuilders > 0;
};

KONOHA.BuildManager.prototype.isUnbuildable = function(gameState, template)
{
	return this.unbuildables.has(template) && this.unbuildables.get(template).time > gameState.ai.elapsedTime;
};

KONOHA.BuildManager.prototype.setBuildable = function(template)
{
...
KONOHA.BuildManager.prototype.setUnbuildable = function(gameState, template, time = 90, reason = "room")
{
	if (!this.unbuildables.has(template))
...
KONOHA.BuildManager.prototype.numberMissingRoom = function(gameState)
{
	let num = 0;
...
KONOHA.BuildManager.prototype.resetMissingRoom = function(gameState)
{
...
KONOHA.BuildManager.prototype.Serialize = function()
{
	return {
...
KONOHA.BuildManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
