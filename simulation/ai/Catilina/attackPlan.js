...
CATILINA.AttackPlan = function(gameState, Config, uniqueID, type, data)
{
	this.Config = Config;
	this.name = uniqueID;
	...
	for (let base of gameState.ai.HQ.baseManagers)
	{
		if (!base.anchor || !base.anchor.position())
			continue;
		let access = CATILINA.getLandAccess(gameState, base.anchor);
		if (!rallyPoint)
		{
	...
	if (gameState.ai.HQ.navalMap)
	{
		for (let structure of gameState.getEnemyStructures().values())
		{
			...
			if (!structure.position())
				continue;
			let access = CATILINA.getLandAccess(gameState, structure);
			...
			else if (!this.overseas)
			{
				...
				if (!sea)
				{
					if (this.target)
					{
						API3.warn("Catilina: " + this.type + " " + this.name + " has an inaccessible target " +
	...
	{
		priority = 10000;
  /*  this.unitStat.RangedInfantry = { "priority": 0.7, "minSize": 0, "targetSize": 0, "batchSize": 3, "classes": ["Infantry+Ranged"],
			"interests": [["strength", 1],["canGather", 200], ["costsResource", 0.3, "stone"], ["costsResource", 0.3, "metal"]] };
		this.unitStat.MeleeInfantry = { "priority": 0.7, "minSize": 0, "targetSize": 0, "batchSize": 3, "classes": ["Infantry+Melee"],
			"interests": [["strength", 1],["canGather", 200], ["costsResource", 0.3, "stone"], ["costsResource", 0.3, "metal"]] };*/
		this.unitStat.FastMoving = { "priority": 1, "minSize": 2, "targetSize": 14, "batchSize": 2, "classes": ["FastMoving+CitizenSoldier"],
	...
	else if (type == "HugeAttack")
	{
		...
		this.unitStat.ChampMeleeInfantry = { "priority": 0.7, "minSize": 0, "targetSize": 10, "batchSize": 3, "classes": ["Infantry+Melee+Champion"],
			"interests": [["strength", 3]] };
		this.unitStat.RangedFastMoving = { "priority": 0.5, "minSize": 0, "targetSize": 10, "batchSize": 5, "classes": ["FastMoving+Ranged+CitizenSoldier"],
			"interests": [["strength", 2]] };
		this.unitStat.MeleeFastMoving = { "priority": 0.5, "minSize": 0, "targetSize": 10, "batchSize": 5, "classes": ["FastMoving+Melee+CitizenSoldier"],
			"interests": [["strength", 2]] };
		this.unitStat.ChampRangedFastMoving = { "priority": 0.7, "minSize": 0, "targetSize": 10, "batchSize": 3, "classes": ["FastMoving+Ranged+Champion"],
			"interests": [["strength", 3]] };
		this.unitStat.ChampMeleeFastMoving = { "priority": 0.7, "minSize": 0, "targetSize": 10, "batchSize": 3, "classes": ["FastMoving+Melee+Champion"],
			"interests": [["strength", 2]] };
    this.unitStat.Bolts = { "priority": 1, "minSize": 0, "targetSize": 3, "batchSize": 3, "classes": ["Siege+Ranged+BoltShooter"],
      "interests": [["strength", 2]] };
		this.unitStat.Hero = { "priority": 1, "minSize": 0, "targetSize": 1, "batchSize": 1, "classes": ["Hero"],
			"interests": [["strength", 2], ["speed", 1]] };
		this.neededShips = 5;
	}
  else if (type == "Reinforcement")
	{
		priority = 400;
		this.unitStat.RangedInfantry = { "priority": 1, "minSize": 10, "targetSize": 20, "batchSize": 4, "classes": ["Infantry+Ranged"],
			"interests": [["strength", 3]] };
		this.unitStat.MeleeInfantry = { "priority": 1, "minSize": 10, "targetSize": 20, "batchSize": 4, "classes": ["Infantry+Melee"],
			"interests": [["strength", 3]] };
		this.unitStat.FastMoving = { "priority": 1, "minSize": 0, "targetSize": 6, "batchSize": 2, "classes": ["FastMoving+CitizenSoldier"],
			"interests": [["strength", 2]] };
    this.unitStat.Siege = { "priority": 1, "minSize": 1, "targetSize": 1, "batchSize": 1, "classes": ["Siege"],
      "interests": [["strength", 2]] };
    this.unitStat.Elephant = { "priority": 1, "minSize": 0, "targetSize": 3, "batchSize": 3, "classes": ["Elephant+Melee"],
      "interests": [["strength", 2]] };
    this.unitStat.Hero = { "priority": 1, "minSize": 0, "targetSize": 1, "batchSize": 1, "classes": ["Hero"],
      "interests": [["strength", 2], ["speed", 1]] };
		this.neededShips = 3;
...
CATILINA.AttackPlan.prototype.init = function(gameState)
{
...
CATILINA.AttackPlan.prototype.getName = function()
{
	return this.name;
};

CATILINA.AttackPlan.prototype.getType = function()
{
	return this.type;
};

CATILINA.AttackPlan.prototype.isStarted = function()
{
	return this.state !== "unexecuted" && this.state !== "completing";
};

CATILINA.AttackPlan.prototype.isPaused = function()
{
	return this.paused;
};

CATILINA.AttackPlan.prototype.setPaused = function(boolValue)
{
 ...
 */
CATILINA.AttackPlan.prototype.canStart = function()
{
	if (!this.canBuildUnits)
		return true;
...
CATILINA.AttackPlan.prototype.mustStart = function()
{
	if (this.isPaused())
		return false;
...
CATILINA.AttackPlan.prototype.forceStart = function()
{
	for (let unitCat in this.unitStat)
	{
		let Unit = this.unitStat[unitCat];
...
CATILINA.AttackPlan.prototype.emptyQueues = function()
{
	this.queue.empty();
...
CATILINA.AttackPlan.prototype.removeQueues = function(gameState)
{
...
CATILINA.AttackPlan.prototype.addBuildOrder = function(gameState, name, unitStats, resetQueue)
{
	if (!this.isStarted())
	{
...
CATILINA.AttackPlan.prototype.updatePreparation = function(gameState)
{
	...
	else
	{
		if (this.type == "Rush" || this.forced)
			this.maxCompletingTime = gameState.ai.elapsedTime + 10;
		else
			this.maxCompletingTime = gameState.ai.elapsedTime + 30; //Nagasushi
	...
	for (let ent of this.unitCollection.values())
	{
		...
		if (ent.resourceCarrying() && ent.resourceCarrying().length)
			queued = CATILINA.returnResources(gameState, ent);
		let index = CATILINA.getLandAccess(gameState, ent);
		if (index == rallyIndex)
...
CATILINA.AttackPlan.prototype.trainMoreUnits = function(gameState)
{
	...
	if (firstOrder[0] < firstOrder[3].targetSize)
	{
		...
		if (queue.length() <= 5)
		{
			...
			else
			{
				...
				let trainingPlan = new CATILINA.TrainingPlan(gameState, template, data, max, max);
...
CATILINA.AttackPlan.prototype.assignUnits = function(gameState)
{
	let plan = this.name;
	let added = false;
	...
	let keep = this.type != "Rush" ?
		/*6 + 4 * gameState.getNumPlayerEnemies() + 8 * this.Config.personality.defensive*/0 : 0; //0: Nagasushi
	...
	for (const ent of gameState.getOwnEntitiesByRole("worker", true).values())
	{
		...
		{
			API3.warn("Catilina problem ent without base ");
...
CATILINA.AttackPlan.prototype.isAvailableUnit = function(gameState, ent)
{
	if (!ent.position())
		return false;
...
CATILINA.AttackPlan.prototype.reassignFastUnit = function(gameState)
{
	for (let ent of this.unitCollection.values())
	{
...
CATILINA.AttackPlan.prototype.chooseTarget = function(gameState)
{
	...
	let targetIndex = CATILINA.getLandAccess(gameState, this.target);
 ...
 */
CATILINA.AttackPlan.prototype.getNearestTarget = function(gameState, position, sameLand)
{
 ...
 */
CATILINA.AttackPlan.prototype.defaultTargetFinder = function(gameState, playerEnemy)
{
...
CATILINA.AttackPlan.prototype.isValidTarget = function(ent)
{
	if (!ent.position())
		return false;
	if (this.sameLand && CATILINA.getLandAccess(this.gameState, ent) != this.sameLand)
		return false;
...
CATILINA.AttackPlan.prototype.rushTargetFinder = function(gameState, playerEnemy)
{
	let targets = new API3.EntityCollection(gameState.sharedScript);
	let buildings;
...
CATILINA.AttackPlan.prototype.raidTargetFinder = function(gameState)
{
	let targets = new API3.EntityCollection(gameState.sharedScript);
 ...
 */
CATILINA.AttackPlan.prototype.checkTargetObstruction = function(gameState, target, position)
{
	if (CATILINA.getLandAccess(gameState, target) != gameState.ai.accessibility.getAccessValue(position))
		return target;
...
CATILINA.AttackPlan.prototype.getPathToTarget = function(gameState, fixedRallyPoint = false)
{
	let startAccess = gameState.ai.accessibility.getAccessValue(this.rallyPoint);
	let endAccess = CATILINA.getLandAccess(gameState, this.target);
...
CATILINA.AttackPlan.prototype.setRallyPoint = function(gameState)
{
 ...
 */
CATILINA.AttackPlan.prototype.StartAttack = function(gameState)
{
	if (this.Config.debug > 1)
	...
	for (let ent of this.unitCollection.values())
	{
		...
		if ( !(ent.hasClass("Melee") && ent.hasClass("Elephant")) && !(ent.hasClass("Melee") && ent.hasClasses("Siege")) && ent.getStance() != stance)
			ent.setStance(stance);
    else if ( (ent.hasClass("Melee") && ent.hasClass("Elephant")) || (ent.hasClass("Melee") && ent.hasClass("Siege")))
  		ent.setStance("aggressive");
	}

	let rallyAccess = gameState.ai.accessibility.getAccessValue(this.rallyPoint);
	let targetAccess = CATILINA.getLandAccess(gameState, this.target);
  ...
  this.nUnits = this.unitCollection.length; //Nagasushi
	return true;
};

/** Runs every turn after the attack is executed */
CATILINA.AttackPlan.prototype.update = function(gameState, events)
{
  ...
  if (this.unitCollection.length < this.nUnits * 0.4 && this.type != "Reinforcement")
  {
    /*gameState.ai.HQ.attackManager.startedAttacks.HugeAttack.length = 0;*/
    let startedAttacks = gameState.ai.HQ.attackManager.startedAttacks;
    for (let attackType in startedAttacks)
  		for (let i = 0; i < startedAttacks[attackType].length; ++i)
  		{
  			let attack = startedAttacks[attackType][i];
  			{
  				attack.Abort(gameState);
  				startedAttacks[attackType].splice(i--, 1);
  			}
  		}
    Engine.ProfileStop();
    return 0;
	...
	if (this.state == "")
	{
      ...
      if (attacker.hasClass("Siege"))
      {	// if our unit is attacked by a siege unit, we'll send some melee units to help it.
        let collec = this.unitCollection.filter(API3.Filters.byClass("Melee")).filterNearest(ourUnit.position(), 10);
        ...
        if (CATILINA.isSiegeUnit(attacker) && attacker.hasClass("Melee") && ourUnit.hasClass("Melee") && ourUnit.canAttackTarget(attacker, CATILINA.allowCapture(gameState, ourUnit, attacker)))
				{
					ourUnit.attack(attacker.id(), CATILINA.allowCapture(gameState, ourUnit, attacker));
					ourUnit.setMetadata(PlayerID, "lastAttackPlanUpdateTime", time);
				}
      }

			else if (CATILINA.isSiegeUnit(ourUnit) || ourUnit.hasClass("BoltShooter"))
			{	// if our siege units are attacked, we'll send some units to deal with enemies.
				let collec = this.unitCollection.filter(API3.Filters.not(API3.Filters.byClass("Siege"))).filterNearest(ourUnit.position(), 10); //Nagaushi
				for (let ent of collec.values())
				{
					if (CATILINA.isSiegeUnit(ent))	// needed as mauryan elephants are not filtered out
						continue;
			...
			else
			{
				...
				/*else if (CATILINA.isSiegeUnit(attacker)|| attacker.hasClass("BoltShooter"))
				{	// if our unit is attacked by a siege unit, we'll send some melee units to help it.
				...
				else
				{
					...
					let allowCapture = CATILINA.allowCapture(gameState, ourUnit, attacker);
		...
		for (let ent of this.unitCollection.values())
		{
			...
			{
				if (CATILINA.isSiegeUnit(target) || target.hasClass("Hero"))
    ...
    let targetClassesPierce;
		if (this.type == "Rush")
			targetClassesUnit = { "attack": ["Unit", "House"], "avoid": ["Palisade", "Wall", "Tower", "Fortress", "CivCentre", "Field", "Palisade", "Infantry+Melee"], "vetoEntities": veto };
		else
		{
			if (this.target.hasClass("Fortress"))
				targetClassesUnit = { "attack": ["Unit", "Structure"], "avoid": ["Palisade", "Wall", "Field"], "vetoEntities": veto };
			else if (this.target.hasClasses(["Palisade", "Wall"]))
				targetClassesUnit = { "attack": ["Unit", "Structure"], "avoid": ["Fortress", "Field"], "vetoEntities": veto };
			else
				targetClassesUnit = { "attack": ["Unit", "Structure"], "avoid": ["Palisade", "Wall", "Fortress", "Field"], "vetoEntities": veto };
		}
		if (this.target.hasClass("Structure"))
	   {
       targetClassesSiege = { "attack": ["Structure"], "avoid": ["Unit", "House", "Storehouse", "Farmstead", "Field", "Forge"], "vetoEntities": veto };
       targetClassesPierce = { "attack": ["Unit", "Structure"], "avoid": ["Siege", "Palisade", "Wall", "Fortress", "Field"], "vetoEntities": veto };
     }
		else
		 { targetClassesSiege = { "attack": ["Structure"], "avoid": ["Unit", "House", "Storehouse", "Farmstead", "Field", "Forge"], "vetoEntities": veto };
       targetClassesPierce = { "attack": ["Unit", "Structure"], "avoid": ["Structure", "Field", "Siege"], "vetoEntities": veto };
     }
		...
		let lgth = this.unitCollUpdateArray.length < 15 || this.startingAttack ? this.unitCollUpdateArray.length : 30;
		for (let check = 0; check < lgth; check++)
		{
			...
			let siegeUnit = CATILINA.isSiegeUnit(ent);
      let pierceUnit = CATILINA.isPierceUnit(ent);
			if (ent.isIdle())
				needsUpdate = true;
			...
			if (this.isBlocked)
			{
  			...
  			range += 60; //Nagaushi
				...
				range += 30;
			range *= range;
			let entAccess = CATILINA.getLandAccess(gameState, ent);
			// Checking for gates if we're a siege unit.
			if (siegeUnit)
			{
				let mStruct = enemyStructures.filter(enemy => {
					if (!enemy.position() || !ent.canAttackTarget(enemy, CATILINA.allowCapture(gameState, ent, enemy)))
						return false;
					if (API3.SquareVectorDistance(enemy.position(), ent.position()) > range)
						return false;
					if (enemy.foundationProgress() == 0)
						return false;
					if (CATILINA.getLandAccess(gameState, enemy) != entAccess)
						return false;
					return true;
				}).toEntityArray();
				if (mStruct.length)
				{
					mStruct.sort((structa, structb) => {
						...
						if (structa.hasClass("Gate") && ent.canAttackClass("Wall"))
							vala += 10000;
            else if (structa.hasClass("CivCentre"))
  						vala += 8000;
            else if (structa.hasClass("Fortress"))
    					vala += 6000;
      				...
      				valb += 6000;
							...
							valb += 200;
						return valb - vala; //Nagasushi
					});
					if (mStruct[0].hasClass("Gate"))
						ent.attack(mStruct[0].id(), CATILINA.allowCapture(gameState, ent, mStruct[0]));
					else
					{
						let rand = randIntExclusive(0, mStruct.length * 0.2);
						ent.attack(mStruct[0].id(), CATILINA.allowCapture(gameState, ent, mStruct[rand]));
					}
				}
				else
				{
			...
			else
			{
				...
				if (mUnit.length)
				{
					...
					ent.attack(mUnit[rand].id(), CATILINA.allowCapture(gameState, ent, mUnit[rand]));
				}
				...
				else if (API3.SquareVectorDistance(this.targetPos, ent.position()) > 2500)
				{
					...
					if (maybeUpdate && ent.unitAIState() === "INDIVIDUAL.COMBAT.APPROACHING")	// we may be blocked by walls, attack everything
					{
  					...
  					targetClasses = targetClassesPierce;
					ent.attackMove(this.targetPos[0], this.targetPos[1], targetClasses);
				}
				else
				{
					...
					if (mStruct.length)
					{
						...
						{
							let rand = randIntExclusive(0, mStruct.length * 0.2);
							ent.attack(mStruct[rand].id(), CATILINA.allowCapture(gameState, ent, mStruct[rand]));
						}
					}
					else if (needsUpdate)  // really nothing   let's try to help our nearest unit
					{
						...
						this.unitCollection.forEach(unit => {
							...
							if (dist > distmin)
								return;
							distmin = dist;
							if (!ent.canAttackTarget(target, CATILINA.allowCapture(gameState, ent, target)))
								return;
							attacker = target;
						});
						if (attacker)
							ent.attack(attacker.id(), CATILINA.allowCapture(gameState, ent, attacker));
					}
				}
			}
		}
...
CATILINA.AttackPlan.prototype.UpdateTransporting = function(gameState, events)
{
	let done = true;
	for (let ent of this.unitCollection.values())
	{
...
CATILINA.AttackPlan.prototype.UpdateWalking = function(gameState, events)
{
...
CATILINA.AttackPlan.prototype.UpdateTarget = function(gameState)
{
	...
	if (this.target && this.target.hasClass("Unit"))
	{
		...
		if (!this.targetPos)
		{
			let holder = CATILINA.getHolder(gameState, this.target);
			...
			for (let attackType in attackManager.startedAttacks)
			{
				for (let attack of attackManager.startedAttacks[attackType])
				{
					...
					if (!attack.target || !gameState.getEntityById(attack.target.id()) ||
						...
						continue;
					if (accessIndex != CATILINA.getLandAccess(gameState, attack.target))
						continue;
...
/** reset any units */
CATILINA.AttackPlan.prototype.Abort = function(gameState)
{
...
CATILINA.AttackPlan.prototype.removeUnit = function(ent, update)
{
	if (ent.getMetadata(PlayerID, "role") == "attack")
	{
...
CATILINA.AttackPlan.prototype.checkEvents = function(gameState, events)
{
...
CATILINA.AttackPlan.prototype.waitingForTransport = function()
{
	for (let ent of this.unitCollection.values())
		if (ent.getMetadata(PlayerID, "transport") !== undefined)
			return true;
...
CATILINA.AttackPlan.prototype.hasSiegeUnits = function()
{
	for (let ent of this.unitCollection.values())
		if (CATILINA.isSiegeUnit(ent))
			return true;
	return false;
};

CATILINA.AttackPlan.prototype.hasForceOrder = function(data, value)
{
	for (let ent of this.unitCollection.values())
	{
 ...
 */
CATILINA.AttackPlan.prototype.getAttackAccess = function(gameState)
{
	for (let ent of this.unitCollection.filterNearest(this.position, 1).values())
		return CATILINA.getLandAccess(gameState, ent);

	return 0;
};

CATILINA.AttackPlan.prototype.debugAttack = function()
{
...
CATILINA.AttackPlan.prototype.Serialize = function()
{
	let properties = {
		"name": this.name,
		"type": this.type,
...
CATILINA.AttackPlan.prototype.Deserialize = function(gameState, data)
{
