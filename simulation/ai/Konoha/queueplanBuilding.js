...
KONOHA.ConstructionPlan = function(gameState, type, metadata, position)
{
	if (!KONOHA.QueuePlan.call(this, gameState, type, metadata))
		return false;
...
KONOHA.ConstructionPlan.prototype = Object.create(KONOHA.QueuePlan.prototype);

KONOHA.ConstructionPlan.prototype.canStart = function(gameState)
{
...
KONOHA.ConstructionPlan.prototype.start = function(gameState)
{
	...
	if (!builder)
	{
		API3.warn("konoha error: builder not found when starting construction.");
		Engine.ProfileStop();
		return;
	}
...
KONOHA.ConstructionPlan.prototype.findGoodPosition = function(gameState)
{
	...
	else	// No position was specified so try and find a sensible place to build
	{
		...
		if (!HQ.requireHouses || !template.hasClass("House"))
		{
			gameState.getOwnStructures().forEach(function(ent) {
				...
				let struct = KONOHA.getBuiltEntity(gameState, ent);
		...
		if (template.hasClass("Farmstead"))
		{
			for (let j = 0; j < placement.map.length; ++j)
			{
				...
				if (HQ.borderMap.map[j] & KONOHA.fullBorder_Mask)
	...
	if (this.metadata && this.metadata.base !== undefined)
	{
		...
		for (let j = 0; j < placement.map.length; ++j)
		{
			...
			{
				if (favorBorder && HQ.borderMap.map[j] & KONOHA.border_Mask)
					placement.set(j, placement.map[j] + 50);
				else if (disfavorBorder && !(HQ.borderMap.map[j] & KONOHA.fullBorder_Mask))
 ...
 */
KONOHA.ConstructionPlan.prototype.findDockPosition = function(gameState)
{
	let template = this.template;
	...
	for (let j = 0; j < territoryMap.length; ++j)
	{
		...
		if (oversea)
		{
			...
			{
				if (KONOHA.getSeaAccess(gameState, dock) != navalPassMap[i])
					continue;
		...
		if (gameState.ai.HQ.borderMap.map[j] & KONOHA.fullBorder_Mask)
			score += 20;
 ...
 */
KONOHA.ConstructionPlan.prototype.buildOverseaDock = function(gameState, template)
{
	...
	for (let i = 0; i < gameState.ai.accessibility.regionSize.length; ++i)
	{
		...
		for (let dock of docks.values())
		{
			if (KONOHA.getLandAccess(gameState, dock) != i)
				continue;
			keep = false;
			break;
		}
		...
		for (let cc of ccEnts.values())
		{
			let ccAccess = KONOHA.getLandAccess(gameState, cc);
			if (ccAccess != i)
			{
	...
	if (!gameState.ai.HQ.navalMap)
		API3.warn("konoha.findOverseaLand on a non-naval map??? we should never go there ");
...
KONOHA.ConstructionPlan.prototype.getDockAngle = function(gameState, x, z, size)
{
 ...
 */
KONOHA.ConstructionPlan.prototype.checkDockPlacement = function(gameState, x, z, halfDepth, halfWidth, angle)
{
 ...
 */
KONOHA.ConstructionPlan.prototype.isDockLocation = function(gameState, j, dimension, wantedLand, wantedSea)
{
 ...
 */
KONOHA.ConstructionPlan.prototype.getFrontierProximity = function(gameState, j)
{
	...
	for (let a of around)
	{
		for (let i = 1; i < 5; ++i)
		{
			...
			if (jz < 0 || jz >= width)
				continue;
			if (borderMap.map[jx+width*jz] & KONOHA.outside_Mask)
				continue;
 ...
 */
KONOHA.ConstructionPlan.prototype.getResourcesAround = function(gameState, types, i, radius)
{
...
KONOHA.ConstructionPlan.prototype.isGo = function(gameState)
{
...
KONOHA.ConstructionPlan.prototype.onStart = function(gameState)
{
...
KONOHA.ConstructionPlan.prototype.Serialize = function()
{
	return {
...
KONOHA.ConstructionPlan.prototype.Deserialize = function(gameState, data)
{
	for (let key in data)
		this[key] = data[key];
