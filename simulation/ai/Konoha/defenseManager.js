KONOHA.DefenseManager = function(Config)
{
...
KONOHA.DefenseManager.prototype.update = function(gameState, events)
{
...
KONOHA.DefenseManager.prototype.makeIntoArmy = function(gameState, entityID, type = "default")
{
	if (type == "default")
	{
	...
	let army = new KONOHA.DefenseArmy(gameState, [entityID], type);

	this.armies.push(army);
};

KONOHA.DefenseManager.prototype.getArmy = function(partOfArmy)
{
...
KONOHA.DefenseManager.prototype.isDangerous = function(gameState, entity)
{
	if (!entity.position())
		return false;
...
KONOHA.DefenseManager.prototype.checkEnemyUnits = function(gameState)
{
...
KONOHA.DefenseManager.prototype.checkEnemyArmies = function(gameState)
{
...
KONOHA.DefenseManager.prototype.assignDefenders = function(gameState)
{
	if (!this.armies.length)
		return;
	...
	for (let army of this.armies)
	{
		...
		for (let entId of army.foeEntities)
		{
			...
			if (!ent || !ent.position())
				continue;
			armyAccess = KONOHA.getLandAccess(gameState, ent);
			break;
		}
		if (!armyAccess)
			API3.warn(" Konoha error: attacking army " + army.ID + " without access");
		army.recalculatePosition(gameState);
	...
	for (let ipass = 0; ipass < 2; ++ipass)
	{
		...
		for (let i = 0; i < potentialDefenders.length; ++i)
		{
			...
			let aMin;
			let distMin;
			let access = ipass == 0 ? KONOHA.getLandAccess(gameState, ent) : undefined;
			for (let a = 0; a < armiesNeeding.length; ++a)
			{
				...
				if (!armiesNeeding[a].army.foeEntities.some(eEnt => {
					...
					return ent.canAttackTarget(eEntID, KONOHA.allowCapture(gameState, ent, eEntID));
					}))
					continue;
			...
			armiesNeeding[aMin].need -= KONOHA.getMaxStrength(ent, this.Config.debug, this.Config.DamageTypeImportance);
...
KONOHA.DefenseManager.prototype.abortArmy = function(gameState, army)
{
	army.clear(gameState);
	for (let i = 0; i < this.armies.length; ++i)
	{
 ...
 */
KONOHA.DefenseManager.prototype.checkEvents = function(gameState, events)
{
		...
		if (target.hasClass("Unit") && attacker.hasClass("Unit"))
		{
			...
			if (currentTarget)
			{
				...
				if (unitAIStateOrder == "REPAIR" && currentTarget.hasDefensiveFire())
					continue;
				if (unitAIStateOrder == "COMBAT" && !KONOHA.isSiegeUnit(currentTarget) &&
				    gameState.ai.HQ.capturableTargets.has(orderData[0].target))
				{
					...
					for (let entId of capturableTarget.ents)
					{
						if (allAttacked[entId])
							continue;
						let ent = gameState.getEntityById(entId);
						if (!ent || !ent.position() || !ent.canAttackTarget(attacker, KONOHA.allowCapture(gameState, ent, attacker)))
							continue;
					...
					if (minEnt)
					{
						...
						minEnt.attack(attacker.id(), KONOHA.allowCapture(gameState, minEnt, attacker));
					}
				}
			}
			let allowCapture = KONOHA.allowCapture(gameState, target, attacker);
...
KONOHA.DefenseManager.prototype.garrisonUnitsInside = function(gameState, target, data)
{
	...
	let access = KONOHA.getLandAccess(gameState, target);
	...
	if (allowMelee === undefined)
	{
		...
		if (data.attacker)
			allowMelee = data.attacker.hasClass("Structure") ? data.attacker.attackRange("Ranged") : !KONOHA.isSiegeUnit(data.attacker);
		else
			allowMelee = true;
	}
	let units = gameState.getOwnUnits().filter(ent => {
		...
		if (KONOHA.getLandAccess(gameState, ent) != access)
			return false;
		return true;
...
KONOHA.DefenseManager.prototype.garrisonSiegeUnit = function(gameState, unit)
{
	let distmin = Math.min();
	let nearest;
	let unitAccess = KONOHA.getLandAccess(gameState, unit);
	let garrisonManager = gameState.ai.HQ.garrisonManager;
	for (let ent of gameState.getAllyStructures().values())
	{
		...
		if (ent.hitpoints() < ent.garrisonEjectHealth() * ent.maxHitpoints())
			continue;
		if (KONOHA.getLandAccess(gameState, ent) != unitAccess)
			continue;
 ...
 */
KONOHA.DefenseManager.prototype.garrisonAttackedUnit = function(gameState, unit, emergency = false)
{
	let distmin = Math.min();
	let nearest;
	let unitAccess = KONOHA.getLandAccess(gameState, unit);
 ...
 */
KONOHA.DefenseManager.prototype.GetCooperationLevel = function(ally)
{
 ...
 */
KONOHA.DefenseManager.prototype.switchToAttack = function(gameState, army)
{
	if (!army)
		return;
	for (let targetId of this.targetList)
	{
		...
		{
			let ent = gameState.getEntityById(entId);
			if (!ent || !ent.position() || KONOHA.getLandAccess(gameState, ent) != targetAccess)
				continue;
...
KONOHA.DefenseManager.prototype.Serialize = function()
{
	let properties = {
...
KONOHA.DefenseManager.prototype.Deserialize = function(gameState, data)
{
	...
	{
		let army = new KONOHA.DefenseArmy(gameState, []);
