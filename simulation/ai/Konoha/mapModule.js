...
KONOHA.TERRITORY_PLAYER_MASK = 0x1F;
KONOHA.TERRITORY_BLINKING_MASK = 0x40;

KONOHA.createObstructionMap = function(gameState, accessIndex, template)
{
	...
	{
		let tilePlayer = territoryMap.data[k] & KONOHA.TERRITORY_PLAYER_MASK;
		let isConnected = (territoryMap.data[k] & KONOHA.TERRITORY_BLINKING_MASK) == 0;
...
KONOHA.createTerritoryMap = function(gameState)
{
	let map = gameState.ai.territoryMap;

	let ret = new API3.Map(gameState.sharedScript, "territory", map.data);
	ret.getOwner = function(p) { return this.point(p) & KONOHA.TERRITORY_PLAYER_MASK; };
	ret.getOwnerIndex = function(p) { return this.map[p] & KONOHA.TERRITORY_PLAYER_MASK; };
	ret.isBlinking = function(p) { return (this.point(p) & KONOHA.TERRITORY_BLINKING_MASK) != 0; };
	return ret;
};
...
KONOHA.outside_Mask = 1;
KONOHA.border_Mask = 2;
KONOHA.fullBorder_Mask = KONOHA.outside_Mask | KONOHA.border_Mask;
KONOHA.narrowFrontier_Mask = 4;
KONOHA.largeFrontier_Mask = 8;
KONOHA.fullFrontier_Mask = KONOHA.narrowFrontier_Mask | KONOHA.largeFrontier_Mask;

KONOHA.createBorderMap = function(gameState)
{
...
KONOHA.debugMap = function(gameState, map)
{
	let width = map.width;
