 ...
 */
CATILINA.NavalManager = function(Config)
{
	this.Config = Config;
...
CATILINA.NavalManager.prototype.init = function(gameState, deserializing)
{
	// docks
	...
	for (let ship of this.ships.values())
		CATILINA.setSeaAccess(gameState, ship);
	for (let dock of this.docks.values())
		CATILINA.setSeaAccess(gameState, dock);
};

CATILINA.NavalManager.prototype.updateFishingBoats = function(sea, num)
{
	if (this.wantedFishShips[sea])
		this.wantedFishShips[sea] = num;
};

CATILINA.NavalManager.prototype.resetFishingBoats = function(gameState, sea)
{
	if (sea !== undefined)
...
CATILINA.NavalManager.prototype.getFishSea = function(gameState, fish)
{
...
CATILINA.NavalManager.prototype.canFishSafely = function(gameState, fish)
{
...
CATILINA.NavalManager.prototype.getUnconnectedSeas = function(gameState, region)
{
	let seas = gameState.ai.accessibility.regionLinks[region].slice();
	this.docks.forEach(dock => {
		if (!dock.hasClass("Dock") || CATILINA.getLandAccess(gameState, dock) != region)
			return;
		let i = seas.indexOf(CATILINA.getSeaAccess(gameState, dock));
		if (i != -1)
			seas.splice(i--, 1);
	});
	return seas;
...
CATILINA.NavalManager.prototype.checkEvents = function(gameState, queues, events)
{
...
CATILINA.NavalManager.prototype.getPlan = function(ID)
{
...
CATILINA.NavalManager.prototype.addPlan = function(plan)
{
 ...
 */
CATILINA.NavalManager.prototype.requireTransport = function(gameState, ent, startIndex, endIndex, endPos)
{
	if (!ent.canGarrison())
		return false;
	...
	if (ent.getMetadata(PlayerID, "transport") !== undefined)
	{
		if (this.Config.debug > 0)
			API3.warn("Catilina naval manager error: unit " + ent.id() + " has already required a transport");
		return false;
	}
	...
	for (let plan of this.transportPlans)
	{
		...
		if (CATILINA.isSiegeUnit(ent) && plan.units.filter(unit => CATILINA.isSiegeUnit(unit)).length > 3)
			continue;
		plans.push(plan);
	}
	...
	let plan = new CATILINA.TransportPlan(gameState, [ent], startIndex, endIndex, endPos);
	if (plan.failed)
	{
		if (this.Config.debug > 1)
...
CATILINA.NavalManager.prototype.splitTransport = function(gameState, plan)
{
	if (this.Config.debug > 1)
		API3.warn(">>>> split of transport plan started <<<<");
	let newplan = new CATILINA.TransportPlan(gameState, [], plan.startIndex, plan.endIndex, plan.endPos);
	if (newplan.failed)
	{
		if (this.Config.debug > 1)
 ...
 */
CATILINA.NavalManager.prototype.createTransportIfNeeded = function(gameState, fromPos, toPos, toAccess)
{
	...
	for (let ship of this.ships.values())
	{
		...
		let plan = new CATILINA.TransportPlan(gameState, units, fromAccess, toAccess, toPos, ship);
		if (plan.failed)
			continue;
		plan.init(gameState);
		this.transportPlans.push(plan);
...
CATILINA.NavalManager.prototype.setMinimalTransportShips = function(gameState, sea, number)
{
	if (!sea)
		return;
...
CATILINA.NavalManager.prototype.checkLevels = function(gameState, queues)
{
...
CATILINA.NavalManager.prototype.maintainFleet = function(gameState, queues)
{
	if (queues.ships.hasQueuedUnits())
		return;
	...
	for (let sea = 0; sea < this.seaShips.length; ++sea)
	{
		...
		if (this.seaTransportShips[sea].length < this.wantedTransportShips[sea])
		{
			...
			if (template)
			{
				queues.ships.addPlan(new CATILINA.TrainingPlan(gameState, template, { "sea": sea }, 1, 1));
				continue;
			}
		}
		...
		if (this.seaFishShips[sea].length < this.wantedFishShips[sea])
		{
			...
			if (template)
			{
				queues.ships.addPlan(new CATILINA.TrainingPlan(gameState, template, { "base": 0, "role": "worker", "sea": sea }, 1, 1));
				continue;
			}
		}
	}
...
CATILINA.NavalManager.prototype.assignShipsToPlans = function(gameState)
{
	for (let plan of this.transportPlans)
...
CATILINA.NavalManager.prototype.isShipBoarding = function(ship)
{
	if (!ship.position())
		return false;
 ...
 */
CATILINA.NavalManager.prototype.moveApart = function(gameState)
{
	let blockedShips = [];
	let blockedIds = [];
	...
	for (let ship of this.ships.values())
	{
		...
		else if (ship.isIdle())
		{
			...
			if (CATILINA.gatherTreasure(gameState, ship, true))
				continue;
			...
			{
				if (CATILINA.getSeaAccess(gameState, dock) != sea)
					continue;
...
CATILINA.NavalManager.prototype.buildNavalStructures = function(gameState, queues)
{
			...
			gameState.ai.HQ.canBuild(gameState, "structures/{civ}/dock"))
		{
			...
			for (let base of gameState.ai.HQ.baseManagers)
			{
				...
				for (let sea of remaining)
				{
					...
					queues.dock.addPlan(new CATILINA.ConstructionPlan(gameState, "structures/{civ}/dock", { "land": wantedLand, "sea": sea }));
					dockStarted = true;
					break;
				}
			}
	...
	queues.militaryBuilding.addPlan(new CATILINA.ConstructionPlan(gameState, template, { "land": wantedLand, "sea": sea }));
};

/** goal can be either attack (choose ship with best arrowCount) or transport (choose ship with best capacity) */
CATILINA.NavalManager.prototype.getBestShip = function(gameState, sea, goal)
{
...
CATILINA.NavalManager.prototype.update = function(gameState, queues, events)
{
...
CATILINA.NavalManager.prototype.Serialize = function()
{
	let properties = {
...
CATILINA.NavalManager.prototype.Deserialize = function(gameState, data)
{
	...
	for (let i in data.transports)
	{
		...
		let plan = new CATILINA.TransportPlan(gameState, [], dataPlan.startIndex, dataPlan.endIndex, dataPlan.endPos);
