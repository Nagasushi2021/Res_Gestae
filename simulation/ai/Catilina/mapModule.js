...
CATILINA.TERRITORY_PLAYER_MASK = 0x1F;
CATILINA.TERRITORY_BLINKING_MASK = 0x40;

CATILINA.createObstructionMap = function(gameState, accessIndex, template)
{
	...
	{
		let tilePlayer = territoryMap.data[k] & CATILINA.TERRITORY_PLAYER_MASK;
		let isConnected = (territoryMap.data[k] & CATILINA.TERRITORY_BLINKING_MASK) == 0;
...
CATILINA.createTerritoryMap = function(gameState)
{
	let map = gameState.ai.territoryMap;

	let ret = new API3.Map(gameState.sharedScript, "territory", map.data);
	ret.getOwner = function(p) { return this.point(p) & CATILINA.TERRITORY_PLAYER_MASK; };
	ret.getOwnerIndex = function(p) { return this.map[p] & CATILINA.TERRITORY_PLAYER_MASK; };
	ret.isBlinking = function(p) { return (this.point(p) & CATILINA.TERRITORY_BLINKING_MASK) != 0; };
	return ret;
};
...
CATILINA.outside_Mask = 1;
CATILINA.border_Mask = 2;
CATILINA.fullBorder_Mask = CATILINA.outside_Mask | CATILINA.border_Mask;
CATILINA.narrowFrontier_Mask = 4;
CATILINA.largeFrontier_Mask = 8;
CATILINA.fullFrontier_Mask = CATILINA.narrowFrontier_Mask | CATILINA.largeFrontier_Mask;

CATILINA.createBorderMap = function(gameState)
{
...
CATILINA.debugMap = function(gameState, map)
{
	let width = map.width;
