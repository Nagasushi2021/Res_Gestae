...
CATILINA.Worker.prototype.update = function(gameState, ent)
{
	...
	if (ent.getMetadata(PlayerID, "transport") !== undefined)
	{
		...
		if (subrole == "gatherer" || subrole == "hunter")
		{
			...
			if (plan.state == "boarding" && ent.position())
			{
				...
				{
					if (CATILINA.getLandAccess(gameState, structure) != plan.endIndex)
						continue;
					let resourceDropsiteTypes = CATILINA.getBuiltEntity(gameState, structure).resourceDropsiteTypes();
				...
				if (!hasDropsite)
				{
					...
					{
						if (!unit.position() || CATILINA.getLandAccess(gameState, unit) != plan.endIndex)
							continue;
	...
	this.entAccess = CATILINA.getLandAccess(gameState, ent);
		...
		if (unitAIState == "INDIVIDUAL.COMBAT.APPROACHING" && ent.unitAIOrderData().length)
		{
			...
			if (orderData && orderData.target)
			{
				...
				if (target && target.resourceSupplyType() && target.resourceSupplyType().generic == "food")
				{
					...
					{
						let distanceSquare = CATILINA.isFastMoving(ent) ? 90000 : 3000; //4550 calculated by Nagasushi
						...
						for (let dropsite of foodDropsites.values())
						{
							...
							if (owner != PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
								continue;
							if (targetAccess != CATILINA.getLandAccess(gameState, dropsite))
								continue;
	...
	if (unitAIStateOrder == "COMBAT")
	{
			...
			!ent.getMetadata(PlayerID, "PartOfArmy"))
		{
			...
			{
				let target = gameState.getEntityById(orderData.target);
				if (target && (!target.position() || CATILINA.getLandAccess(gameState, target) != this.entAccess))
				{
					if (this.retryWorking(gameState, subrole))
						return;
					ent.stopMoving();
			...
			!ent.getMetadata(PlayerID, "PartOfArmy"))
		{
			...
			if (orderData && orderData.target && orderData.attackType && orderData.attackType == "Capture")
			{
				...
				if (target && target.owner() > 0 && !gameState.isPlayerAlly(target.owner()))
					ent.attack(orderData.target, CATILINA.allowCapture(gameState, ent, target));
			}
		}
		return;
	}
	...
	if (subrole == "gatherer")
	{
		if (ent.isIdle())
		{
			...
			else if (!CATILINA.returnResources(gameState, ent))     // try to deposit resources
			{
	...
	else if (subrole == "builder")
	{
		...
		{
			let goalAccess = CATILINA.getLandAccess(gameState, target);
			let queued = CATILINA.returnResources(gameState, ent);
		...
		else	// Perform some sanity checks
		{
			if (unitAIStateOrder == "GATHER")
			{
				...
				else if (unitAIState == "INDIVIDUAL.GATHER.RETURNINGRESOURCE.APPROACHING")
				{
					...
					let dropsite = gameState.getEntityById(ent.unitAIOrderData()[0].target);
					if (dropsite && dropsite.position() && this.entAccess != CATILINA.getLandAccess(gameState, dropsite))
						CATILINA.returnResources(gameState, ent);
				}
			}
		}
	}
...
CATILINA.Worker.prototype.retryWorking = function(gameState, subrole)
{
	switch (subrole)
	{
	case "gatherer":
...
CATILINA.Worker.prototype.startBuilding = function(gameState)
{
	let target = gameState.getEntityById(this.ent.getMetadata(PlayerID, "target-foundation"));
	if (!target || target.foundationProgress() === undefined && target.needsRepair() == false)
		return false;
	if (CATILINA.getLandAccess(gameState, target) != this.entAccess)
		return false;
	this.ent.repair(target, target.hasClass("House"));  // autocontinue=true for houses
	return true;
};

CATILINA.Worker.prototype.startGathering = function(gameState)
{
	// First look for possible treasure if any
	if (CATILINA.gatherTreasure(gameState, this.ent))
		return true;
	...
	let findSupply = function(ent, supplies) {
		...
		for (let i = 0; i < supplies.length; ++i)
		{
			...
			if (CATILINA.IsSupplyFull(gameState, supplies[i].ent))
				continue;
	...
	let shouldBuild = this.ent.isBuilder() && foundations.some(function(foundation) {
		if (!foundation || CATILINA.getLandAccess(gameState, foundation) != this.entAccess)
			return false;
	...
	shouldBuild = this.ent.isBuilder() && foundations.some(function(foundation) {
		...
		if (structure.resourceDropsiteTypes() && structure.resourceDropsiteTypes().indexOf(resource) != -1)
		{
			let foundationAccess = CATILINA.getLandAccess(gameState, foundation);
 ...
 */
CATILINA.Worker.prototype.startHunting = function(gameState, position)
{
	// First look for possible treasure if any
	if (!position && CATILINA.gatherTreasure(gameState, this.ent))
		return true;
	...
	let isFastMoving = CATILINA.isFastMoving(this.ent);
	...
	let hasFoodDropsiteWithinDistance = function(supplyPosition, supplyAccess, distSquare)
	{
		for (let dropsite of foodDropsites.values())
		{
			...
			if (owner != PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
				continue;
			if (supplyAccess != CATILINA.getLandAccess(gameState, dropsite))
				continue;
	...
	for (let supply of resources.values())
	{
		...
		if (CATILINA.IsSupplyFull(gameState, supply))
			continue;
		// Check if available resource is worth one additionnal gatherer (except for farms).
		...
		let supplyAccess = CATILINA.getLandAccess(gameState, supply);
		...
		if (!isFastMoving && canFlee && territoryOwner == 0)
			continue;
		let distanceSquare = isFastMoving ? 200000 : (canFlee ? 4500 : 4500); //Nagasushi
...
CATILINA.Worker.prototype.startFishing = function(gameState)
{
	...
	let fisherSea = CATILINA.getSeaAccess(gameState, this.ent);
	...
	let nearestDropsiteDist = function(supply) {
		...
		for (let dropsite of fishDropsites)
		{
			...
			if (owner != PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
				continue;
			if (fisherSea != CATILINA.getSeaAccess(gameState, dropsite))
				continue;
...
CATILINA.Worker.prototype.gatherNearestField = function(gameState, baseID)
{
	...
	{
		if (CATILINA.IsSupplyFull(gameState, field))
			continue;
 ...
 */
CATILINA.Worker.prototype.buildAnyField = function(gameState, baseID)
{
 ...
 */
CATILINA.Worker.prototype.moveToGatherer = function(gameState, ent, forced)
{
	...
	for (let gatherer of gatherers.values())
	{
		...
		if (!gatherer.position() || gatherer.getMetadata(PlayerID, "transport") !== undefined ||
		    CATILINA.getLandAccess(gameState, gatherer) != access || gatherer.isIdle())
			continue;
 ...
 */
CATILINA.Worker.prototype.isInaccessibleSupply = function(gameState)
{
