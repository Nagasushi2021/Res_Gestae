...
CATILINA.BaseManager = function(gameState, Config)
{
	this.Config = Config;
...
CATILINA.BaseManager.prototype.init = function(gameState, state)
{
	if (state == "unconstructed")
		this.constructing = true;
	else if (state != "captured")
		this.neededDefenders = 0;
	this.workerObject = new CATILINA.Worker(this);
	// entitycollections
...
CATILINA.BaseManager.prototype.reset = function(gameState, state)
{
	if (state == "unconstructed")
		this.constructing = true;
	else
...
CATILINA.BaseManager.prototype.assignEntity = function(gameState, ent)
{
...
CATILINA.BaseManager.prototype.setAnchor = function(gameState, anchorEntity)
{
	if (!anchorEntity.hasClass("CivCentre"))
		API3.warn("Error: Catilina base " + this.ID + " has been assigned " + ent.templateName() + " as anchor.");
	else
	{
	...
	this.accessIndex = CATILINA.getLandAccess(gameState, anchorEntity);
	return true;
};

/* we lost our anchor. Let's reassign our units and buildings */
CATILINA.BaseManager.prototype.anchorLost = function(gameState, ent)
{
	this.anchor = undefined;
	this.anchorId = undefined;
	this.neededDefenders = 0;
...
CATILINA.BaseManager.prototype.setAnchorlessEntity = function(gameState, ent)
{
	if (!this.buildings.hasEntities())
	{
		if (!CATILINA.getBuiltEntity(gameState, ent).resourceDropsiteTypes())
			API3.warn("Error: Catilina base " + this.ID + " has been assigned " + ent.templateName() + " as origin.");
		this.accessIndex = CATILINA.getLandAccess(gameState, ent);
	}
	else if (this.accessIndex != CATILINA.getLandAccess(gameState, ent))
		API3.warn(" Error: Catilina base " + this.ID + " with access " + this.accessIndex +
		          " has been assigned " + ent.templateName() + " with access" + CATILINA.getLandAccess(gameState, ent));
 ...
 */
CATILINA.BaseManager.prototype.assignResourceToDropsite = function(gameState, dropsite)
{
	...
	if (this.ID == gameState.ai.HQ.baseManagers[0].ID)
		accessIndex = CATILINA.getLandAccess(gameState, dropsite);
...
CATILINA.BaseManager.prototype.removeDropsite = function(gameState, ent)
{
	if (!ent.id())
		return;
 ...
 */
CATILINA.BaseManager.prototype.findBestDropsiteAndLocation = function(gameState, resource)
{
	let bestResult = {
		"quality": 0,
		"pos": [0, 0]
 ...
 */
CATILINA.BaseManager.prototype.findBestDropsiteLocation = function(gameState, resource, templateName)
{
	...
	let obstructions = CATILINA.createObstructionMap(gameState, this.accessIndex, template);
	...
	for (const foundation of gameState.getOwnFoundations().toEntityArray())
		if (CATILINA.getBuiltEntity(gameState, foundation).isResourceDropsite(resource))
...
CATILINA.BaseManager.prototype.getResourceLevel = function(gameState, type, nearbyOnly = false)
{
	let count = 0;
	let check = {};
...
/*	if (nearbyOnly)
	...
	return count; //Nagasushi
};

/** check our resource levels and react accordingly */
CATILINA.BaseManager.prototype.checkResourceLevels = function(gameState, queues)
{
		...
		if (type == "food")
		{
			if (gameState.ai.HQ.canBuild(gameState, "structures/{civ}/field"))	// let's see if we need to add new farms.
			{
				...
				if (numFarms + numQueue == 0)	// starting game, rely on fruits as long as we have enough of them
				{
					if (count < 600)
					{
						queues.field.addPlan(new CATILINA.ConstructionPlan(gameState, "structures/{civ}/field", { "favoredBase": this.ID }));
				...
				else if (!gameState.ai.HQ.maxFields || numFarms + numQueue < gameState.ai.HQ.maxFields)
				{
					...
					queues.corral.addPlan(new CATILINA.ConstructionPlan(gameState, "structures/{civ}/corral", { "favoredBase": this.ID }));
				continue;
			}
		...
		if (total > 150 || total > 60 && type != "wood")
		{
			...
			if (ratio > 0.15)
			{
				...
				if (newDP.quality > 50 && gameState.ai.HQ.canBuild(gameState, newDP.templateName))
					queues.dropsites.addPlan(new CATILINA.ConstructionPlan(gameState, newDP.templateName, { "base": this.ID, "type": type }, newDP.pos));
...
CATILINA.BaseManager.prototype.addGatherRates = function(gameState, currentRates)
{
...
CATILINA.BaseManager.prototype.assignRolelessUnits = function(gameState, roleless)
{
	if (!roleless)
 ...
 */
CATILINA.BaseManager.prototype.setWorkersIdleByPriority = function(gameState)
{
 ...
 */
CATILINA.BaseManager.prototype.switchGatherer = function(gameState, from, to, number)
{
	let num = number;
	...
	return num;
};//Nagasushi, from Kiara

CATILINA.BaseManager.prototype.reassignIdleWorkers = function(gameState, idleWorkers)
{
...
//							usedPriority = true;
		...
		else if (CATILINA.isFastMoving(ent) && ent.canGather("food") && ent.canAttackClass("Animal"))
...
CATILINA.BaseManager.prototype.workersBySubrole = function(gameState, subrole)
{
...
CATILINA.BaseManager.prototype.gatherersByType = function(gameState, type)
{
 ...
 */
CATILINA.BaseManager.prototype.pickBuilders = function(gameState, workers, number)
{
 ...
 */
CATILINA.BaseManager.prototype.assignToFoundations = function(gameState, noRepair)
{
	...
	for (let target of foundations.values())
	{
		...
		if (CATILINA.isNotWorthBuilding(gameState, target))
			continue;
...
CATILINA.BaseManager.prototype.update = function(gameState, queues, events)
{
	if (this.ID == gameState.ai.HQ.baseManagers[0].ID)	// base for unaffected units
	{
		...
		if (gameState.ai.HQ.numActiveBases() > 0)
		{
			...
			{
				let bestBase = CATILINA.getBestBase(gameState, ent);
			...
			for (let ent of this.buildings.values())
			{
				...
				if (!bestBase)
				{
					if (ent.hasClass("Dock"))
						API3.warn("Catilina: dock in baseManager[0]. It may be useful to do an anchorless base for " + ent.templateName());
					continue;
				}
	...
	if (!this.anchor)   // This anchor has been destroyed, but the base may still be usable
	{
		if (!this.buildings.hasEntities())
		{
			...
			for (let ent of this.units.values())
			{
				let base = CATILINA.getBestBase(gameState, ent, false, this.ID);
		...
		for (let ent of this.buildings.values())
		{
			if (!ent.position())
				continue;
			let base = CATILINA.getBestBase(gameState, ent);
			if (base.anchor)
				reassignedBase = base;
			break;
		}
...
CATILINA.BaseManager.prototype.Serialize = function()
{
	return {
		"ID": this.ID,
...
CATILINA.BaseManager.prototype.Deserialize = function(gameState, data)
{
	for (let key in data)
		this[key] = data[key];
