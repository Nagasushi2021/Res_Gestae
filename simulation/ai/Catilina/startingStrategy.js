...
CATILINA.HQ.prototype.gameAnalysis = function(gameState)
{
	...
	let nobase = new CATILINA.BaseManager(gameState, this.Config);
 ...
 */
CATILINA.HQ.prototype.assignStartingEntities = function(gameState)
{
	for (let ent of gameState.getOwnEntities().values())
	{
		...
		if (!pos)
		{
			...
			if (ent.isGarrisonHolder() && ent.garrisoned().length)
				API3.warn("Catilina warning: support for garrisoned units inside garrisoned holders not yet implemented");
			continue;
		}
		...
		if (!bestbase)	// entity outside our territory
		{
			if (ent.hasClass("Structure") && !ent.decaying() && ent.resourceDropsiteTypes())
				...
				bestbase = CATILINA.getBestBase(gameState, ent) || this.baseManagers[0];
 ...
 */
CATILINA.HQ.prototype.regionAnalysis = function(gameState)
{
	...
	{
		API3.warn("Catilina error: it does not know how to interpret this map");
		return false;
	}
 ...
 */
CATILINA.HQ.prototype.structureAnalysis = function(gameState)
{
 ...
 */
CATILINA.HQ.prototype.buildFirstBase = function(gameState)
{
	...
	if (!total.canAfford(new API3.Resources(template.cost())))
	{
		...
		for (let ent of gameState.getOwnUnits().values())
		{
			...
			if (ent.isIdle())
				CATILINA.gatherTreasure(gameState, ent);
	...
	if (goal == "dock")
	{
		...
		gameState.ai.queues.dock.addPlan(new CATILINA.ConstructionPlan(gameState, "structures/{civ}/dock", { "sea": sea, "proximity": startingPoint[imax].pos }));
	}
	else
		gameState.ai.queues.civilCentre.addPlan(new CATILINA.ConstructionPlan(gameState, "structures/{civ}/civil_centre", { "base": -1, "resource": "wood", "proximity": startingPoint[imax].pos }));
};
 ...
 */
CATILINA.HQ.prototype.dispatchUnits = function(gameState)
{
	...
	if (allycc.length)
	{
		...
		units.filter(API3.Filters.byClasses(["Infantry+Ranged"])).forEach(ent => {
			...
			{
				if (!cc.position() || CATILINA.getLandAccess(gameState, cc) != access)
					continue;
				--num;
				--num1;
 ...
 */
CATILINA.HQ.prototype.configFirstBase = function(gameState)
{
	...
	if (!gameState.getOwnEntitiesByClass("DropsiteWood", true).hasEntities())
	{
		...
		if (newDP.quality > 40 && this.canBuild(gameState, newDP.templateName))
		{
			...
			gameState.ai.queues.dropsites.addPlan(new CATILINA.ConstructionPlan(gameState, newDP.templateName, { "base": this.baseManagers[1].ID }, newDP.pos));
		}
	}
	...
	if (this.needCorral)
	{
		...
		if (!gameState.getOwnEntitiesByClass("Corral", true).hasEntities() && this.canBuild(gameState, template))
			gameState.ai.queues.corral.addPlan(new CATILINA.ConstructionPlan(gameState, template, { "base": this.baseManagers[1].ID }));
	}
};
