 ...
 * Manage the research
 */
CATILINA.ResearchManager = function(Config)
{
	this.Config = Config;
};
 ...
 */
CATILINA.ResearchManager.prototype.checkPhase = function(gameState, queues)
{
	...
	if (catilinaRequirements && gameState.hasResearchers(nextPhaseName, true))
	{
		...
		queues.majorTech.addPlan(new CATILINA.ResearchPlan(gameState, nextPhaseName, true));
	}
};

CATILINA.ResearchManager.prototype.researchPopulationBonus = function(gameState, queues)
{
	...
	for (let tech of techs)
	{
		...
		if (tech[1]._template.modifications[0].value !== "Population/Bonus")
			continue;
		queues.minorTech.addPlan(new CATILINA.ResearchPlan(gameState, tech[0]));
		break;
	}
};

CATILINA.ResearchManager.prototype.researchTradeBonus = function(gameState, queues)
{
/*	if (queues.minorTech.hasQueuedUnits())
		return;
...
CATILINA.ResearchManager.prototype.researchTechsBandA = function(gameState, techs)
{
...
CATILINA.ResearchManager.prototype.researchTechsBandB = function(gameState, techs)
{
	let phase1 = gameState.currentPhase() === 1;
...
CATILINA.ResearchManager.prototype.researchTechsBandC = function(gameState, techs)
{
...
CATILINA.ResearchManager.prototype.researchTechsBandD = function(gameState, techs)
{
...
CATILINA.ResearchManager.prototype.researchTechsBandE = function(gameState, techs)
{
	let phase3 = gameState.currentPhase() === 3;
	let pop = gameState.getPopulation();
	let available = phase3 ? gameState.ai.queueManager.getAvailableResources(gameState) : null;
...
CATILINA.ResearchManager.prototype.update = function(gameState, queues)
{
	...
	if (techName)
	{
		if (techName.increasePriority)
		{
			...
			queues.minorTech.addPlan(new CATILINA.ResearchPlan(gameState, techName.name));
		return;
	}
 ...
 if (gameState.currentPhase() < 4)
   return;
	...
	queues.minorTech.addPlan(new CATILINA.ResearchPlan(gameState, pickRandom(techs)[0]));
};

CATILINA.ResearchManager.prototype.CostSum = function(cost)
{
	let costSum = 0;
	for (let res in cost)
		costSum += cost[res];
...
CATILINA.ResearchManager.prototype.Serialize = function()
{
	return {};
};

CATILINA.ResearchManager.prototype.Deserialize = function(data)
{
};
