...
KONOHA.KonohaBot = function(settings)
{
	...
	this.Config = new KONOHA.Config(settings.difficulty, settings.behavior);

	this.savedEvents = {};
};

KONOHA.KonohaBot.prototype = Object.create(API3.BaseAI.prototype);

KONOHA.KonohaBot.prototype.CustomInit = function(gameState)
{
	...
	else
	{
		...
		for (let i in this.Config.priorities)
			this.queues[i] = new KONOHA.Queue();

		this.queueManager = new KONOHA.QueueManager(this.Config, this.queues);
...
KONOHA.KonohaBot.prototype.OnUpdate = function(sharedScript)
{
	...
	{
		if (i == "AIMetadata")   // not used inside konoha
			continue;
	...
	{
		Engine.ProfileStart("KonohaBot bot (player " + this.player +")");
...
KONOHA.KonohaBot.prototype.Serialize = function()
{
	let savedEvents = {};
	for (let key in this.savedEvents)
	{
...
KONOHA.KonohaBot.prototype.Deserialize = function(data, sharedScript)
{
