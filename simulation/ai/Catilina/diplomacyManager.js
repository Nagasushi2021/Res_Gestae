 ...
 */
CATILINA.DiplomacyManager = function(Config)
{
	this.Config = Config;
 ...
 */
CATILINA.DiplomacyManager.prototype.init = function(gameState)
{
 ...
 */
CATILINA.DiplomacyManager.prototype.tributes = function(gameState)
{
	...
	for (let i = 1; i < gameState.sharedScript.playersData.length; ++i)
	{
			...
			else if (this.Config.chat && availableResources[res] === 0 && allyResources[res] > totalResources[res] + 600)
			{
				...
				for (let k = 0; k < mostNeeded.length; ++k)
				{
					if (mostNeeded[k].type == res && mostNeeded[k].wanted > 0)
					{
						...
						CATILINA.chatRequestTribute(gameState, res);
		...
		if (this.Config.chat)
			CATILINA.chatSentTribute(gameState, i);
		Engine.PostCommand(PlayerID, { "type": "tribute", "player": i, "amounts": tribute });
	}
};

CATILINA.DiplomacyManager.prototype.checkEvents = function(gameState, events)
{
	...
	for (let evt of events.DiplomacyChanged)
	{
		...
		else if (gameState.sharedScript.playersData[evt.player].isAlly[PlayerID] && gameState.isPlayerEnemy(evt.player))
		{
			...
			CATILINA.chatAnswerRequestDiplomacy(gameState, evt.player, "ally", response);
		}
 ...
 */
CATILINA.DiplomacyManager.prototype.lastManStandingCheck = function(gameState)
{
	...
	if (playerToTurnAgainst)
	{
		...
		{
			if (request.status === "waitingForTribute")
				CATILINA.chatAnswerRequestDiplomacy(gameState, player, request.requestType, "decline");
 ...
 */
CATILINA.DiplomacyManager.prototype.handleDiplomacyRequest = function(gameState, player, requestType)
{
	...
	CATILINA.chatAnswerRequestDiplomacy(gameState, player, requestType, response, requiredTribute);
};

CATILINA.DiplomacyManager.prototype.changePlayerDiplomacy = function(gameState, player, newDiplomaticStance)
{
	...
	if (this.Config.chat)
		CATILINA.chatNewDiplomacy(gameState, player, newDiplomaticStance);
};

CATILINA.DiplomacyManager.prototype.checkRequestedTributes = function(gameState)
{
	for (let [player, data] of this.receivedDiplomacyRequests)
		if (data.status === "waitingForTribute" && gameState.ai.elapsedTime > data.warnTime)
		{
			if (data.sentWarning)
			{
				...
				CATILINA.chatAnswerRequestDiplomacy(gameState, player, data.requestType, "decline");
			}
			else
			{
				data.sentWarning = true;
				data.warnTime = gameState.ai.elapsedTime + 60;
				CATILINA.chatAnswerRequestDiplomacy(gameState, player, data.requestType, "waitingForTribute", {
					"wanted": data.wanted,
					"type": data.type
				});
			}
 ...
 */
CATILINA.DiplomacyManager.prototype.sendDiplomacyRequest = function(gameState)
{
	let player;
	let max = 0;
	for (let i = 1; i < gameState.sharedScript.playersData.length; ++i)
	...
	CATILINA.chatNewRequestDiplomacy(gameState, player, requestType, "sendRequest");
};

CATILINA.DiplomacyManager.prototype.checkSentDiplomacyRequests = function(gameState)
{
	for (let [player, data] of this.sentDiplomacyRequests)
		...
		{
			CATILINA.chatNewRequestDiplomacy(gameState, player, data.requestType, "requestExpired");
			this.sentDiplomacyRequests.delete(player);
		}
};

CATILINA.DiplomacyManager.prototype.update = function(gameState, events)
{
...
CATILINA.DiplomacyManager.prototype.Serialize = function()
{
	return {
...
CATILINA.DiplomacyManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
