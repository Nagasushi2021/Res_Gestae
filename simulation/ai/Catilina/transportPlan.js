...
CATILINA.TransportPlan = function(gameState, units, startIndex, endIndex, endPos, ship)
{
...
CATILINA.TransportPlan.prototype.init = function(gameState)
{
...
CATILINA.TransportPlan.prototype.countFreeSlots = function()
{
	let slots = 0;
...
CATILINA.TransportPlan.prototype.countFreeSlotsOnShip = function(ship)
{
...
CATILINA.TransportPlan.prototype.assignUnitToShip = function(gameState, ent)
{
...
CATILINA.TransportPlan.prototype.assignShip = function(gameState)
{
	let pos;
...
CATILINA.TransportPlan.prototype.addUnit = function(unit, endPos)
{
...
CATILINA.TransportPlan.prototype.removeUnit = function(gameState, unit)
{
...
CATILINA.TransportPlan.prototype.releaseShip = function(ship)
{
	if (ship.getMetadata(PlayerID, "transporter") != this.ID)
	{
		API3.warn(" Catilina: try removing a transporter ship with " + ship.getMetadata(PlayerID, "transporter") +
...
CATILINA.TransportPlan.prototype.releaseAll = function()
{
...
CATILINA.TransportPlan.prototype.cancelTransport = function(gameState)
{
...
CATILINA.TransportPlan.prototype.update = function(gameState)
{
...
CATILINA.TransportPlan.prototype.onBoarding = function(gameState)
{
	let ready = true;
...
CATILINA.TransportPlan.prototype.isOnBoard = function(ent)
{
	for (let ship of this.transportShips.values())
	{
...
CATILINA.TransportPlan.prototype.getBoardingPos = function(gameState, ship, landIndex, seaIndex, destination, avoidEnnemy)
{
...
CATILINA.TransportPlan.prototype.onSailing = function(gameState)
{
	...
	for (let entId of this.unloaded)
	{
		...
		else if (!ent.position())  // unloading failed
		{
			...
			if (ship)
			{
				...
				{
					API3.warn("Catilina transportPlan problem: unit not on ship without position ???");
					this.resetUnit(gameState, ent);
					ent.destroy();
				}
			}
			...
			{
				API3.warn("Catilina transportPlan problem: unit on ship, but no ship ???");
				this.resetUnit(gameState, ent);
				ent.destroy();
			}
		}
		else if (CATILINA.getLandAccess(gameState, ent) != this.endIndex)
		{
...
CATILINA.TransportPlan.prototype.resetUnit = function(gameState, ent)
{
	ent.setMetadata(PlayerID, "transport", undefined);
	ent.setMetadata(PlayerID, "onBoard", undefined);
	ent.setMetadata(PlayerID, "endPos", undefined);
...
CATILINA.TransportPlan.prototype.Serialize = function()
{
	return {
		"ID": this.ID,
...
CATILINA.TransportPlan.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
