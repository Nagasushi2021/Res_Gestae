...
CATILINA.ConstructionPlan = function(gameState, type, metadata, position)
{
	if (!CATILINA.QueuePlan.call(this, gameState, type, metadata))
		return false;
...
CATILINA.ConstructionPlan.prototype = Object.create(CATILINA.QueuePlan.prototype);

CATILINA.ConstructionPlan.prototype.canStart = function(gameState)
{
...
CATILINA.ConstructionPlan.prototype.start = function(gameState)
{
	...
	if (!builder)
	{
		API3.warn("catilina error: builder not found when starting construction.");
		Engine.ProfileStop();
		return;
	}
...
CATILINA.ConstructionPlan.prototype.findGoodPosition = function(gameState)
{
	...
	else	// No position was specified so try and find a sensible place to build
	{
		...
		if (!HQ.requireHouses || !template.hasClass("House"))
		{
			gameState.getOwnStructures().forEach(function(ent) {
				...
				let struct = CATILINA.getBuiltEntity(gameState, ent);
	...
	if (this.metadata && this.metadata.base !== undefined)
	{
		...
		for (let j = 0; j < placement.map.length; ++j)
		{
			...
			{
				if (favorBorder && HQ.borderMap.map[j] & CATILINA.border_Mask)
					placement.set(j, placement.map[j] + 50);
				else if (disfavorBorder && !(HQ.borderMap.map[j] & CATILINA.fullBorder_Mask))
 ...
 */
CATILINA.ConstructionPlan.prototype.findDockPosition = function(gameState)
{
	let template = this.template;
	...
	for (let j = 0; j < territoryMap.length; ++j)
	{
		...
		if (oversea)
		{
			...
			{
				if (CATILINA.getSeaAccess(gameState, dock) != navalPassMap[i])
					continue;
		...
		if (gameState.ai.HQ.borderMap.map[j] & CATILINA.fullBorder_Mask)
			score += 20;
 ...
 */
CATILINA.ConstructionPlan.prototype.buildOverseaDock = function(gameState, template)
{
	...
	for (let i = 0; i < gameState.ai.accessibility.regionSize.length; ++i)
	{
		...
		for (let dock of docks.values())
		{
			if (CATILINA.getLandAccess(gameState, dock) != i)
				continue;
			keep = false;
			break;
		}
		...
		for (let cc of ccEnts.values())
		{
			let ccAccess = CATILINA.getLandAccess(gameState, cc);
			if (ccAccess != i)
			{
	...
	if (!gameState.ai.HQ.navalMap)
		API3.warn("catilina.findOverseaLand on a non-naval map??? we should never go there ");
...
CATILINA.ConstructionPlan.prototype.getDockAngle = function(gameState, x, z, size)
{
 ...
 */
CATILINA.ConstructionPlan.prototype.checkDockPlacement = function(gameState, x, z, halfDepth, halfWidth, angle)
{
 ...
 */
CATILINA.ConstructionPlan.prototype.isDockLocation = function(gameState, j, dimension, wantedLand, wantedSea)
{
 ...
 */
CATILINA.ConstructionPlan.prototype.getFrontierProximity = function(gameState, j)
{
	...
	for (let a of around)
	{
		for (let i = 1; i < 5; ++i)
		{
			...
			if (jz < 0 || jz >= width)
				continue;
			if (borderMap.map[jx+width*jz] & CATILINA.outside_Mask)
				continue;
 ...
 */
CATILINA.ConstructionPlan.prototype.getResourcesAround = function(gameState, types, i, radius)
{
...
CATILINA.ConstructionPlan.prototype.isGo = function(gameState)
{
...
CATILINA.ConstructionPlan.prototype.onStart = function(gameState)
{
...
CATILINA.ConstructionPlan.prototype.Serialize = function()
{
	return {
...
CATILINA.ConstructionPlan.prototype.Deserialize = function(gameState, data)
{
	for (let key in data)
		this[key] = data[key];
