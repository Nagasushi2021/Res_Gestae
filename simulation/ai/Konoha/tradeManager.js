 ...
 * Manage the trade
 */
KONOHA.TradeManager = function(Config)
{
	this.Config = Config;
...
KONOHA.TradeManager.prototype.init = function(gameState)
{
...
KONOHA.TradeManager.prototype.hasTradeRoute = function()
{
	return this.tradeRoute !== undefined;
};

KONOHA.TradeManager.prototype.assignTrader = function(ent)
{
	ent.setMetadata(PlayerID, "role", "trader");
	this.traders.updateEnt(ent);
};

KONOHA.TradeManager.prototype.trainMoreTraders = function(gameState, queues)
{
	...
	if (!gameState.getTemplate(template))
	{
		if (this.Config.debug > 0)
			API3.warn("Konoha error: trying to train " + template + " for civ " +
			          gameState.getPlayerCiv() + " but no template found.");
		return;
	}
	queues.trader.addPlan(new KONOHA.TrainingPlan(gameState, template, metadata, 1, 1));
};

KONOHA.TradeManager.prototype.updateTrader = function(gameState, ent)
{
	if (ent.hasClass("Ship") && gameState.ai.playedTurn % 5 == 0 &&
	    !ent.unitAIState().startsWith("INDIVIDUAL.COLLECTTREASURE") &&
	    KONOHA.gatherTreasure(gameState, ent, true))
		return;
	...
	let access = ent.hasClass("Ship") ? KONOHA.getSeaAccess(gameState, ent) : KONOHA.getLandAccess(gameState, ent);
...
KONOHA.TradeManager.prototype.setTradingGoods = function(gameState)
{
 ...
 */
KONOHA.TradeManager.prototype.performBarter = function(gameState)
{
...
KONOHA.TradeManager.prototype.checkEvents = function(gameState, events)
{
...
KONOHA.TradeManager.prototype.activateProspection = function(gameState)
{
 ...
 */
KONOHA.TradeManager.prototype.checkRoutes = function(gameState, accessIndex)
{
	...
	for (let m1 of market1.values())
	{
		if (!m1.position())
			continue;
		let access1 = KONOHA.getLandAccess(gameState, m1);
		let sea1 = m1.hasClass("Naval") ? KONOHA.getSeaAccess(gameState, m1) : undefined;
		for (let m2 of market2.values())
		{
			...
			if (!m2.position())
				continue;
			let access2 = KONOHA.getLandAccess(gameState, m2);
			let sea2 = m2.hasClass("Naval") ? KONOHA.getSeaAccess(gameState, m2) : undefined;
			let land = access1 == access2 ? access1 : undefined;
			let sea = sea1 && sea1 == sea2 ? sea1 : undefined;
			if (!land && !sea)
				continue;
			if (land && KONOHA.isLineInsideEnemyTerritory(gameState, m1.position(), m2.position()))
				continue;
			let gainMultiplier;
	...
	if (this.Config.chat)
	{
		...
		{	// Warn an ally that we have a trade route with him
			KONOHA.chatNewTradeRoute(gameState, owner);
...
KONOHA.TradeManager.prototype.checkTrader = function(gameState, ent)
{
	    ...
	    possibleRoute.target.id() != presentRoute.source && possibleRoute.target.id() != presentRoute.target)
	{
		...
		{
			let closestBase = KONOHA.getBestBase(gameState, ent, true);
...
KONOHA.TradeManager.prototype.prospectForNewMarket = function(gameState, queues)
{
...
KONOHA.TradeManager.prototype.isNewMarketWorth = function(expectedGain)
{
	if (!Resources.GetTradableCodes().length)
		return false;
...
KONOHA.TradeManager.prototype.update = function(gameState, events, queues)
{
...
KONOHA.TradeManager.prototype.routeEntToId = function(route)
{
	if (!route)
		return undefined;
...
KONOHA.TradeManager.prototype.routeIdToEnt = function(gameState, route)
{
	if (!route)
		return undefined;
...
KONOHA.TradeManager.prototype.Serialize = function()
{
	return {
...
KONOHA.TradeManager.prototype.Deserialize = function(gameState, data)
{
	for (let key in data)
	{
