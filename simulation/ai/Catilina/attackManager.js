 ...
 * Attack Manager
 */
CATILINA.AttackManager = function(Config)
{
	this.Config = Config;
	...
	this.upcomingAttacks = { "Rush": [], "Raid": [], "Attack": [], "HugeAttack": [], "Reinforcement": [] };
	this.startedAttacks = { "Rush": [], "Raid": [], "Attack": [], "HugeAttack": [], "Reinforcement": [] };
...
CATILINA.AttackManager.prototype.checkEvents = function(gameState, events)
{
	...
	if (targetPlayer !== undefined)
		CATILINA.chatAnswerRequestAttack(gameState, targetPlayer, answer, other);
 ...
 */
CATILINA.AttackManager.prototype.assignBombers = function(gameState)
{
	...
	const bombers = gameState.updatingCollection("bombers", API3.Filters.byClasses(["StoneThrower"]), gameState.getOwnUnits());
	for (let ent of bombers.values())
	{
		...
		{
			if (!ent.canAttackTarget(struct, CATILINA.allowCapture(gameState, ent, struct)))
				continue;
 ...
 */
CATILINA.AttackManager.prototype.update = function(gameState, queues, events)
{
	...
	let unexecutedAttacks = { "Rush": 0, "Raid": 0, "Attack": 0, "HugeAttack": 0, "Reinforcement": 0};
	for (let attackType in this.upcomingAttacks)
	{
		for (let i = 0; i < this.upcomingAttacks[attackType].length; ++i)
		{
			...
			if (attack.isStarted())
				API3.warn("Catilina problem in attackManager: attack in preparation has already started ???");
			...
			else if (updateStep == 2)
			{
				if (attack.StartAttack(gameState))
				{
					...
					if (this.Config.chat)
						CATILINA.chatLaunchAttack(gameState, attack.targetPlayer, attack.getType());
	...
	if (this.rushNumber < this.maxRushes && gameState.getPopulation() > 12 /*barracksNb >= 1*/)
	{
		if (unexecutedAttacks.Rush === 0)
		{
			// we have a barracks and we want to rush, rush.
			let data = { "targetSize": this.rushSize[this.rushNumber] };
			let attackPlan = new CATILINA.AttackPlan(gameState, this.Config, this.totalNumber, "Rush", data);
	...
	else if (gameState.getPopulation() > this.Config.Economy.targetNumWorkers && unexecutedAttacks.Attack == 0 && unexecutedAttacks.HugeAttack == 0 &&
		...
		(this.startedAttacks.Attack.length + this.startedAttacks.HugeAttack.length == 0 || gameState.getPopulationMax() - gameState.getPopulation() > 12))
	{
		...
		{
			let type = this.attackNumber < 2 || this.attackNumber >= 2 ? "Attack" : "HugeAttack";
			let attackPlan = new CATILINA.AttackPlan(gameState, this.Config, this.totalNumber, type);
			if (attackPlan.failed)
		 ...
		 unexecutedAttacks.Reinforcement == 0 &&
		 this.startedAttacks.Attack.length + this.startedAttacks.HugeAttack.length > 0 &&
		 gameState.getPopulation() > gameState.getPopulationMax()/2)
	{
		if (barracksNb >= 1 && (gameState.currentPhase() > 1 || gameState.isResearching(gameState.getPhaseName(2))) ||
			!gameState.ai.HQ.baseManagers[1])	// if we have no base ... nothing else to do than attack
		{
			let type = "Reinforcement";
			let attackPlan = new CATILINA.AttackPlan(gameState, this.Config, this.totalNumber, type);
			if (attackPlan.failed)
				this.attackPlansEncounteredWater = true; // hack
			else
...
CATILINA.AttackManager.prototype.getPlan = function(planName)
{
	for (let attackType in this.upcomingAttacks)
	{
		for (let attack of this.upcomingAttacks[attackType])
...
CATILINA.AttackManager.prototype.pausePlan = function(planName)
{
...
CATILINA.AttackManager.prototype.unpausePlan = function(planName)
{
	let attack = this.getPlan(planName);
	if (attack)
...
CATILINA.AttackManager.prototype.pauseAllPlans = function()
{
	for (let attackType in this.upcomingAttacks)
		for (let attack of this.upcomingAttacks[attackType])
			attack.setPaused(true);
...
CATILINA.AttackManager.prototype.unpauseAllPlans = function()
{
	for (let attackType in this.upcomingAttacks)
		for (let attack of this.upcomingAttacks[attackType])
			attack.setPaused(false);
...
CATILINA.AttackManager.prototype.getAttackInPreparation = function(type)
{
 ...
 * for future attacks.
 */
CATILINA.AttackManager.prototype.getEnemyPlayer = function(gameState, attack)
{
	let enemyPlayer;
	...
	if (attack.type != "HugeAttack")
	{
		...
		for (let ourcc of ccEnts.values())
		{
			...
			let access = CATILINA.getLandAccess(gameState, ourcc);
			for (let enemycc of ccEnts.values())
			{
				...
				if (!gameState.isPlayerEnemy(enemycc.owner()))
					continue;
				if (access != CATILINA.getLandAccess(gameState, enemycc))
					continue;
 ...
 */
CATILINA.AttackManager.prototype.getWonderEnemyPlayer = function(gameState, attack)
{
	let enemyPlayer;
	let enemyWonder;
	let moreAdvanced;
 ...
 */
CATILINA.AttackManager.prototype.getRelicEnemyPlayer = function(gameState, attack)
{
	let enemyPlayer;
...
CATILINA.AttackManager.prototype.cancelAttacksAgainstPlayer = function(gameState, player)
{
	for (let attackType in this.upcomingAttacks)
		for (let attack of this.upcomingAttacks[attackType])
...
CATILINA.AttackManager.prototype.raidTargetEntity = function(gameState, ent)
{
	let data = { "target": ent };
	let attackPlan = new CATILINA.AttackPlan(gameState, this.Config, this.totalNumber, "Raid", data);
	if (attackPlan.failed)
		return null;
	if (this.Config.debug > 1)
 ...
 */
CATILINA.AttackManager.prototype.numAttackingUnitsAround = function(pos, dist)
{
	let num = 0;
	for (let attackType in this.startedAttacks)
		for (let attack of this.startedAttacks[attackType])
 ...
 */
CATILINA.AttackManager.prototype.switchDefenseToAttack = function(gameState, target, data)
{
	...
	let attackPlan = new CATILINA.AttackPlan(gameState, this.Config, this.totalNumber, attackType, attackData);
	if (attackPlan.failed)
		return false;
	this.totalNumber++;
	attackPlan.init(gameState);
	...
	let targetAccess = CATILINA.getLandAccess(gameState, target);
	for (let army of gameState.ai.HQ.defenseManager.armies)
	{
		...
		while (army.ownEntities.length > 0)
		{
			...
			let accessOk = unit.getMetadata(PlayerID, "transport") !== undefined ||
			               unit.position() && CATILINA.getLandAccess(gameState, unit) == targetAccess;
...
CATILINA.AttackManager.prototype.Serialize = function()
{
	let properties = {
...
CATILINA.AttackManager.prototype.Deserialize = function(gameState, data)
{
	...
	for (let key in data.upcomingAttacks)
	{
		...
		{
			let attack = new CATILINA.AttackPlan(gameState, this.Config, dataAttack.properties.name);
