CATILINA.DefenseManager = function(Config)
{
...
CATILINA.DefenseManager.prototype.update = function(gameState, events)
{
...
CATILINA.DefenseManager.prototype.makeIntoArmy = function(gameState, entityID, type = "default")
{
	if (type == "default")
	{
	...
	let army = new CATILINA.DefenseArmy(gameState, [entityID], type);

	this.armies.push(army);
};

CATILINA.DefenseManager.prototype.getArmy = function(partOfArmy)
{
...
CATILINA.DefenseManager.prototype.isDangerous = function(gameState, entity)
{
	if (!entity.position())
		return false;
...
CATILINA.DefenseManager.prototype.checkEnemyUnits = function(gameState)
{
...
CATILINA.DefenseManager.prototype.checkEnemyArmies = function(gameState)
{
...
CATILINA.DefenseManager.prototype.assignDefenders = function(gameState)
{
	if (!this.armies.length)
		return;
	...
	for (let army of this.armies)
	{
		...
		for (let entId of army.foeEntities)
		{
			...
			if (!ent || !ent.position())
				continue;
			armyAccess = CATILINA.getLandAccess(gameState, ent);
			break;
		}
		if (!armyAccess)
			API3.warn(" Catilina error: attacking army " + army.ID + " without access");
		army.recalculatePosition(gameState);
	...
	for (let ipass = 0; ipass < 2; ++ipass)
	{
		...
		for (let i = 0; i < potentialDefenders.length; ++i)
		{
			...
			let aMin;
			let distMin;
			let access = ipass == 0 ? CATILINA.getLandAccess(gameState, ent) : undefined;
			for (let a = 0; a < armiesNeeding.length; ++a)
			{
				...
				if (!armiesNeeding[a].army.foeEntities.some(eEnt => {
					...
					return ent.canAttackTarget(eEntID, CATILINA.allowCapture(gameState, ent, eEntID));
					}))
					continue;
			...
			armiesNeeding[aMin].need -= CATILINA.getMaxStrength(ent, this.Config.debug, this.Config.DamageTypeImportance);
...
CATILINA.DefenseManager.prototype.abortArmy = function(gameState, army)
{
	army.clear(gameState);
	for (let i = 0; i < this.armies.length; ++i)
	{
 ...
 */
CATILINA.DefenseManager.prototype.checkEvents = function(gameState, events)
{
		...
		if (target.hasClass("Unit") && attacker.hasClass("Unit"))
		{
			...
			if (currentTarget)
			{
				...
				if (unitAIStateOrder == "REPAIR" && currentTarget.hasDefensiveFire())
					continue;
				if (unitAIStateOrder == "COMBAT" && !CATILINA.isSiegeUnit(currentTarget) &&
				    gameState.ai.HQ.capturableTargets.has(orderData[0].target))
				{
					...
					for (let entId of capturableTarget.ents)
					{
						if (allAttacked[entId])
							continue;
						let ent = gameState.getEntityById(entId);
						if (!ent || !ent.position() || !ent.canAttackTarget(attacker, CATILINA.allowCapture(gameState, ent, attacker)))
							continue;
					...
					if (minEnt)
					{
						...
						minEnt.attack(attacker.id(), CATILINA.allowCapture(gameState, minEnt, attacker));
					}
				}
			}
			let allowCapture = CATILINA.allowCapture(gameState, target, attacker);
...
CATILINA.DefenseManager.prototype.garrisonUnitsInside = function(gameState, target, data)
{
	...
	let access = CATILINA.getLandAccess(gameState, target);
	...
	if (allowMelee === undefined)
	{
		...
		if (data.attacker)
			allowMelee = data.attacker.hasClass("Structure") ? data.attacker.attackRange("Ranged") : !CATILINA.isSiegeUnit(data.attacker);
		else
			allowMelee = true;
	}
	let units = gameState.getOwnUnits().filter(ent => {
		...
		if (CATILINA.getLandAccess(gameState, ent) != access)
			return false;
		return true;
...
CATILINA.DefenseManager.prototype.garrisonSiegeUnit = function(gameState, unit)
{
	let distmin = Math.min();
	let nearest;
	let unitAccess = CATILINA.getLandAccess(gameState, unit);
	let garrisonManager = gameState.ai.HQ.garrisonManager;
	for (let ent of gameState.getAllyStructures().values())
	{
		...
		if (ent.hitpoints() < ent.garrisonEjectHealth() * ent.maxHitpoints())
			continue;
		if (CATILINA.getLandAccess(gameState, ent) != unitAccess)
			continue;
 ...
 */
CATILINA.DefenseManager.prototype.garrisonAttackedUnit = function(gameState, unit, emergency = false)
{
	let distmin = Math.min();
	let nearest;
	let unitAccess = CATILINA.getLandAccess(gameState, unit);
 ...
 */
CATILINA.DefenseManager.prototype.GetCooperationLevel = function(ally)
{
 ...
 */
CATILINA.DefenseManager.prototype.switchToAttack = function(gameState, army)
{
	if (!army)
		return;
	for (let targetId of this.targetList)
	{
		...
		{
			let ent = gameState.getEntityById(entId);
			if (!ent || !ent.position() || CATILINA.getLandAccess(gameState, ent) != targetAccess)
				continue;
...
CATILINA.DefenseManager.prototype.Serialize = function()
{
	let properties = {
...
CATILINA.DefenseManager.prototype.Deserialize = function(gameState, data)
{
	...
	{
		let army = new CATILINA.DefenseArmy(gameState, []);
