 ...
 */
CATILINA.DefenseArmy = function(gameState, foeEntities, type)
{
 ...
 */
CATILINA.DefenseArmy.prototype.addFoe = function(gameState, enemyId, force)
{
 ...
 */
CATILINA.DefenseArmy.prototype.removeFoe = function(gameState, enemyId, enemyEntity)
{
 ...
 */
CATILINA.DefenseArmy.prototype.addOwn = function(gameState, id, force)
{
	...
	ent.setMetadata(PlayerID, "subrole", "defender");
  	ent.setStance("aggressive"); //Nagasushi
	return true;
};

CATILINA.DefenseArmy.prototype.removeOwn = function(gameState, id, Entity)
{
 ...
 */
CATILINA.DefenseArmy.prototype.clear = function(gameState)
{
	...
	else
	{
		...
		for (let struct of gameState.getAllyStructures().values())
		{
			...
			if (!pos || !gameState.isPlayerMutualAlly(gameState.ai.HQ.territoryMap.getOwner(pos)))
				continue;
			if (CATILINA.getLandAccess(gameState, struct) !== armyAccess)
				continue;
...
CATILINA.DefenseArmy.prototype.assignUnit = function(gameState, entID)
{
	...
	for (let id of this.foeEntities)
	{
		...
		if (!ent.canAttackTarget(eEnt, CATILINA.allowCapture(gameState, ent, eEnt)))
			continue;
		...
		if (this.assignedAgainst[id].length > 8 ||
			this.assignedAgainst[id].length > 5 && !eEnt.hasClass("Hero") && !CATILINA.isSiegeUnit(eEnt))
			continue;
	...
	let ownIndex = CATILINA.getLandAccess(gameState, ent);
	...
	if (ownIndex == foeIndex || ent.hasClass("Ship"))
	{
		...
		ent.attack(idFoe, CATILINA.allowCapture(gameState, ent, foeEnt), queued);
	}
	else
...
CATILINA.DefenseArmy.prototype.getType = function()
{
	return this.type;
};

CATILINA.DefenseArmy.prototype.getState = function()
{
 ...
 */
CATILINA.DefenseArmy.prototype.merge = function(gameState, otherArmy)
{
...
CATILINA.DefenseArmy.prototype.needsDefenders = function(gameState)
{
	let defenseRatio;
...
CATILINA.DefenseArmy.prototype.recalculatePosition = function(gameState, force)
{
...
CATILINA.DefenseArmy.prototype.recalculateStrengths = function(gameState)
{
	this.ownStrength = 0;
	this.foeStrength = 0;
...
CATILINA.DefenseArmy.prototype.evaluateStrength = function(ent, isOwn, remove)
{
	if (!ent)
		return;
	...
	if (ent.hasClass("Structure"))
	{
		...
		entStrength = CATILINA.getMaxStrength(ent, this.Config.debug, this.Config.DamageTypeImportance);
...
CATILINA.DefenseArmy.prototype.checkEvents = function(gameState, events)
{
...
CATILINA.DefenseArmy.prototype.update = function(gameState)
{
	for (let entId of this.ownEntities)
	{
		let ent = gameState.getEntityById(entId);
		...
		else if (orderData.length && orderData[0].target && orderData[0].attackType && orderData[0].attackType === "Capture")
		{
			...
			if (target && !CATILINA.allowCapture(gameState, ent, target))
...
CATILINA.DefenseArmy.prototype.Serialize = function()
{
	return {
		"ID": this.ID,
		"type": this.type,
...
CATILINA.DefenseArmy.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
