...
CATILINA.CatilinaBot = function(settings)
{
	...
	this.Config = new CATILINA.Config(settings.difficulty, settings.behavior);

	this.savedEvents = {};
};

CATILINA.CatilinaBot.prototype = Object.create(API3.BaseAI.prototype);

CATILINA.CatilinaBot.prototype.CustomInit = function(gameState)
{
	...
	else
	{
		...
		for (let i in this.Config.priorities)
			this.queues[i] = new CATILINA.Queue();

		this.queueManager = new CATILINA.QueueManager(this.Config, this.queues);
...
CATILINA.CatilinaBot.prototype.OnUpdate = function(sharedScript)
{
	...
	{
		if (i == "AIMetadata")   // not used inside catilina
			continue;
	...
	{
		Engine.ProfileStart("CatilinaBot bot (player " + this.player +")");
...
CATILINA.CatilinaBot.prototype.Serialize = function()
{
	let savedEvents = {};
	for (let key in this.savedEvents)
	{
...
CATILINA.CatilinaBot.prototype.Deserialize = function(data, sharedScript)
{
