...
KONOHA.BaseManager = function(gameState, Config)
{
	this.Config = Config;
...
KONOHA.BaseManager.prototype.init = function(gameState, state)
{
	if (state == "unconstructed")
		this.constructing = true;
	else if (state != "captured")
		this.neededDefenders = 0;
	this.workerObject = new KONOHA.Worker(this);
	// entitycollections
...
KONOHA.BaseManager.prototype.reset = function(gameState, state)
{
	if (state == "unconstructed")
		this.constructing = true;
	else
...
KONOHA.BaseManager.prototype.assignEntity = function(gameState, ent)
{
...
KONOHA.BaseManager.prototype.setAnchor = function(gameState, anchorEntity)
{
	if (!anchorEntity.hasClass("CivCentre"))
		API3.warn("Error: Konoha base " + this.ID + " has been assigned " + ent.templateName() + " as anchor.");
	else
	{
	...
	this.accessIndex = KONOHA.getLandAccess(gameState, anchorEntity);
	return true;
};

/* we lost our anchor. Let's reassign our units and buildings */
KONOHA.BaseManager.prototype.anchorLost = function(gameState, ent)
{
	this.anchor = undefined;
	this.anchorId = undefined;
	this.neededDefenders = 0;
...
KONOHA.BaseManager.prototype.setAnchorlessEntity = function(gameState, ent)
{
	if (!this.buildings.hasEntities())
	{
		if (!KONOHA.getBuiltEntity(gameState, ent).resourceDropsiteTypes())
			API3.warn("Error: Konoha base " + this.ID + " has been assigned " + ent.templateName() + " as origin.");
		this.accessIndex = KONOHA.getLandAccess(gameState, ent);
	}
	else if (this.accessIndex != KONOHA.getLandAccess(gameState, ent))
		API3.warn(" Error: Konoha base " + this.ID + " with access " + this.accessIndex +
		          " has been assigned " + ent.templateName() + " with access" + KONOHA.getLandAccess(gameState, ent));
 ...
 */
KONOHA.BaseManager.prototype.assignResourceToDropsite = function(gameState, dropsite)
{
	...
	if (this.ID == gameState.ai.HQ.baseManagers[0].ID)
		accessIndex = KONOHA.getLandAccess(gameState, dropsite);
	...
	for (let type of dropsite.resourceDropsiteTypes())
	{
		...
		resources.forEach(function(supply)
		{
			...
			if (KONOHA.getLandAccess(gameState, supply) != accessIndex)
				return;
...
KONOHA.BaseManager.prototype.removeDropsite = function(gameState, ent)
{
	if (!ent.id())
		return;
 ...
 */
KONOHA.BaseManager.prototype.findBestDropsiteAndLocation = function(gameState, resource)
{
	let bestResult = {
		"quality": 0,
		"pos": [0, 0]
 ...
 */
KONOHA.BaseManager.prototype.findBestDropsiteLocation = function(gameState, resource, templateName)
{
	...
	let obstructions = KONOHA.createObstructionMap(gameState, this.accessIndex, template);
	...
	for (const foundation of gameState.getOwnFoundations().toEntityArray())
		if (KONOHA.getBuiltEntity(gameState, foundation).isResourceDropsite(resource))
...
KONOHA.BaseManager.prototype.getResourceLevel = function(gameState, type, nearbyOnly = false)
{
	let count = 0;
	let check = {};
...
KONOHA.BaseManager.prototype.checkResourceLevels = function(gameState, queues)
{
		...
		if (type == "food")
		{
			if (gameState.ai.HQ.canBuild(gameState, "structures/{civ}/field"))	// let's see if we need to add new farms.
			{
				...
				if (numFarms + numQueue == 0)	// starting game, rely on fruits as long as we have enough of them
				{
					if (count < 600)
					{
						queues.field.addPlan(new KONOHA.ConstructionPlan(gameState, "structures/{civ}/field", { "favoredBase": this.ID }));
				...
				else if (!gameState.ai.HQ.maxFields || numFarms + numQueue < gameState.ai.HQ.maxFields)
				{
					...
					queues.corral.addPlan(new KONOHA.ConstructionPlan(gameState, "structures/{civ}/corral", { "favoredBase": this.ID }));
				continue;
			}
		...
		if (total > 150 || total > 60 && type != "wood")
		{
			...
			if (ratio > 0.15)
			{
				...
				if (newDP.quality > 50 && gameState.ai.HQ.canBuild(gameState, newDP.templateName))
					queues.dropsites.addPlan(new KONOHA.ConstructionPlan(gameState, newDP.templateName, { "base": this.ID, "type": type }, newDP.pos));
...
KONOHA.BaseManager.prototype.addGatherRates = function(gameState, currentRates)
{
...
KONOHA.BaseManager.prototype.assignRolelessUnits = function(gameState, roleless)
{
	if (!roleless)
 ...
 */
KONOHA.BaseManager.prototype.setWorkersIdleByPriority = function(gameState)
{
 ...
 */
KONOHA.BaseManager.prototype.switchGatherer = function(gameState, from, to, number)
{
	let num = number;
...
KONOHA.BaseManager.prototype.reassignIdleWorkers = function(gameState, idleWorkers)
{
...
//							usedPriority = true;
		...
		else if (KONOHA.isFastMoving(ent) && ent.canGather("food") && ent.canAttackClass("Animal"))
...
KONOHA.BaseManager.prototype.workersBySubrole = function(gameState, subrole)
{
...
KONOHA.BaseManager.prototype.gatherersByType = function(gameState, type)
{
 ...
 */
KONOHA.BaseManager.prototype.pickBuilders = function(gameState, workers, number)
{
 ...
 */
KONOHA.BaseManager.prototype.assignToFoundations = function(gameState, noRepair)
{
	...
	for (let target of foundations.values())
	{
		...
		if (KONOHA.isNotWorthBuilding(gameState, target))
			continue;
...
KONOHA.BaseManager.prototype.update = function(gameState, queues, events)
{
	if (this.ID == gameState.ai.HQ.baseManagers[0].ID)	// base for unaffected units
	{
		...
		if (gameState.ai.HQ.numActiveBases() > 0)
		{
			...
			{
				let bestBase = KONOHA.getBestBase(gameState, ent);
			...
			for (let ent of this.buildings.values())
			{
				...
				if (!bestBase)
				{
					if (ent.hasClass("Dock"))
						API3.warn("Konoha: dock in baseManager[0]. It may be useful to do an anchorless base for " + ent.templateName());
					continue;
				}
	...
	if (!this.anchor)   // This anchor has been destroyed, but the base may still be usable
	{
		if (!this.buildings.hasEntities())
		{
			...
			for (let ent of this.units.values())
			{
				let base = KONOHA.getBestBase(gameState, ent, false, this.ID);
		...
		for (let ent of this.buildings.values())
		{
			if (!ent.position())
				continue;
			let base = KONOHA.getBestBase(gameState, ent);
			if (base.anchor)
				reassignedBase = base;
			break;
		}
...
KONOHA.BaseManager.prototype.Serialize = function()
{
	return {
		"ID": this.ID,
...
KONOHA.BaseManager.prototype.Deserialize = function(gameState, data)
{
	for (let key in data)
		this[key] = data[key];
