...
KONOHA.Queue.prototype.empty = function()
{
	this.plans = [];
};

KONOHA.Queue.prototype.addPlan = function(newPlan)
{
	if (!newPlan)
		return;
...
KONOHA.Queue.prototype.check= function(gameState)
{
...
KONOHA.Queue.prototype.getNext = function()
{
...
KONOHA.Queue.prototype.startNext = function(gameState)
{
	if (this.plans.length > 0)
	{
 ...
 */
KONOHA.Queue.prototype.maxAccountWanted = function(gameState, fraction)
{
...
KONOHA.Queue.prototype.queueCost = function()
{
	let cost = new API3.Resources();
	for (let plan of this.plans)
...
KONOHA.Queue.prototype.length = function()
{
	return this.plans.length;
};

KONOHA.Queue.prototype.hasQueuedUnits = function()
{
	return this.plans.length > 0;
};

KONOHA.Queue.prototype.countQueuedUnits = function()
{
	let count = 0;
	for (let plan of this.plans)
		count += plan.number;
...
KONOHA.Queue.prototype.hasQueuedUnitsWithClass = function(classe)
{
	return this.plans.some(plan => plan.template && plan.template.hasClass(classe));
};

KONOHA.Queue.prototype.countQueuedUnitsWithClass = function(classe)
{
	let count = 0;
	for (let plan of this.plans)
...
KONOHA.Queue.prototype.countQueuedUnitsWithMetadata = function(data, value)
{
	let count = 0;
	for (let plan of this.plans)
...
KONOHA.Queue.prototype.Serialize = function()
{
	let plans = [];
	for (let plan of this.plans)
...
KONOHA.Queue.prototype.Deserialize = function(gameState, data)
{
	...
	for (let dataPlan of data.plans)
	{
		...
		if (dataPlan.category == "unit")
			plan = new KONOHA.TrainingPlan(gameState, dataPlan.type);
		else if (dataPlan.category == "building")
			plan = new KONOHA.ConstructionPlan(gameState, dataPlan.type);
		else if (dataPlan.category == "technology")
			plan = new KONOHA.ResearchPlan(gameState, dataPlan.type);
		else
		{
			API3.warn("Konoha deserialization error: plan unknown " + uneval(dataPlan));
			continue;
		}
