...
CATILINA.VictoryManager = function(Config)
{
	this.Config = Config;
 ...
 */
CATILINA.VictoryManager.prototype.init = function(gameState)
{
 ...
 */
CATILINA.VictoryManager.prototype.checkEvents = function(gameState, events)
{
	if (gameState.getVictoryConditions().has("wonder"))
	{
...
CATILINA.VictoryManager.prototype.removeCriticalEnt = function(gameState, criticalEntId)
{
 ...
 */
CATILINA.VictoryManager.prototype.manageCriticalEntHealers = function(gameState, queues)
{
	...
	for (let data of this.criticalEnts.values())
	{
		...
		queues.healer.addPlan(new CATILINA.TrainingPlan(gameState, template, { "role": "criticalEntHealer", "base": 0 }, 1, 1));
		return;
	}
};
 ...
 */
CATILINA.VictoryManager.prototype.manageCriticalEntGuards = function(gameState)
{
...
CATILINA.VictoryManager.prototype.tryAssignMilitaryGuard = function(gameState, guardEnt, criticalEnt, checkForSameAccess)
{
	if (guardEnt.getMetadata(PlayerID, "plan") !== undefined ||
	    ...
	    CATILINA.getLandAccess(gameState, criticalEnt) != CATILINA.getLandAccess(gameState, guardEnt)))
		return false;
...
CATILINA.VictoryManager.prototype.pickCriticalEntRetreatLocation = function(gameState, criticalEnt, emergency)
{
	...
	let accessIndex = CATILINA.getLandAccess(gameState, criticalEnt);
	let bestBase = CATILINA.getBestBase(gameState, criticalEnt, true);
 ...
 */
CATILINA.VictoryManager.prototype.assignGuardToCriticalEnt = function(gameState, guardEnt, criticalEntId)
{
	...
	let guardEntAccess = CATILINA.getLandAccess(gameState, guardEnt);
	let criticalEntAccess = CATILINA.getLandAccess(gameState, criticalEnt);
	if (guardEntAccess == criticalEntAccess)
	{
		let queued = CATILINA.returnResources(gameState, guardEnt);
...
CATILINA.VictoryManager.prototype.resetCaptureGaiaRelic = function(gameState)
{
...
CATILINA.VictoryManager.prototype.update = function(gameState, events, queues)
{
 ...
 */
CATILINA.VictoryManager.prototype.captureGaiaRelic = function(gameState, relic)
{
	...
	for (let plan of plans)
	{
		...
		for (let ent of attack.unitCollection.values())
			capture += ent.captureStrength() * CATILINA.getAttackBonus(ent, relic, "Capture");
	}
	// No need to make a new attack if already enough units
	if (capture > sumCapturePoints / 50)
		return;
	let relicPosition = relic.position();
	let access = CATILINA.getLandAccess(gameState, relic);
...
CATILINA.VictoryManager.prototype.abortCaptureGaiaRelic = function(gameState, relicId)
{
...
CATILINA.VictoryManager.prototype.Serialize = function()
{
	return {
...
CATILINA.VictoryManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
