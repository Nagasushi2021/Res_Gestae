...
KONOHA.TransportPlan = function(gameState, units, startIndex, endIndex, endPos, ship)
{
...
KONOHA.TransportPlan.prototype.init = function(gameState)
{
...
KONOHA.TransportPlan.prototype.countFreeSlots = function()
{
	let slots = 0;
...
KONOHA.TransportPlan.prototype.countFreeSlotsOnShip = function(ship)
{
...
KONOHA.TransportPlan.prototype.assignUnitToShip = function(gameState, ent)
{
...
KONOHA.TransportPlan.prototype.assignShip = function(gameState)
{
	let pos;
...
KONOHA.TransportPlan.prototype.addUnit = function(unit, endPos)
{
...
KONOHA.TransportPlan.prototype.removeUnit = function(gameState, unit)
{
...
KONOHA.TransportPlan.prototype.releaseShip = function(ship)
{
	if (ship.getMetadata(PlayerID, "transporter") != this.ID)
	{
		API3.warn(" Konoha: try removing a transporter ship with " + ship.getMetadata(PlayerID, "transporter") +
...
KONOHA.TransportPlan.prototype.releaseAll = function()
{
...
KONOHA.TransportPlan.prototype.cancelTransport = function(gameState)
{
...
KONOHA.TransportPlan.prototype.update = function(gameState)
{
...
KONOHA.TransportPlan.prototype.onBoarding = function(gameState)
{
	let ready = true;
...
KONOHA.TransportPlan.prototype.isOnBoard = function(ent)
{
	for (let ship of this.transportShips.values())
	{
...
KONOHA.TransportPlan.prototype.getBoardingPos = function(gameState, ship, landIndex, seaIndex, destination, avoidEnnemy)
{
...
KONOHA.TransportPlan.prototype.onSailing = function(gameState)
{
	...
	for (let entId of this.unloaded)
	{
		...
		else if (!ent.position())  // unloading failed
		{
			...
			if (ship)
			{
				...
				{
					API3.warn("Konoha transportPlan problem: unit not on ship without position ???");
					this.resetUnit(gameState, ent);
					ent.destroy();
				}
			}
			...
			{
				API3.warn("Konoha transportPlan problem: unit on ship, but no ship ???");
				this.resetUnit(gameState, ent);
				ent.destroy();
			}
		}
		else if (KONOHA.getLandAccess(gameState, ent) != this.endIndex)
		{
...
KONOHA.TransportPlan.prototype.resetUnit = function(gameState, ent)
{
	ent.setMetadata(PlayerID, "transport", undefined);
	ent.setMetadata(PlayerID, "onBoard", undefined);
	ent.setMetadata(PlayerID, "endPos", undefined);
...
KONOHA.TransportPlan.prototype.Serialize = function()
{
	return {
		"ID": this.ID,
...
KONOHA.TransportPlan.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
